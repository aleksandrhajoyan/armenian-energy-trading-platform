...
The current Phase 3 orchestration foundation is four application-owned seams:
...
No current node calls the failure policy. No retries or fallbacks execute.
...
LangGraph ≠ agent interface. The skeleton does not invoke agents. Four concrete agents exist and remain unwired. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.
...
Four concrete agents exist. LangGraph still invokes none. No weather, hydro, generation, or news provider exists. No weather, hydro, generation, or news persistence exists. No retry/fallback execution exists.
