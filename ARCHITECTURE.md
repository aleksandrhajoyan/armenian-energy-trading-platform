...
The workflow step exists as an application-only composition. It returns the original `WorkflowState` unchanged and does not mutate phase or status. Application-owned `advance_after_forecasting` is a separate pure control-state transition from a valid `forecasting`/`running` snapshot to `risk_and_bid`/`running`. The context port exists only as a Protocol. It externalizes Phase-3 payload ownership: plan retrieval and success publication stay outside the seven-field `WorkflowState` snapshot. `WorkflowState` does not carry forecast requests, forecast outputs, the plan, or the success aggregate. The plan does not execute either agent, does not imply parallelism or Consumer → DAM sequencing, and does not expand `WorkflowState`. `ForecastingSuccess` reuses canonical `LoadForecastPoint` and `PriceForecastPoint` tuples and likewise does not execute agents or choose an execution order. `ForecastingExecutionPort` is the async application seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. The execution port exists; a concrete executor does not. The context port exists; a concrete context implementation does not. Graph wiring does not. Execution ordering remains undecided. Phase 3 execution remains absent.
...
- `tests/architecture/test_regulatory_intelligence_graph_wiring_boundary.py` — application LangGraph may import `RegulatoryIntelligenceWorkflowNodeAdapter` only; AST proves `build_workflow_graph` requires keyword-only `regulatory_intelligence_node`, `parallel_ingestion_step`, and `parallel_ingestion_failure_runtime_handler` with no defaults; the Regulatory node delegates exactly once to `.run(state)` with no `try/except`; entry routing recognizes exactly contract/running and ingestion/running and otherwise fails closed; topology is 4 nodes / 3 direct edges / 2 conditional-edge registrations with `regulatory_intelligence → END`; no CONTRACT→INGESTION transition, no Pricing & Sales, no `WorkflowState` expansion, and no API/production graph composition.
...
Broader ML/agent import rules remain for later chunks. Chunk 118 locked the first Consumer Load Forecast application↔ML port: `application model port ← future ml adapter`. Chunk 119 added `ConsumerLoadForecastAgent` as `application agent → application model port ← future ml adapter`. The agent contains no numerical forecasting logic. Chunk 120 locked the DAM Price Forecast application↔ML port. Chunk 121 added `DAMPriceForecastAgent` as `application agent → application model port ← future ml adapter`. Chunk 122 added application-owned `ForecastingPlan` as the typed pair of those already-published request DTOs. Chunk 123 added application-owned `ForecastingSuccess` as the typed pair of existing canonical `LoadForecastPoint` and `PriceForecastPoint` tuples. Chunk 124 added application-owned `ForecastingExecutionPort` as the async seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Chunk 125 added application-owned `ForecastingWorkflowContextPort` as the typed external payload seam `resolve_plan(*, state) -> ForecastingPlan` and `record_success(*, state, success) -> None`. Chunk 126 added application-owned `ForecastingWorkflowStep` as the resolve → execute → record composition over those two ports. Chunk 127 added application-owned `advance_after_forecasting` as the separate pure success transition from `forecasting`/`running` to `risk_and_bid`/`running`. Both forecasting agents are thin application delegates; neither contains numerical ML logic. The DAM agent performs no currency conversion. The plan, success aggregate, execution port, context port, workflow step, and success transition do not execute either agent and do not choose concurrency or Consumer → DAM ordering. The workflow step exists; a concrete forecasting executor does not. A concrete forecasting context adapter does not. Graph wiring does not. Concrete ML adapters remain outer implementations. Currency is explicit `CurrencyCode` and is not inferred from history.
...
A LangGraph runtime now exists in the application orchestration layer. Application-owned `WorkflowState` from Chunk 27 remains the authoritative graph schema. Factory `build_workflow_graph(*, regulatory_intelligence_node: RegulatoryIntelligenceWorkflowNodeAdapter, parallel_ingestion_step: ParallelIngestionWorkflowStep, parallel_ingestion_failure_runtime_handler: ParallelIngestionFailureRuntimeHandlingService)` constructs and compiles a fresh graph on every call. The Regulatory node adapter, Phase 2 workflow step, and outer runtime failure handler are required through keyword-only dependency injection. Current topology is:
...
3. LangGraph runtime (`START → workflow_entry`, then contract/running → Regulatory → `END` or ingestion/running → parallel ingestion then success or terminal-failure routing)
...
The workflow step now exists as `async run(self, state: WorkflowState) -> WorkflowState`. It injects the published context and execution ports, preserves typed object identity, and returns the original snapshot unchanged. It does not mutate phase or status. `advance_after_forecasting(state: WorkflowState) -> WorkflowState` is a separate pure function. Valid source is `FORECASTING`/`RUNNING`. Destination is `RISK_AND_BID`/`RUNNING`. It returns a new immutable snapshot and does not embed forecast payloads. LangGraph does not import or call it. The context port now exists as keyword-only `async resolve_plan(*, state: WorkflowState) -> ForecastingPlan` and keyword-only `async record_success(*, state: WorkflowState, success: ForecastingSuccess) -> None`. It exists only as an interface. It externalizes Phase-3 payload ownership. A concrete context implementation does not. The execution port now exists as keyword-only `async execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. A concrete executor does not. Graph wiring does not. Execution ordering remains undecided. `ForecastingSuccess` fields are exactly `consumer_load_forecast: tuple[LoadForecastPoint, ...]` and `dam_price_forecast: tuple[PriceForecastPoint, ...]`. Canonical forecast points are reused; empty tuples remain valid successful outputs. There is no duplicate result schema, no cross-result timestamp/horizon invariant, no execution ordering, and no concurrency or Consumer → DAM dependency decision. Everything after these seams remains absent: no executor, no context adapter, no LangGraph route, no Phase-3 failure transition, no ML adapter, no feature pipeline, and no runtime/API composition. Both aggregates retain supplied objects unchanged. `WorkflowState` remains the seven-field snapshot and does not carry the forecast payload. Phase 3 is not executed.
