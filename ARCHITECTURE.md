...
| Concern | Owner |
| --- | --- |
| Load forecast numbers | `ml/consumer_load` behind application ports. Current unwired implementation: `PreviousDayPersistenceConsumerLoadForecastModel` (exact `T - 24h` persistence baseline, not a trained champion). Invoked by Consumer Load Forecast Agent only after future wiring. |
