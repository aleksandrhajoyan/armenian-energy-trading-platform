...
- `tests/architecture/test_openai_query_embedding_boundary.py` — `OpenAIDocumentQueryEmbeddingAdapter` lives in `infrastructure/embeddings`; OpenAI SDK imports are confined to the two approved OpenAI adapter modules (`infrastructure/embeddings/openai_query_embedding.py` and `infrastructure/regulatory/openai_constraint_inference.py`); domain/application/API/ML remain OpenAI-free; the adapter structurally satisfies `DocumentQueryEmbeddingPort` without inheriting it; public result is `DocumentQueryEmbedding`; AST proves `embeddings.create` with `encoding_format="float"`, no chat/completions/responses, no client construction, no API-key/environment access, no retry loop; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired.
- `tests/architecture/test_openai_regulatory_constraint_inference_boundary.py` — `OpenAIRegulatoryConstraintInferenceAdapter` lives in `infrastructure/regulatory`; it structurally satisfies `RegulatoryConstraintInferencePort` without inheriting it; public result is `tuple[RegulatoryConstraint, ...]`; OpenAI SDK imports remain confined to the two approved adapter modules; domain/application/API/ML remain OpenAI-free; AST proves `responses.parse` with a private `text_format`, no embeddings/chat/completions, no client construction, no API-key/environment access, no retry loop, no Qdrant/LangGraph/FastAPI/generic `LLMPort`; evidence checking is infrastructure-local; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired.
