...
LangGraph ≠ agent interface. The skeleton does not invoke agents. Two concrete agents exist and remain unwired. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.
...
Two concrete agents exist. LangGraph still invokes none. No weather or hydro provider exists. No weather or hydro persistence exists. No retry/fallback execution exists.
