...
The workflow step exists as an application-only composition. It returns the original `WorkflowState` unchanged and does not mutate phase or status. LangGraph injects that already-composed step on `FORECASTING`/`RUNNING` (`workflow_entry` → `forecasting` → `END`) and does not apply `advance_after_forecasting`. Application-owned `advance_after_forecasting` is a separate pure control-state transition from a valid `forecasting`/`running` snapshot to `risk_and_bid`/`running` and remains unwired. The context port exists only as a Protocol. It externalizes Phase-3 payload ownership: plan retrieval and success publication stay outside the seven-field `WorkflowState` snapshot. `WorkflowState` does not carry forecast requests, forecast outputs, the plan, or the success aggregate. The plan does not execute either agent, does not imply parallelism or Consumer → DAM sequencing, and does not expand `WorkflowState`. `ForecastingSuccess` reuses canonical `LoadForecastPoint` and `PriceForecastPoint` tuples and likewise does not execute agents or choose an execution order. `ForecastingExecutionPort` is the async application seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. The execution port exists; a concrete executor does not. The context port exists; a concrete context implementation does not. Graph wiring of the success transition does not. Execution ordering remains undecided. Phase 3 execution remains absent.
...
3. LangGraph runtime (`START → workflow_entry`, then contract/running → Regulatory → `END`, ingestion/running → parallel ingestion then success or terminal-failure routing, or forecasting/running → `forecasting` → `END`)
