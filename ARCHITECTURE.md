...
- `tests/architecture/test_regulatory_intelligence_workflow_node_boundary.py` — application `RegulatoryIntelligenceWorkflowNodeAdapter` lives in `application/orchestration`; it injects existing `RegulatoryIntelligenceWorkflowContextPort` and `RegulatoryIntelligenceWorkflowStep`; public operation is async `run(state: WorkflowState) -> WorkflowState`; AST proves exactly one `resolve_request`, one `workflow_step.run`, and one `record_result`, with original `WorkflowState` returned unchanged and no reconstruction/`try/except`/generic payload bag; no API/infrastructure/config/LangGraph/provider/DB/cache imports; `graph.py` remains Regulatory-unwired (no node/edge/topology change); HTTP, production runtime, and `create_app()` remain unaware of the adapter.
