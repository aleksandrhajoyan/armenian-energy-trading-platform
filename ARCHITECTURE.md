...
- `tests/architecture/test_openai_query_embedding_boundary.py` — `OpenAIDocumentQueryEmbeddingAdapter` lives in `infrastructure/embeddings`; OpenAI SDK imports are confined to that adapter; domain/application/API/ML remain OpenAI-free; the adapter structurally satisfies `DocumentQueryEmbeddingPort` without inheriting it; public result is `DocumentQueryEmbedding`; AST proves `embeddings.create` with `encoding_format="float"`, no chat/completions/responses, no client construction, no API-key/environment access, no retry loop; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired.
...
The adapter receives an already-constructed `AsyncOpenAI` client and an explicit model string. It does not load API keys, construct the client, or become the Chunk 67 composition root.
...
Chunk 65 implements the application-owned query-preparation composition. `DocumentVectorSearchQueryPreparationService.prepare(*, query_text, limit)` awaits `DocumentQueryEmbeddingPort.embed_query` exactly once and returns existing `DocumentVectorSearchQuery(vector=embedding.vector, limit=limit)`. It does not search, rewrite query text, default or clamp the limit, or inject into Regulatory Intelligence. Chunk 68 added an infrastructure OpenAI adapter behind the embedding port; OpenAI client/runtime composition and actual regulatory RAG remain deferred.
