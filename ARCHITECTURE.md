...
`workflow_entry` is an internal async no-op: it does not call an agent, change phase or status, append diagnostics, perform I/O, persist state, or consult `FailurePolicyPort`. `parallel_ingestion` is a thin async node whose only application behavior is `await parallel_ingestion_step.run(state)` and return of that `WorkflowState`. The graph does not construct plans, record success, call agents, translate exceptions, mutate phase/status, or invoke `advance_after_parallel_ingestion`. Compilation is plain (no checkpointer, store, cache, or interrupt). There is no API composition wiring and no concrete context implementation. Direct project dependency is `langgraph>=1.2.11,<1.3`. Direct `langchain` and LLM-provider packages are not project dependencies. Project source must not import `langchain` or `langchain_core`.

Graph nodes must remain thin: they load canonical snapshots, call an agent or use case through application abstractions, write canonical snapshots, and record diagnostics. Nodes depend on application abstractions, not on concrete infrastructure or ML packages. The current Phase 2 node depends only on `ParallelIngestionWorkflowStep`. The graph itself still does not know the executor, context implementation, five agents, infrastructure, storage, failure policy, or the Phase 2 success-transition function.

The current Phase 3/4 orchestration foundation is nine application-owned seams:

1. Agent execution contract (`AgentName`, `AgentPort`)
2. Immutable framework-neutral `WorkflowState`
3. LangGraph runtime (`START → workflow_entry → parallel_ingestion → END`)
...
No current node calls the failure policy. No retries or fallbacks execute. The graph injects `ParallelIngestionWorkflowStep` only. It does not import the plan, execution port, success aggregate, executor, context port, or `advance_after_parallel_ingestion`. The executor remains an application-layer coordinator. The context port remains Protocol-only. Successful Phase 2 phase-transition policy exists outside LangGraph; the graph still ends after `parallel_ingestion` without applying it.
...
`ParallelIngestionWorkflowStep` coordinates that sequence. It depends only on the two application ports. It returns the original `WorkflowState` object unchanged and performs no phase/status transition. Failures propagate naturally. There is no retry, fallback, or degraded policy. There is still no concrete context implementation and no API/composition wiring.
...
The function requires `WorkflowPhase.INGESTION` and `WorkflowStatus.RUNNING`. It returns a new frozen snapshot via `dataclasses.replace`, preserving `workflow_id`, `portfolio_id`, `delivery_date`, `correlation_id`, and `diagnostics`. Any other phase or status fails closed as `InvalidRequestError`. The graph still does not call this function. Phase 3 is not executed.
...
`parallel_ingestion` awaits the injected step and returns the resulting `WorkflowState`. The graph still does not import the executor, context port, five agents, `FailurePolicyPort`, or `advance_after_parallel_ingestion`. There is no concrete context implementation and no `create_app()` wiring. Actual orchestrator Phase 2 join remains future work.
...
Wrong phase or status fails closed as `InvalidRequestError`. The input snapshot is not mutated. The graph still ends after `parallel_ingestion` without applying this transition. Phase 3 agents are not invoked.
