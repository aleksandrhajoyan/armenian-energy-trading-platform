...
DAM Price Forecast numerical inference is owned by application `DAMPriceForecastModelPort`. A future DAM Price Forecast Agent would depend on that port; the agent does not exist yet. Dependency direction is:
...
Broader ML/agent import rules remain for later chunks. Chunk 118 locked the first Consumer Load Forecast application↔ML port: `application model port ← future ml adapter`. Chunk 119 added `ConsumerLoadForecastAgent` as `application agent → application model port ← future ml adapter`. The agent contains no numerical forecasting logic. Chunk 120 locked the DAM Price Forecast application↔ML port: `future application agent → application model port ← future ml adapter`. The DAM agent does not exist yet. Currency is explicit `CurrencyCode` and is not inferred from history; this boundary does not convert currencies.
