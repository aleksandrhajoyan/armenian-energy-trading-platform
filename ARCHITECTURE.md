...
Broader ML/agent import rules remain for later chunks. Chunk 118 locked the first Consumer Load Forecast application↔ML port: `application model port ← future ml adapter`. Chunk 119 added `ConsumerLoadForecastAgent` as `application agent → application model port ← future ml adapter`. The agent contains no numerical forecasting logic. Chunk 120 locked the DAM Price Forecast application↔ML port. Chunk 121 added `DAMPriceForecastAgent` as `application agent → application model port ← future ml adapter`. Both forecasting agents are thin application delegates; neither contains numerical ML logic. The DAM agent performs no currency conversion. Concrete ML adapters remain outer implementations. Currency is explicit `CurrencyCode` and is not inferred from history.
