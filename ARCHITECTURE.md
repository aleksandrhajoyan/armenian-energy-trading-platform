...
Broader ML/agent import rules remain for later chunks. Chunk 118 locked the first Consumer Load Forecast application↔ML port: `application model port ← future ml adapter`. Chunk 119 added `ConsumerLoadForecastAgent` as `application agent → application model port ← future ml adapter`. The agent contains no numerical forecasting logic. Chunk 120 locked the DAM Price Forecast application↔ML port. Chunk 121 added `DAMPriceForecastAgent` as `application agent → application model port ← future ml adapter`. Chunk 122 added application-owned `ForecastingPlan` as the typed pair of those already-published request DTOs. Chunk 123 added application-owned `ForecastingSuccess` as the typed pair of existing canonical `LoadForecastPoint` and `PriceForecastPoint` tuples. Chunk 124 added application-owned `ForecastingExecutionPort` as the async seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Chunk 125 added application-owned `ForecastingWorkflowContextPort` as the typed external payload seam `resolve_plan(*, state) -> ForecastingPlan` and `record_success(*, state, success) -> None`. Chunk 126 added application-owned `ForecastingWorkflowStep` as the resolve → execute → record composition over those two ports. Chunk 127 added application-owned `advance_after_forecasting` as the separate pure success transition from `forecasting`/`running` to `risk_and_bid`/`running`. Chunk 128 wires LangGraph injection of that already-composed `ForecastingWorkflowStep` on `FORECASTING`/`RUNNING` without applying the success transition. Both forecasting agents are thin application delegates; neither contains numerical ML logic. The DAM agent performs no currency conversion. The plan, success aggregate, execution port, context port, workflow step, and success transition do not execute either agent and do not choose concurrency or Consumer → DAM ordering. The workflow step exists and can be invoked by LangGraph; a concrete forecasting executor does not. A concrete forecasting context adapter does not. Graph wiring of `advance_after_forecasting` does not. Concrete ML adapters remain outer implementations. Currency is explicit `CurrencyCode` and is not inferred from history.
