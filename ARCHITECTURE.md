...
Chunk 26 introduced the framework-neutral agent invocation seam that future LangGraph nodes must use:
...
`AgentName` is the closed set of 13 canonical identities from `AGENTS.md`. `AgentPort` is a generic `typing.Protocol`. The only shared operations are identity (`name`) and `async run(request)`. There is no shared workflow snapshot, retry, fallback, registry, or base class. Concrete agents, when added, satisfy the protocol structurally. Infrastructure, ML, FastAPI, n8n, and vendor SDK types must not cross this interface.
