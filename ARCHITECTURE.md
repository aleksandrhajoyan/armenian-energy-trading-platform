...
Chunk 70 implements the missing OpenAI client-construction foundation. Typed `OpenAISettings` lives in `shared/config` with a required `api_key: SecretStr` (`ENERGY_OPENAI_API_KEY`) and no model fields; it is loaded separately from `AppSettings` and is not required for process health or `create_app()`. The settings module does not import the OpenAI SDK. Infrastructure `create_openai_client(settings)` synchronously constructs `AsyncOpenAI` from `settings.api_key.get_secret_value()` with `max_retries=0`, performs no network request, creates no global client, and owns no adapter lifecycle. Production OpenAI SDK imports are exactly three infrastructure modules: `openai_query_embedding.py`, `openai_constraint_inference.py`, and `openai/client.py`. Neither adapter, the Chunk 67 builder, `create_app()`, nor LangGraph is wired to this factory.
