...
`AgentName` is the closed set of 13 canonical identities from `AGENTS.md`. `AgentPort` is a generic `typing.Protocol`. The only shared operations are identity (`name`) and `async run(request)`. There is no retry, fallback, registry, or base class. Concrete agents, when added, satisfy the protocol structurally. Infrastructure, ML, FastAPI, n8n, and vendor SDK types must not cross this interface.

LangGraph ≠ agent interface. The skeleton does not invoke agents. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.

Chunk 27 introduced the framework-neutral application workflow snapshot. LangGraph consumes that contract as `state_schema`; it does not define a second state object. No transition policy exists, no persistence exists, and no concrete agent exists. Phase-specific canonical output slices remain deferred.
