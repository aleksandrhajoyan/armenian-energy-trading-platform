...
- `tests/architecture/test_parallel_ingestion_plan_boundary.py` — application `ParallelIngestionPlan` imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, or ML/LLM libraries. The DTO has exactly the five existing typed request fields and annotations. No `Any`/`dict`/`Mapping` payload, callbacks, exception objects, graph/runtime types, execution helpers, or fan-in result types. `WorkflowState`, the skeleton graph, `FailurePolicyPort`, and `create_app()` remain unwired.
...
5. Framework-neutral parallel-ingestion fan-out plan (`ParallelIngestionPlan`)

No current node calls the failure policy. No retries or fallbacks execute. `ParallelIngestionPlan` is not consumed by the graph.
...
Five concrete agents exist. LangGraph still invokes none. Application-owned `ParallelIngestionPlan` can hold the five existing typed Phase 2 requests together; it does not execute them. No weather, hydro, generation, news, or market provider exists. No weather, hydro, generation, news, or market persistence exists. No retry/fallback execution exists. Parallel execution and fan-in/join remain future work.
...
It reuses the existing request DTOs. It is not `WorkflowState`. It does not execute agents, use LangGraph, define fan-in/results, or derive `location_id` / `resource_id` / `asset_id` / `market_id` from `portfolio_id`. Horizons are not required to be equal. Actual parallel execution and join remain future work.
