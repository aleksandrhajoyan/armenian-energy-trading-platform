...
The Regulatory application layer does not parse PDFs, call an LLM SDK, embed query text, or import Qdrant types. No prompt, provider, model, collection, score, or raw document crosses the agent boundary. Empty retrieval is fail-safe: no evidence yields no constraints, and inference is not invoked. The agent does not construct `RegulatoryConstraint` values and does not hardcode Armenian DAM rules. There is no concrete inference provider. The graph does not import this agent. Actual regulatory RAG is not operational.
