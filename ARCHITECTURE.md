...
The current Phase 3/4 orchestration foundation is six application-owned seams:
...
No current node calls the failure policy. No retries or fallbacks execute. Neither the plan, the execution port, the success aggregate, nor the executor is consumed by the graph. The executor is an application-layer coordinator only.
