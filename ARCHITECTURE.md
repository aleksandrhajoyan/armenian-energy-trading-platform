...
LangGraph ≠ agent interface. The skeleton does not invoke agents. Three concrete agents exist and remain unwired. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.
...
Three concrete agents exist. LangGraph still invokes none. No weather, hydro, or generation provider exists. No weather, hydro, or generation persistence exists. No retry/fallback execution exists.
