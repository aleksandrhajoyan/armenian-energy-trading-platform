...
The route exists but is not installed in `create_app()`. There is no raw document/file acquisition surface.
...
Remaining indexing gaps: HTTP indexing route / later API entrypoint; document acquisition; PDF extraction; OCR fallback; normalized chunk production; actual corpus indexing trigger; reindex/replacement strategy; collection-management strategy; Regulatory contract-phase integration.
