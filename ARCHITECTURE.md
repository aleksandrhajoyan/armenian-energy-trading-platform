...
`AgentName` is the closed set of 13 canonical identities from `AGENTS.md`. `AgentPort` is a generic `typing.Protocol`. The only shared operations are identity (`name`) and `async run(request)`. There is no retry, fallback, registry, or base class. The first concrete agent, Weather & Renewable Forecast Agent, satisfies the protocol structurally. Infrastructure, ML, FastAPI, n8n, and vendor SDK types must not cross this interface.

LangGraph ≠ agent interface. The skeleton does not invoke agents. One concrete agent exists and remains unwired. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.
...
One concrete agent exists. LangGraph still invokes none. No weather provider exists. No weather persistence exists. No retry/fallback execution exists.
