...
The current Phase 3/4 orchestration foundation is seven application-owned seams:
...
No current node calls the failure policy. No retries or fallbacks execute. Neither the plan, the execution port, the success aggregate, the executor, nor the context port is consumed by the graph. The executor is an application-layer coordinator only. The context port is Protocol-only.

Future Phase 2 graph-node interaction (not implemented):
...
`WorkflowState` stays the published seven-field control snapshot. Plan and success objects are not embedded in workflow state. The context port is storage-neutral: it does not choose in-memory, Redis, PostgreSQL, or LangGraph checkpoint storage. There is no concrete context implementation, no LangGraph wiring, and no failure/degraded semantics yet.
...
`workflow_id` reuses the published `WorkflowState.workflow_id` type (`str`). The port does not embed plan or success on `WorkflowState`, does not choose storage technology, and has no concrete production implementation. LangGraph remains `START → workflow_entry → END`. Actual orchestrator Phase 2 join remains future work.
