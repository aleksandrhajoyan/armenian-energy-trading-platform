...
- Consumer Load Forecast numerical inference is owned by application `ConsumerLoadForecastModelPort`. Application-owned `ConsumerLoadForecastAgent` depends on that port and contains no numerical forecasting logic. Dependency direction is:
...
The request requires caller-supplied `forecast_run_id` (`EntityId`) and `generated_at` (`UtcDateTime`) plus the target consumer with canonical `EntityId` (`consumer_id`), independently of whether historical observations are supplied, plus canonical `ConsumptionRecord` history whose quantity is load/power in **MW** (`value_mw`) and explicit UTC `target_timestamps`. Those identity fields are not generated by the request DTO, the agent, a clock, or a UUID factory, and they are not derived from `workflow_id`, target timestamps, or historical observation timestamps. When history is present, every record must belong to that same consumer; empty history remains valid; identity is never inferred from an arbitrary first observation. Output reuses existing `LoadForecastPoint` (`value_mw`). MW and MWh are not interchangeable; this port does not convert. ML-owned `PreviousDayPersistenceConsumerLoadForecastModel` structurally implements the port as a deterministic exact `T - 24h` MW persistence baseline: for each requested target `T` it copies `value_mw` from the unique history observation at exactly `T - 24 hours`, passes through caller-supplied identity, and fails closed on missing or duplicate exact lag observations. It is a benchmark/reference implementation, not a production-trained champion model, and remains unwired from agents, API composition, FastAPI, LangGraph, and `ForecastingExecutionPort`. ML-owned `Lag24h168hOLSConsumerLoadForecastModel` is an unwired two-feature OLS candidate that implements the same port from already-fitted Chunk 143 parameters and exact `T - 24h` / `T - 168h` history; negative finite live predictions fail closed because `LoadForecastPoint.value_mw` remains `NonNegativeMW`; it is not a selected production model. Vendor-specific arrays, DataFrames, ndarrays, model files, and LightGBM/XGBoost classes may exist internally in a future trained ML adapter but must not cross into application or domain contracts. There is no LangGraph wiring of this port.

DAM Price Forecast numerical inference is owned by application `DAMPriceForecastModelPort`. Application-owned `DAMPriceForecastAgent` depends on that port and contains no numerical forecasting logic. Dependency direction is:
...
DAMPriceForecastModelRequest
      ...
      → DAMPriceForecastModelPort.forecast(...)
      → tuple[PriceForecastPoint, ...]
```

The request identifies the target market with canonical `EntityId` (`market_id`) and the target currency with canonical `CurrencyCode`, both independently of whether historical observations are supplied, plus canonical `MarketPriceRecord` history and explicit UTC `target_timestamps`. Currency is never inferred from history. When history is present, every record must belong to that same market and carry that same currency; mixed-market or mixed-currency history fails closed. Empty history remains valid. This boundary does not convert currencies. Output reuses existing `PriceForecastPoint` (`EnergyPrice`). The agent does not reconstruct points, convert currencies, or calculate prices. Vendor arrays/dataframes remain inside a future outer ML adapter. There is no production adapter or LangGraph wiring of this agent or port.

Both forecasting agents are application-owned thin delegates. Neither contains numerical ML logic. The DAM agent performs no currency conversion. Concrete ML adapters remain outer implementations:

```text
ConsumerLoadForecastAgent → ConsumerLoadForecastModelPort ← PreviousDayPersistenceConsumerLoadForecastModel / Lag24h168hOLSConsumerLoadForecastModel

DAMPriceForecastAgent → DAMPriceForecastModelPort ← future ML adapter
```

Those two published request DTOs can be composed, without execution, into application-owned `ForecastingPlan`. Application-owned `ForecastingWorkflowStep` composes the published context and execution ports:

```text
WorkflowState
  → ForecastingWorkflowStep
  → ForecastingWorkflowContextPort.resolve_plan
  → ForecastingPlan
  → ForecastingExecutionPort.execute
  → ForecastingSuccess
  → ForecastingWorkflowContextPort.record_success
  → unchanged WorkflowState
  → advance_after_forecasting
  → risk_and_bid / running
```

The workflow step exists as an application-only composition. It returns the original `WorkflowState` unchanged and does not mutate phase or status. LangGraph injects that already-composed step on `FORECASTING`/`RUNNING` (`workflow_entry` → `forecasting` → `forecasting_success_transition` → `END`). Application-owned `advance_after_forecasting` is the sole owner of the pure control-state transition from a valid `forecasting`/`running` snapshot to `risk_and_bid`/`running`; LangGraph node `forecasting_success_transition` delegates to it and does not reconstruct that policy. `ForecastingWorkflowContextPort` is the application-owned Protocol. It externalizes Phase-3 payload ownership: plan retrieval and success publication stay outside the seven-field `WorkflowState` snapshot. `WorkflowState` does not carry forecast requests, forecast outputs, the plan, or the success aggregate. The plan DTO does not execute either agent and does not encode concurrency. `ForecastingSuccess` reuses canonical `LoadForecastPoint` and `PriceForecastPoint` tuples and likewise does not execute agents. `ForecastingExecutionPort` is the async application seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Application-owned `ParallelForecastingExecutionService` structurally implements that port by running Consumer Load and DAM agents concurrently with `asyncio.TaskGroup`, attributing ordinary branch failures as `ForecastingAgentFailure`, and constructing `ForecastingSuccess` only after both branches succeed. Empty tuples remain valid. Ordinary branch `Exception` values are attributed as `ForecastingAgentFailure` with canonical `AgentName` before native TaskGroup aggregation; sibling cancellation is not wrapped. Native TaskGroup cancellation and exception aggregation still apply. The executor does not own context, `WorkflowState`, retry, or model selection, and remains unwired from LangGraph/`create_app()`. Infrastructure `InMemoryForecastingWorkflowContext` exists as an unwired local/dev structural implementation of the context Protocol. There is no production/durable forecasting context implementation and no Redis/PostgreSQL forecasting context. LangGraph still depends on the already-composed `ForecastingWorkflowStep`, not the concrete executor or in-memory adapter. Application-owned `fail_after_forecasting` is the separate pure terminal failure replacement from a valid `forecasting`/`running` snapshot to `forecasting`/`failed`; it preserves identity and diagnostics, does not accept exceptions, and remains unwired from LangGraph, the executor, and the workflow step. Phase 3 remains incomplete.
...
- `tests/architecture/test_regulatory_intelligence_workflow_node_boundary.py` — application `RegulatoryIntelligenceWorkflowNodeAdapter` lives in `application/orchestration`; it injects existing `RegulatoryIntelligenceWorkflowContextPort` and `RegulatoryIntelligenceWorkflowStep`; public operation is async `run(state: WorkflowState) -> WorkflowState`; AST proves exactly one `resolve_request`, one `workflow_step.run`, and one `record_result`, with original `WorkflowState` returned unchanged and no reconstruction/`try/except`/generic payload bag; no API/infrastructure/config/LangGraph/provider/DB/cache imports; the adapter itself remains LangGraph-free; `graph.py` may import and inject the adapter but must not import the context port or workflow step; HTTP, production runtime, and `create_app()` remain unaware of the adapter.
- `tests/architecture/test_regulatory_intelligence_graph_wiring_boundary.py` — application LangGraph may import `RegulatoryIntelligenceWorkflowNodeAdapter` only among Regulatory internals; AST proves `build_workflow_graph` requires keyword-only `regulatory_intelligence_node`, `parallel_ingestion_step`, `parallel_ingestion_failure_runtime_handler`, and `forecasting_step` with no defaults; the Regulatory node delegates exactly once to `.run(state)` with no `try/except`; entry routing recognizes contract/running, ingestion/running, and forecasting/running and otherwise fails closed; topology is 6 nodes / 5 direct edges / 2 conditional-edge registrations with `regulatory_intelligence → END` and `forecasting → forecasting_success_transition → END`; no CONTRACT→INGESTION transition, no Pricing & Sales, no `WorkflowState` expansion, and no API/production graph composition.
...
- `tests/architecture/test_langgraph_boundary.py` — production `langgraph` imports are allowed only in `application/orchestration/graph.py`. `state.py`, `failure_policy.py`, `parallel_ingestion.py`, `forecasting_plan.py`, `forecasting_success.py`, `forecasting_execution.py`, `forecasting_executor.py`, `forecasting_context.py`, `forecasting_workflow.py`, `forecasting_transition.py`, `forecasting_failure_transition.py`, `forecasting_agent_failure.py`, `forecasting_exception_group.py`, `forecasting_failure_fact.py`, `forecasting_failure_classification.py`, `forecasting_failure_context.py`, `forecasting_failure_context_resolution.py`, `forecasting_failure_selection.py`, `forecasting_strict_single_failure_selector.py`, `forecasting_attempt_number.py`, `forecasting_initial_attempt_number_source.py`, `forecasting_initial_failure_policy.py`, `forecasting_failure_decision.py`, `forecasting_failure_action.py`, `forecasting_failure_handling.py`, `parallel_ingestion_context.py`, `parallel_ingestion_executor.py`, `parallel_ingestion_workflow.py`, `parallel_ingestion_failure_context.py`, `parallel_ingestion_failure_context_resolution.py`, `parallel_ingestion_failure_context_preparation.py`, `parallel_ingestion_failure_decision.py`, `parallel_ingestion_failure_transition.py`, `parallel_ingestion_failure_action.py`, `parallel_ingestion_failure_handling.py`, `parallel_ingestion_agent_failure.py`, `parallel_ingestion_exception_group.py`, `parallel_ingestion_failure_fact.py`, `parallel_ingestion_failure_classification.py`, `parallel_ingestion_failure_selection.py`, `parallel_ingestion_attempt_number.py`, `parallel_ingestion_strict_single_failure_selector.py`, `parallel_ingestion_initial_attempt_number_source.py`, `parallel_ingestion_initial_failure_policy.py`, `parallel_ingestion_failure_runtime_handling.py`, `regulatory_intelligence_context.py`, `regulatory_intelligence_workflow_step.py`, `regulatory_intelligence_workflow_node.py`, agent base, domain, infrastructure, ML, API, and unrelated application ports/use cases remain LangGraph-free. The graph module consumes `WorkflowState`, injects `RegulatoryIntelligenceWorkflowNodeAdapter`, `ParallelIngestionWorkflowStep`, `ParallelIngestionFailureRuntimeHandlingService`, and `ForecastingWorkflowStep`, and delegates the Phase-2 success-transition node to `advance_after_parallel_ingestion` and the Phase-3 success-transition node to `advance_after_forecasting`. It imports no agents/infrastructure/ML/API/LangChain/`ParallelIngestionPlan`/`ParallelIngestionSuccess`/`ParallelIngestionExecutionPort`/`ParallelIngestionWorkflowContextPort`/`ConcurrentParallelIngestionExecutor`/`FailurePolicyPort`/`fail_parallel_ingestion`/`ParallelIngestionFailureDecisionService`/`build_parallel_ingestion_failure_policy_context`/`ParallelIngestionFailureContextResolutionService`/`ParallelIngestionFailureContextPreparationService`/`InitialParallelIngestionFailurePolicy`/`execute_parallel_ingestion_failure_action`/`ParallelIngestionFailureHandlingService`/`ParallelIngestionAgentFailure`/`extract_parallel_ingestion_agent_failures`/`classify_parallel_ingestion_agent_failure`/`classify_parallel_ingestion_agent_failures`/`ParallelIngestionFailureFact`/`ParallelIngestionFailureSelectionPort`/`ParallelIngestionAttemptNumberPort`/`StrictSingleParallelIngestionFailureSelector`/`InitialParallelIngestionAttemptNumberSource`/`RegulatoryIntelligenceWorkflowContextPort`/`RegulatoryIntelligenceWorkflowStep`/`ForecastingPlan`/`ForecastingSuccess`/`ForecastingExecutionPort`/`ForecastingWorkflowContextPort`/`ForecastingAgentFailure`/`extract_forecasting_agent_failures`/`ForecastingFailureFact`/`classify_forecasting_agent_failure`/`classify_forecasting_agent_failures`/`ForecastingFailureSelectionPort`/`StrictSingleForecastingFailureSelector`/`ForecastingAttemptNumberPort`/`InitialForecastingAttemptNumberSource`/`InitialForecastingFailurePolicy`/`ForecastingFailureDecisionService`. It may use conditional edges after `workflow_entry` and after `parallel_ingestion` and may import `WorkflowPhase`/`WorkflowStatus`/`InvalidRequestError` for fail-closed routing. It does not use messages, reducers, checkpointers, stores, or `AgentPort`. AST proves the graph catches `BaseExceptionGroup` rather than broad `Exception`/`BaseException`. Factory `build_workflow_graph` requires keyword-only `regulatory_intelligence_node`, `parallel_ingestion_step`, `parallel_ingestion_failure_runtime_handler`, and `forecasting_step`. Topology is `START → workflow_entry`, then contract/running → `regulatory_intelligence` → `END`, or ingestion/running → `parallel_ingestion` then `ingestion`/`running` → `parallel_ingestion_success_transition` → `END` or `ingestion`/`failed` → `END`, or forecasting/running → `forecasting` → `forecasting_success_transition` → `END`. `create_app()` remains unwired.
...
- `tests/architecture/test_consumer_load_forecast_model_boundary.py` — `ConsumerLoadForecastModelPort` lives under application ports and is the only Consumer Load Forecast model Protocol. Request fields are exactly `forecast_run_id: EntityId`, `generated_at: UtcDateTime`, `consumer_id: EntityId`, canonical `history: tuple[ConsumptionRecord, ...]`, and explicit `target_timestamps: tuple[UtcDateTime, ...]`. There are no defaults, clock, UUID factory, or generic metadata/provider/version fields. `forecast` is keyword-only async and returns `tuple[LoadForecastPoint, ...]`. Quantity is MW load/power (`value_mw`), not MWh; there is no conversion. `EntityId` is reused from the existing domain value-object module; no duplicate consumer identifier type is introduced. The port imports none of `energy_trading.ml`, infrastructure, API, LangGraph/FastAPI, pandas/NumPy, sklearn/LightGBM/XGBoost, Redis/Qdrant/SQL, or LLM libraries, and has no `Any`/`dict`/`Mapping`/DataFrame/ndarray/Path payload. There is no generic `ModelPort`/`MLPort`/`ForecastModelPort`. `graph.py` and `WorkflowState` remain unaware. API/composition remains unaware. Application/API do not own a Consumer Load model adapter. Authorized ML live implementations are the unwired previous-day persistence baseline and the unwired two-feature OLS candidate; the port does not import either.
- `tests/architecture/test_consumer_load_forecast_agent_boundary.py` — `ConsumerLoadForecastAgent` lives under application agents and is the only production class of that name. It injects exactly `ConsumerLoadForecastModelPort`, exposes `AgentName.CONSUMER_LOAD_FORECAST`, and structurally satisfies existing `AgentPort[ConsumerLoadForecastModelRequest, tuple[LoadForecastPoint, ...]]` without a Consumer-specific Protocol, generic forecast-agent framework, `AgentFactory`, `AgentRegistry`, or `ServiceLocator`. `run` awaits `.forecast(request=request)` exactly once and returns that tuple without if/match/loops/`try/except`, retry/fallback, or `LoadForecastPoint` construction. The module imports none of `energy_trading.ml`, infrastructure, API, LangGraph/FastAPI, pandas/NumPy, sklearn/LightGBM/XGBoost, Redis/Qdrant/SQL, or LLM libraries, and has no `Any`/`dict`/`Mapping`/DataFrame/ndarray payload. Chunk 131 request shape is five required fields including caller-supplied `forecast_run_id` and `generated_at`. `graph.py` and `WorkflowState` remain unaware. API/composition remains unaware. The agent does not import or construct the previous-day persistence baseline or the two-feature OLS candidate.
- `tests/architecture/test_consumer_load_previous_day_persistence_boundary.py` — `PreviousDayPersistenceConsumerLoadForecastModel` lives under `energy_trading.ml.consumer_load`. It structurally implements `ConsumerLoadForecastModelPort` without inheriting a Protocol, generic `ModelPort`/`ForecastPort`, registry, factory, or service locator. Keyword-only async `forecast(*, request=...)` returns `tuple[LoadForecastPoint, ...]`. The module may depend inward on the Consumer Load request DTO, `InvalidRequestError`, and canonical domain forecast/observation contracts. It imports none of API, infrastructure, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy. It performs no clock/UUID/random identity generation. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The baseline is unwired.
- `tests/architecture/test_consumer_load_previous_day_persistence_backtest_boundary.py` — `PreviousDayPersistenceBacktestCase` and `build_previous_day_persistence_backtest_cases` live under `energy_trading.ml.consumer_load`. The case is a frozen/slots dataclass with exactly `consumer_id`, `target_timestamp`, `predicted_value_mw`, and `actual_value_mw` using canonical identity/time/MW types. The builder is keyword-only sync `history: tuple[ConsumptionRecord, ...] -> tuple[PreviousDayPersistenceBacktestCase, ...]`. The module may depend inward on `InvalidRequestError` and canonical observation/value-object contracts. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, or the Chunk 132 inference adapter. There is no generic evaluator/backtester/model registry, no `ml/common` Python framework, and no metric calculation. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The builder is unwired.
- `tests/architecture/test_consumer_load_previous_day_persistence_evaluation_boundary.py` — `PreviousDayPersistenceMAEResult` and `evaluate_previous_day_persistence_mae` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `case_count` and `mae_mw`. The evaluator is keyword-only sync `cases: tuple[PreviousDayPersistenceBacktestCase, ...] -> PreviousDayPersistenceMAEResult`. The module may depend inward on `InvalidRequestError`, canonical MW types, and the Chunk 133 case type. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, `ConsumptionRecord`, or the Chunk 132 inference adapter. There is no generic evaluator/metric abstraction, no `ml/common` Python framework, and no RMSE/MSE/MAPE implementation. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The evaluator is unwired.
- `tests/architecture/test_consumer_load_lag_24h_features_boundary.py` — `ConsumerLoadLag24hFeatureRow` and `build_consumer_load_lag_24h_feature_rows` live under `energy_trading.ml.consumer_load`. The row is a frozen/slots dataclass with exactly `consumer_id`, `target_timestamp`, `lag_24h_mw`, and `target_value_mw` using canonical identity/time/MW types. The builder is keyword-only sync `history: tuple[ConsumptionRecord, ...] -> tuple[ConsumerLoadLag24hFeatureRow, ...]`. The module may depend inward on `InvalidRequestError` and canonical observation/value-object contracts. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, Chunk 132 inference, Chunk 133 backtest artifacts, or Chunk 134 evaluation artifacts. There is no generic `Feature`/`FeatureVector`/`FeatureSet`/`FeatureRegistry`/`FeaturePipeline`/`TrainingRow` abstraction, no `ml/common` Python framework, and no calendar/weather/rolling extra features. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The builder is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_features_boundary.py` — `ConsumerLoadLag24h168hFeatureRow` and `build_consumer_load_lag_24h_168h_feature_rows` live under `energy_trading.ml.consumer_load`. The row is a frozen/slots dataclass with exactly `consumer_id`, `target_timestamp`, `lag_24h_mw`, `lag_168h_mw`, and `target_value_mw` using canonical identity/time/MW types. The builder is keyword-only sync `history: tuple[ConsumptionRecord, ...] -> tuple[ConsumerLoadLag24h168hFeatureRow, ...]`. Exact `timedelta(hours=24)` and `timedelta(hours=168)` arithmetic is required. The module may depend inward on `InvalidRequestError` and canonical observation/value-object contracts. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, the Chunk 135 one-feature builder, Chunk 136 splitter, Chunk 137 fitter, Chunk 138 predictor, Chunk 139 evaluator, Chunk 140 comparison, or Chunk 132 live inference. There is no generic `Feature`/`Dataset`/`Trainer`/`ml/common` framework and no interpolation/nearest substitution. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 141 module/type/function. `WorkflowState` remains seven fields. The builder is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_chronological_split_boundary.py` — `ConsumerLoadLag24h168hChronologicalFeatureSplit` and `split_consumer_load_lag_24h_168h_feature_rows_chronologically` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `training_rows` and `evaluation_rows`, both tuples of `ConsumerLoadLag24h168hFeatureRow`. The splitter is keyword-only sync `rows: tuple[ConsumerLoadLag24h168hFeatureRow, ...]`, `cutoff: UtcDateTime` → `ConsumerLoadLag24h168hChronologicalFeatureSplit`. Training is `target_timestamp < cutoff`; evaluation is `target_timestamp >= cutoff`. The module may depend inward on `InvalidRequestError`, `UtcDateTime`, and `ConsumerLoadLag24h168hFeatureRow`. It does not import `ConsumptionRecord`, call the Chunk 141 builder, or call the Chunk 136 splitter. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, `train_test_split`, `TimeSeriesSplit`, random/clock/UUID, filesystem/network/database I/O, or a generic `Dataset`/`DatasetSplit`/`Splitter`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 142 module/type/function. `WorkflowState` remains seven fields. The splitter is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_linear_regression_boundary.py` — `ConsumerLoadLag24h168hLinearRegressionFit` and `fit_consumer_load_lag_24h_168h_linear_regression` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `lag_24h_coefficient: float`, `lag_168h_coefficient: float`, and `intercept_mw: float`. The fitter is keyword-only sync `training_rows: tuple[ConsumerLoadLag24h168hFeatureRow, ...] -> ConsumerLoadLag24h168hLinearRegressionFit`. The module may depend inward on `InvalidRequestError`, `math.isfinite`, and `ConsumerLoadLag24h168hFeatureRow`. It does not import `ConsumptionRecord`, the Chunk 142 split type, or the Chunk 137 one-feature fitter. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas/scipy/statsmodels, DAM contracts, `LoadForecastPoint`, prediction APIs, metric APIs, random/clock/UUID, filesystem/network/database I/O, or a generic `Model`/`Trainer`/`Estimator`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 143 module/type/function. `WorkflowState` remains seven fields. The fitter is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_linear_regression_prediction_boundary.py` — `ConsumerLoadLag24h168hLinearRegressionPrediction` and `predict_consumer_load_lag_24h_168h_linear_regression` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `consumer_id: EntityId`, `target_timestamp: UtcDateTime`, `predicted_value_mw: float`, and `actual_value_mw: NonNegativeMW`. The predictor is keyword-only sync `fit: ConsumerLoadLag24h168hLinearRegressionFit`, `evaluation_rows: tuple[ConsumerLoadLag24h168hFeatureRow, ...] -> tuple[ConsumerLoadLag24h168hLinearRegressionPrediction, ...]`. The module may depend inward on `InvalidRequestError`, `math.isfinite`, `EntityId`, `NonNegativeMW`, `UtcDateTime`, `ConsumerLoadLag24h168hFeatureRow`, and the Chunk 143 fit DTO. It imports the fit DTO but does not call the fitter. It does not import `ConsumptionRecord`, the Chunk 142 split type, or the Chunk 138 one-feature predictor. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas/scipy/statsmodels, DAM contracts, `LoadForecastPoint`, metric APIs, random/clock/UUID, filesystem/network/database I/O, or a generic `Predictor`/`Model`/`Trainer`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 144 module/type/function. `WorkflowState` remains seven fields. The predictor is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_linear_regression_evaluation_boundary.py` — `ConsumerLoadLag24h168hLinearRegressionMAEResult` and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `case_count: int` and `mae_mw: float`. The evaluator is keyword-only sync `predictions: tuple[ConsumerLoadLag24h168hLinearRegressionPrediction, ...] -> ConsumerLoadLag24h168hLinearRegressionMAEResult`. The module may depend inward on `InvalidRequestError` and the Chunk 144 prediction DTO. It does not import the Chunk 139 one-feature evaluator, the Chunk 144 predictor function, the Chunk 143 fitter, the Chunk 142 splitter, or `ConsumptionRecord`. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas, DAM contracts, `LoadForecastPoint`, random/clock/UUID, filesystem/network/database I/O, or a generic `Metric`/`Evaluator`/`Dataset`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 145 module/type/function. `WorkflowState` remains seven fields. The evaluator is unwired.
- `tests/architecture/test_consumer_load_lag_24h_vs_lag_24h_168h_ols_comparison_boundary.py` — `ConsumerLoadLag24hVsLag24h168hOLSMAEComparison` and `compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `case_count: int`, `lag_24h_mae_mw: float`, and `lag_24h_168h_mae_mw: float`. The comparison is keyword-only sync over Chunk 138 and Chunk 144 prediction tuples. The module may depend inward on `InvalidRequestError`, the Chunk 138 prediction DTO, the Chunk 144 prediction DTO, the Chunk 139 evaluator, and the Chunk 145 evaluator. It does not import feature builders, splitters, fitters, prediction functions, or the Chunk 140 persistence comparison. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas, DAM contracts, `LoadForecastPoint`, random/clock/UUID, filesystem/network/database I/O, or a generic `Comparator`/`Metric`/`Evaluator`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 146 module/type/function. `WorkflowState` remains seven fields. The comparison is unwired.
- `tests/architecture/test_consumer_load_persistence_vs_lag_24h_168h_ols_comparison_boundary.py` — `ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison` and `compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `case_count: int`, `persistence_mae_mw: float`, and `lag_24h_168h_mae_mw: float`. The comparison is keyword-only sync over Chunk 133 persistence cases and Chunk 144 two-feature prediction tuples. The module may depend inward on `InvalidRequestError`, the Chunk 133 persistence case type, the Chunk 134 persistence evaluator, the Chunk 144 prediction DTO, and the Chunk 145 evaluator. It does not import one-feature OLS prediction/evaluation, feature builders, splitters, fitters, prediction functions, the Chunk 140 persistence-versus-one-feature comparison, or the Chunk 146 one-feature-versus-two-feature comparison. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas, DAM contracts, `LoadForecastPoint`, random/clock/UUID, filesystem/network/database I/O, or a generic `Comparator`/`Metric`/`Evaluator`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 147 module/type/function. `WorkflowState` remains seven fields. The comparison is unwired.
- `tests/architecture/test_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_comparison_boundary.py` — `ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison` and `compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `case_count: int`, `persistence_mae_mw: float`, `lag_24h_mae_mw: float`, and `lag_24h_168h_mae_mw: float`. The comparison is keyword-only sync over Chunk 133 persistence cases, Chunk 138 one-feature prediction tuples, and Chunk 144 two-feature prediction tuples. The module may depend inward on `InvalidRequestError`, the Chunk 133 persistence case type, the Chunk 134 persistence evaluator, the Chunk 138 prediction DTO, the Chunk 139 evaluator, the Chunk 144 prediction DTO, and the Chunk 145 evaluator. It does not import feature builders, splitters, fitters, prediction functions, the Chunk 140 persistence-versus-one-feature comparison, the Chunk 146 one-feature-versus-two-feature comparison, or the Chunk 147 persistence-versus-two-feature comparison. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas, DAM contracts, `LoadForecastPoint`, random/clock/UUID, filesystem/network/database I/O, or a generic `Comparator`/`Metric`/`Evaluator`/`ml/common` framework. Application agents and orchestration/LangGraph remain unaware. API specifically does not reference the Chunk 148 module/type/function. `WorkflowState` remains seven fields. The comparison is unwired.
- `tests/architecture/test_consumer_load_lag_24h_168h_ols_forecast_boundary.py` — `Lag24h168hOLSConsumerLoadForecastModel` lives under `energy_trading.ml.consumer_load`. It structurally implements `ConsumerLoadForecastModelPort` without inheriting a Protocol, generic `ModelPort`/`ForecastPort`, registry, factory, or service locator. Keyword-only constructor injects already-fitted `ConsumerLoadLag24h168hLinearRegressionFit`. Keyword-only async `forecast(*, request=...)` returns `tuple[LoadForecastPoint, ...]`. The module may depend inward on the Consumer Load request DTO, `InvalidRequestError`, canonical domain forecast/observation contracts, and the Chunk 143 fit type. It imports none of API, infrastructure, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, feature builders, splitters, fit functions, Chunk 144 prediction DTOs, MAE evaluators, Chunk 140/146/147/148 comparisons, or the persistence live baseline. An explicit negative-admissibility branch exists before `LoadForecastPoint` construction; clamping/`abs`/`max(0)` are forbidden. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The candidate is unwired and is not a selected production model.
- `tests/architecture/test_consumer_load_chronological_feature_split_boundary.py` — `ConsumerLoadChronologicalFeatureSplit` and `split_consumer_load_feature_rows_chronologically` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `training_rows` and `evaluation_rows`. The splitter is keyword-only sync `rows: tuple[ConsumerLoadLag24hFeatureRow, ...]`, `cutoff: UtcDateTime` → `ConsumerLoadChronologicalFeatureSplit`. Training is `target_timestamp < cutoff`; evaluation is `target_timestamp >= cutoff`. The module may depend inward on `InvalidRequestError`, `UtcDateTime`, and `ConsumerLoadLag24hFeatureRow`. It does not import `ConsumptionRecord`, Chunk 132 inference, Chunk 133 backtest artifacts, or Chunk 134 evaluation artifacts. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/Prophet/pandas/Polars/NumPy, DAM contracts, `LoadForecastPoint`, `train_test_split`, `TimeSeriesSplit`, random/clock/UUID, filesystem/network/database I/O, or a generic `Dataset`/`DatasetSplit`/`Splitter`/`ml/common` framework. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The splitter is unwired.
- `tests/architecture/test_consumer_load_lag_24h_linear_regression_boundary.py` — `ConsumerLoadLag24hLinearRegressionFit` and `fit_consumer_load_lag_24h_linear_regression` live under `energy_trading.ml.consumer_load`. The result is a frozen/slots dataclass with exactly `slope: float` and `intercept_mw: float`. The fitter is keyword-only sync `training_rows: tuple[ConsumerLoadLag24hFeatureRow, ...] -> ConsumerLoadLag24hLinearRegressionFit`. The module may depend inward on `InvalidRequestError`, `math.isfinite`, and `ConsumerLoadLag24hFeatureRow`. It does not import `ConsumptionRecord`, the Chunk 136 split type, Chunk 132 inference, Chunk 133 backtest artifacts, or Chunk 134 evaluation artifacts. It imports none of API, infrastructure, application agents/orchestration/ports, LangGraph/FastAPI/Starlette, LLM/provider libraries, LightGBM/XGBoost/sklearn/NumPy/pandas/scipy/statsmodels, DAM contracts, `LoadForecastPoint`, prediction APIs, metric APIs, random/clock/UUID, filesystem/network/database I/O, or a generic `Model`/`Trainer`/`Estimator`/`ml/common` framework. Application agents, orchestration/LangGraph, and API/composition do not import or construct it. `WorkflowState` remains seven fields. The fitter is unwired.
- `tests/architecture/test_dam_price_forecast_model_boundary.py` — `DAMPriceForecastModelPort` lives under application ports. The module defines exactly `DAMPriceForecastModelRequest` and `DAMPriceForecastModelPort`. Request fields are exactly `market_id: EntityId`, `currency: CurrencyCode`, canonical `history: tuple[MarketPriceRecord, ...]`, and explicit `target_timestamps: tuple[UtcDateTime, ...]`. `forecast` is keyword-only async and returns `tuple[PriceForecastPoint, ...]`. Currency is the existing `CurrencyCode` used by `EnergyPrice`; it is never inferred from history and is not converted. Historical `MarketPriceRecord` and output `PriceForecastPoint` are reused, not duplicated. Authorized application `*ModelPort` classes are exactly `ConsumerLoadForecastModelPort` and `DAMPriceForecastModelPort`; there is no generic `ModelPort`/`MLPort`/`ForecastModelPort`. The port imports none of `energy_trading.ml`, infrastructure, API, LangGraph/FastAPI, pandas/NumPy, sklearn/LightGBM/XGBoost, Redis/Qdrant/SQL, or LLM libraries, and has no `Any`/`dict`/`Mapping`/DataFrame/ndarray/Path payload. `graph.py` and `WorkflowState` remain unaware. API/composition remains unaware. No production ML adapter exists. The Consumer Load Forecast port contract remains unchanged.
- `tests/architecture/test_dam_price_forecast_agent_boundary.py` — `DAMPriceForecastAgent` lives under application agents and is the only production class of that name. It injects exactly `DAMPriceForecastModelPort`, exposes `AgentName.DAM_PRICE_FORECAST`, and structurally satisfies existing `AgentPort[DAMPriceForecastModelRequest, tuple[PriceForecastPoint, ...]]` without a DAM-specific Protocol, generic forecast-agent framework, `AgentFactory`, `AgentRegistry`, or `ServiceLocator`. `run` awaits `.forecast(request=request)` exactly once and returns that tuple without if/match/loops/`try/except`, retry/fallback, or `PriceForecastPoint`/`EnergyPrice` construction. The module imports none of `energy_trading.ml`, infrastructure, API, LangGraph/FastAPI, pandas/NumPy, sklearn/LightGBM/XGBoost, Redis/Qdrant/SQL, or LLM libraries, and has no `Any`/`dict`/`Mapping`/DataFrame/ndarray payload. Chunk 120 request shape remains unchanged. Consumer Load Forecast agent/port remain unchanged. `graph.py` and `WorkflowState` remain unaware. API/composition remains unaware. No production model adapter exists.
- `tests/architecture/test_forecasting_plan_boundary.py` — `ForecastingPlan` lives under application orchestration as a frozen/slots dataclass with exactly `consumer_load_request: ConsumerLoadForecastModelRequest` and `dam_price_request: DAMPriceForecastModelRequest`. The module imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, or ML/LLM libraries. There is no execution Protocol, success aggregate, executor, `asyncio.gather`/`TaskGroup`, agent `.run()` / `.forecast()` invocation, generic `ModelPort`/`MLPort`/`ForecastModelPort`, `Any`/`dict`/`Mapping` payload, or `WorkflowState` expansion. `graph.py`, `create_app()`, and `energy_trading.ml` remain unwired. Different request horizons remain allowed; no cross-request synchronization rule is invented.
- `tests/architecture/test_forecasting_success_boundary.py` — `ForecastingSuccess` lives under application orchestration as a frozen/slots dataclass with exactly `consumer_load_forecast: tuple[LoadForecastPoint, ...]` and `dam_price_forecast: tuple[PriceForecastPoint, ...]`. Canonical forecast points are reused; there is no duplicate load/price result DTO. The module imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, forecast agents/model ports, or ML/LLM libraries. There is no `ForecastingPlan` dependency, execution Protocol, executor, `asyncio.gather`/`TaskGroup`, agent `.run()` / `.forecast()` invocation, generic `ModelPort`/`MLPort`/`ForecastModelPort`, `Any`/`dict`/`Mapping` payload, or `WorkflowState` expansion. Deferred Phase-3 contracts are checked only on this module. `graph.py` and `create_app()` remain unwired. Empty tuples and different horizons remain allowed; no cross-result synchronization rule is invented.
- `tests/architecture/test_forecasting_execution_boundary.py` — `ForecastingExecutionPort` lives under application orchestration as a non-generic Protocol whose only public operation is keyword-only `async execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. The module imports `typing.Protocol`, `ForecastingPlan`, and `ForecastingSuccess` only. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, forecast agents/model ports, asyncio, or ML/LLM libraries. The port module itself has no concrete executor, `asyncio.gather`/`TaskGroup`/`create_task`, agent `.run()` / `.forecast()` invocation, generic `ExecutionPort`/`WorkflowExecutionPort`/`ForecastExecutionPort`/`ModelPort`/`MLPort`/`ForecastModelPort`, `Any`/`dict`/`Mapping` payload, or `WorkflowState` expansion. Deferred Phase-3 executor/context/step types are checked only on this module. `graph.py` and `create_app()` remain unwired. Concurrent Consumer Load and DAM execution is owned by the separate Chunk 150 executor module, not by this Protocol.
- `tests/architecture/test_forecasting_executor_boundary.py` — `ParallelForecastingExecutionService` lives under application orchestration as the only production class of that name and structurally implements existing `ForecastingExecutionPort` without inheriting it. Constructor injects exactly `ConsumerLoadForecastAgent` and `DAMPriceForecastAgent`. Public operation is keyword-only `async execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Production uses `asyncio.TaskGroup` and two `create_task` calls, not `asyncio.gather`/`wait`/`as_completed`/`to_thread`/thread pools. Ordinary branch attribution catches only `Exception` and raises `ForecastingAgentFailure` from the original exception. `execute` itself has no `try/except`. `ForecastingSuccess` is constructed only after both branches succeed. Empty tuples remain valid. Native TaskGroup cancellation and exception aggregation apply on branch failure. The module imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, forecast model ports, concrete ML adapters, `ForecastingWorkflowContextPort`, `WorkflowState`, or ML/LLM libraries. There is no context resolve/record, no model selection/fallback, no generic executor/registry/service locator, no repository/UoW, no retry, and no success/failure workflow transition. `graph.py` and `create_app()` remain unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_context_boundary.py` — `ForecastingWorkflowContextPort` lives under application orchestration as a non-generic Protocol whose only public operations are keyword-only `async resolve_plan(*, state: WorkflowState) -> ForecastingPlan` and keyword-only `async record_success(*, state: WorkflowState, success: ForecastingSuccess) -> None`. The module imports `typing.Protocol`, `WorkflowState`, `ForecastingPlan`, and `ForecastingSuccess` only. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, forecast agents/model ports, `ForecastingExecutionPort`, asyncio, or ML/LLM libraries. There is no concrete context adapter, executor, workflow step, `asyncio.gather`/`TaskGroup`/`create_task`, agent `.run()` / `.forecast()` invocation, generic `WorkflowContextPort`/`ContextPort`/`ExecutionContext`, `Any`/`dict`/`Mapping` payload, or `WorkflowState` expansion. Deferred Phase-3 adapter/step/executor types are checked only on this module. `graph.py` and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_workflow_boundary.py` — `ForecastingWorkflowStep` lives under application orchestration as a nongeneric concrete class whose constructor injects exactly `ForecastingWorkflowContextPort` and `ForecastingExecutionPort`. The only public operation is `async run(self, state: WorkflowState) -> WorkflowState`. AST proves resolve → execute → record with keyword-only port calls, no `try/except`, and return of the original `WorkflowState`. The module imports those two ports plus `WorkflowState` only. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, forecast agents/model ports, asyncio, or ML/LLM libraries. There is no concrete executor, context adapter, `asyncio.gather`/`TaskGroup`/`create_task`, agent `.run()` / `.forecast()` invocation, generic `WorkflowStep`/`AgentFactory`/`ServiceLocator`, Phase-3 transition, `Any`/`dict`/`Mapping` payload, or `WorkflowState` expansion. The step itself remains LangGraph-free. `graph.py` may import/use `ForecastingWorkflowStep` but must not import the step's context or execution ports. `create_app()` remains unwired.
- `tests/architecture/test_forecasting_transition_boundary.py` — `advance_after_forecasting(state: WorkflowState) -> WorkflowState` is the only public surface of `forecasting_transition.py`. The module may import stdlib `dataclasses.replace`, application errors, and orchestration state types. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, forecast agents/model ports, `ForecastingPlan`/`ForecastingSuccess`/`ForecastingExecutionPort`/`ForecastingWorkflowContextPort`/`ForecastingWorkflowStep`, or `FailurePolicyPort`. There is no Protocol, factory, registry, or generic state machine. Valid source is `FORECASTING`/`RUNNING`; destination is `RISK_AND_BID`/`RUNNING`. `WorkflowState` still has no transition methods. The transition module remains LangGraph-free. `graph.py` imports and calls `advance_after_forecasting` exactly once and does not reconstruct transition policy. `create_app()` remains unwired.
- `tests/architecture/test_forecasting_agent_failure_boundary.py` — `ForecastingAgentFailure` lives under application orchestration as the only production class of that name. It subclasses `Exception`, accepts only canonical `AgentName`, stores `agent_name`, and emits a sanitized message. The module imports only `energy_trading.application.agents.base`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, `WorkflowState`, failure-policy types, `fail_after_forecasting`, error codes, retry, or diagnostics. Graph, workflow step, success/failure transitions, and `create_app()` remain unwired. The forecasting executor, `extract_forecasting_agent_failures`, `classify_forecasting_agent_failure`, and `classify_forecasting_agent_failures` may import the type.
- `tests/architecture/test_forecasting_exception_group_boundary.py` — `extract_forecasting_agent_failures` is the only public production function in `forecasting_exception_group.py`. There is no public class, Protocol, ABC, registry, factory, or visitor. Input is a `BaseExceptionGroup`; return type is `tuple[ForecastingAgentFailure, ...]`. The module may import `InvalidRequestError` and `ForecastingAgentFailure`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, `WorkflowState`, `FailurePolicyContext`/`FailurePolicyPort`/`FailureAction`, Phase 2 failure types, forecast agents, the executor, or `ForecastingFailureFact`. Traversal is recursive over `.exceptions` without `.split()` / `.subgroup()` / `.derive()`, without inspecting exception text or `__cause__`, and without error-code mapping. `graph.py`, the executor, the workflow step, `fail_after_forecasting`, and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_failure_fact_boundary.py` — `ForecastingFailureFact` is the only production DTO class and `classify_forecasting_agent_failure` is the only public function in `forecasting_failure_fact.py`. DTO fields are exactly `agent_name` and `error_code`; the DTO is frozen. The module may import stdlib dataclass, `AgentName`, `ApplicationError`, and `ForecastingAgentFailure`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `WorkflowState`, `FailurePolicyContext`/`FailurePolicyPort`/`FailureAction`, Phase 2 failure types, forecast agents, or the executor. There is no Protocol/ABC/registry/factory, no traceback inspection, no exception-message parsing, no ExceptionGroup traversal, and no exception-class-name-derived code. `graph.py`, the extractor, the executor, the workflow step, `fail_after_forecasting`, and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_failure_classification_boundary.py` — `classify_forecasting_agent_failures` is the only public production function in `forecasting_failure_classification.py`. There is no public class, Protocol, ABC, registry, or factory. Input is `tuple[ForecastingAgentFailure, ...]`; return type is `tuple[ForecastingFailureFact, ...]`. The module may import `ForecastingAgentFailure`, `ForecastingFailureFact`, and `classify_forecasting_agent_failure`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `ApplicationError`, `AgentName`, `WorkflowState`, `FailurePolicyContext`/`FailurePolicyPort`/`FailureAction`, ExceptionGroup extractor, executor implementation, forecast agents, or Consumer/DAM enum literals. There is no ExceptionGroup traversal/rebuilding, no `__cause__` inspection, no second error-code mapping, and no sorting/deduplication/grouping/selection machinery. `graph.py`, the extractor, the executor, the workflow step, `fail_after_forecasting`, and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_failure_selection_boundary.py` — `ForecastingFailureSelectionPort` is the only public production Protocol in `forecasting_failure_selection.py`. There is no public function, concrete class, enum, registry, factory, or selector implementation. The only public operation is synchronous `select(self, facts: tuple[ForecastingFailureFact, ...]) -> ForecastingFailureFact`. The module may import typing `Protocol` and `ForecastingFailureFact`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `ApplicationError`, `InvalidRequestError`, `AgentName`, `WorkflowState`, `FailurePolicyContext`/`FailurePolicyPort`/`FailureAction`, attribution/extraction/classification, the executor implementation, forecast agents, or `StrictSingleForecastingFailureSelector`. There is no `[0]`/`[-1]` indexing, `len(facts)`, `min`/`max`/`sorted`, priority tables, or winner semantics. The port module does not import `forecasting_strict_single_failure_selector.py`. `graph.py` and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_attempt_number_boundary.py` — `ForecastingAttemptNumberPort` is the only public production Protocol in `forecasting_attempt_number.py`. There is no public function, concrete class, enum, registry, factory, or tracker implementation in that module. The only public operation is async `get_attempt_number(self, workflow_id: str) -> int`. The module may import typing `Protocol` only. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `ApplicationError`/`InvalidRequestError`, `WorkflowState`/`WorkflowPhase`/`WorkflowStatus`, `FailurePolicyContext`/`FailurePolicyPort`/`FailureAction`, `ForecastingFailureFact`, failure selection / strict selector, attribution / extraction / classification, context builders/resolution, action executors/runtime handlers, forecast executor/workflow step, forecast agents, or storage backends. There is no increment/reset/set/record/begin/complete write API. `graph.py` and `create_app()` remain unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_initial_attempt_number_source_boundary.py` — `InitialForecastingAttemptNumberSource` is the only public production class in `forecasting_initial_attempt_number_source.py`. The only public operation is async `get_attempt_number(self, workflow_id: str) -> int`. The class does not inherit `ForecastingAttemptNumberPort`. The module has no production imports. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `WorkflowState`/`WorkflowPhase`/`WorkflowStatus`, failure facts, the selector, context-resolution/preparation services, `FailureAction`/`FailurePolicyContext`/`FailurePolicyPort`, forecast executor/workflow step, Phase 2 attempt types, or graph. Production code returns the literal integer `1` with no arithmetic, dictionaries, sets, counters, increment/reset/set/record methods, persistence, locks, timestamps, or randomness. The source is not a tracker. The existing resolver continues to depend on `ForecastingAttemptNumberPort`, not this concrete source. `graph.py` and `create_app()` remain unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_initial_failure_policy_boundary.py` — `InitialForecastingFailurePolicy` is the only public production class in `forecasting_initial_failure_policy.py`. The only public operation is async `decide(self, context: FailurePolicyContext) -> FailureAction`. The class does not inherit `FailurePolicyPort`. Allowed imports are `FailureAction` and `FailurePolicyContext`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `WorkflowState`/`WorkflowPhase`/`WorkflowStatus`, `FailurePolicyPort` inheritance, Phase 2 policy, attempt-number types, selection/strict selector, preparation/resolution services, forecast facts/attribution, executor, workflow step, `fail_after_forecasting`, or graph. Production `decide` returns exactly `FailureAction.FAIL` with no `if`/`match` branching, no dictionaries/sets, no `sorted`/`min`/`max`, and no inspection of context fields. It is not a generic policy engine. `graph.py` and `create_app()` remain unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_decision_boundary.py` — `ForecastingFailureDecisionService` is the only public production class in `forecasting_failure_decision.py`. There is no public function, Protocol, ABC, dataclass, or generic. Constructor injects exactly `FailurePolicyPort` and does not construct `InitialForecastingFailurePolicy`. The only public operation is async `decide(self, context: FailurePolicyContext) -> FailureAction`. Allowed import is `energy_trading.application.orchestration.failure_policy`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `WorkflowState`/`WorkflowStatus`, `InitialForecastingFailurePolicy`, Phase 2 decision/policy classes, attempt/selection/preparation/resolution services, forecast facts/attribution, executor, workflow step, and `fail_after_forecasting`. AST proves one awaited injected-policy `decide(context)` with the exact supplied context forwarded and the result returned unchanged; no `if`/`match`/loops/`try`, no action execution, no context-field inspection, and no state mutation. `graph.py`, `forecasting_workflow.py`, `forecasting_executor.py`, preparation/resolution/initial policy/attempt source, `state.py`, and `create_app()` remain unaware. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_action_boundary.py` — `execute_forecasting_failure_action` is the only public production function in `forecasting_failure_action.py`. There is no production class, Protocol, registry, handler map, or generic action executor. The function is synchronous keyword-only `state: WorkflowState`, `action: FailureAction` returning `WorkflowState`. Allowed imports are application errors, `FailureAction`, `fail_after_forecasting`, and `WorkflowState`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailurePolicyPort`/`FailurePolicyContext`, `ForecastingFailureDecisionService`, `InitialForecastingFailurePolicy`, preparation/resolution/selection/attempt services, `ParallelForecastingExecutionService`, `ForecastingWorkflowStep`, `ParallelIngestionFailureDecisionService`, `execute_parallel_ingestion_failure_action`, `dataclasses.replace`, and direct state-field assignment. AST proves `FAIL` delegates once to `fail_after_forecasting(state)`; `RETRY`/`FALLBACK` raise `InvalidRequestError` with exact stable messages; all current `FailureAction` members are handled explicitly. `graph.py`, `forecasting_workflow.py`, `forecasting_executor.py`, `forecasting_failure_decision.py`, preparation/resolution, `state.py`, and `create_app()` remain unaware. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_handling_boundary.py` — `ForecastingFailureHandlingService` is the only public production class in `forecasting_failure_handling.py`. There is no public function, Protocol, ABC, dataclass, or generic. Constructor injects exactly `ForecastingFailureDecisionService`. The only public operation is keyword-only `async handle(*, state: WorkflowState, context: FailurePolicyContext) -> WorkflowState`. Allowed imports are `FailurePolicyContext`, `ForecastingFailureDecisionService`, `execute_forecasting_failure_action`, and `WorkflowState`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailurePolicyPort`, `InitialForecastingFailurePolicy`, preparation/resolution/selection/attempt services, forecast facts/attribution, `ParallelForecastingExecutionService`, `ForecastingWorkflowStep`, `ParallelIngestionFailureHandlingService`, `ParallelIngestionFailureDecisionService`, `execute_parallel_ingestion_failure_action`, and `fail_after_forecasting`. AST proves one awaited `decide(context)` then one `execute_forecasting_failure_action(state=state, action=action)` with no `if`/`match`/loops/`try`, no action branching, and no context-field inspection. `graph.py`, `forecasting_workflow.py`, `forecasting_executor.py`, preparation/resolution, `state.py`, and `create_app()` remain unaware. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_runtime_handling_boundary.py` — `ForecastingFailureRuntimeHandlingService` is the only public production class in `forecasting_failure_runtime_handling.py`. There is no public function, Protocol, ABC, dataclass, or generic. Constructor injects exactly `ForecastingFailureContextPreparationService` and `ForecastingFailureHandlingService`. The only public operation is keyword-only `async handle(*, state: WorkflowState, failure_group: BaseExceptionGroup) -> WorkflowState`. Allowed imports are the preparation service, the handling service, and `WorkflowState`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailureAction`/`FailurePolicyPort`, `ForecastingFailureDecisionService`, `execute_forecasting_failure_action`, `fail_after_forecasting`, resolution/selection/attempt services, forecast facts/attribution, `ParallelForecastingExecutionService`, `ForecastingWorkflowStep`, and `ParallelIngestionFailureRuntimeHandlingService`. AST proves one awaited `prepare(workflow_id=state.workflow_id, phase=state.phase, error=failure_group)` then one awaited `handle(state=state, context=context)` with no `if`/`match`/loops/`try`, no exception-group traversal, and no state construction. `graph.py`, `forecasting_workflow.py`, `forecasting_executor.py`, `state.py`, and `create_app()` remain unaware. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_context_resolution_boundary.py` — `ForecastingFailureContextResolutionService` is the only public production class in `forecasting_failure_context_resolution.py`. There is no public function, Protocol, ABC, enum, dataclass, registry, factory, tracker, concrete policy, or attempt source. Constructor injects exactly `ForecastingFailureSelectionPort` and `ForecastingAttemptNumberPort`, not `StrictSingleForecastingFailureSelector`. Keyword-only `async resolve(*, workflow_id, phase, facts) -> FailurePolicyContext`. Allowed imports are the selection port, attempt-number port, `ForecastingFailureFact`, `build_forecasting_failure_policy_context`, `FailurePolicyContext`, and `WorkflowPhase`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailureAction`/`FailurePolicyPort`, `WorkflowState`, attribution/extraction/classification, forecast executor/workflow step/agents, `fail_after_forecasting`, Phase 2 resolver/ports/facts, traceback/`Exception` parameters, `try`, branching, loops, ranking, attempt arithmetic, sleep/backoff, and graph/API wiring. AST proves one `.select(facts)`, one awaited `.get_attempt_number(workflow_id)`, and one `build_forecasting_failure_policy_context(...)` with forwarded `phase` / selected `error_code` / attempt number / selected `agent_name`. `graph.py` remains unaware. `create_app()` remains unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_context_preparation_boundary.py` — `ForecastingFailureContextPreparationService` is the only public production class in `forecasting_failure_context_preparation.py`. There is no public function, Protocol, ABC, enum, dataclass, registry, factory, tracker, selector, or classifier. Constructor injects exactly `ForecastingFailureContextResolutionService`. Keyword-only `async prepare(*, workflow_id, phase, error) -> FailurePolicyContext`. Allowed imports are `FailurePolicyContext`, `extract_forecasting_agent_failures`, `classify_forecasting_agent_failures`, `ForecastingFailureContextResolutionService`, and `WorkflowPhase`. Forbidden: LangGraph/LangChain, infrastructure, API, ML, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailureAction`/`FailurePolicyPort`, `WorkflowState`, selection/attempt ports, forecast executor/workflow step/agents, `fail_after_forecasting`, Phase 2 preparation/resolution/facts, traceback/`Exception` parameters, `try`, loops, ranking, attempt arithmetic, sleep/backoff, and graph/API wiring. AST proves one `extract_forecasting_agent_failures(error)`, one `classify_forecasting_agent_failures(...)`, and one awaited `.resolve(...)` with forwarded `workflow_id` / `phase` / classified facts. `graph.py` remains unaware. `create_app()` remains unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_failure_context_boundary.py` — `build_forecasting_failure_policy_context` is the only public surface of `forecasting_failure_context.py`. There is no public class, Protocol, ABC, enum, registry, factory, service, or tracker. The function is keyword-only `phase: WorkflowPhase`, `error_code: str`, `attempt_number: int`, `agent_name: AgentName | None = None` and returns existing `FailurePolicyContext`. The module may import `FailurePolicyContext`, `WorkflowPhase`, and `AgentName`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `FailureAction`/`FailurePolicyPort`, `WorkflowState`, `ForecastingFailureFact`, selection port/strict selector, `ForecastingAttemptNumberPort`, attribution/extraction/classification, forecast executor/workflow step/agents, `fail_after_forecasting`, or the Phase 2 builder. AST proves one `FailurePolicyContext(...)` construction with one-for-one field forwarding and no branching, loops, retry, attempt-number arithmetic, normalization, ranking, selector invocation, attempt lookup, policy invocation, action execution, or graph interaction. `graph.py` and `create_app()` remain unwired. `WorkflowState` remains seven fields.
- `tests/architecture/test_forecasting_strict_single_failure_selector_boundary.py` — `StrictSingleForecastingFailureSelector` is the only public production class in `forecasting_strict_single_failure_selector.py`. The only public operation is synchronous `select(self, facts: tuple[ForecastingFailureFact, ...]) -> ForecastingFailureFact`. The class does not inherit `ForecastingFailureSelectionPort`. Allowed imports are `InvalidRequestError` and `ForecastingFailureFact`. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, Tenacity, `WorkflowState`/`WorkflowPhase`/`WorkflowStatus`, `FailureAction`/`FailurePolicyContext`/`FailurePolicyPort`, attribution/extraction/classification, executor, workflow step, forecast agents, or graph. Production code does not use `sorted`/`min`/`max`/`.sort`, priority mappings, sets for deduplication, ranking dictionaries, or counters. Exact-one extraction is allowed only after a cardinality guard equivalent to `len(facts) == 1`. Zero and multiple facts fail closed. There is no first/last/agent/error/severity winner, no deduplication, and no aggregation. Multi-failure resolution remains unresolved. The selection port remains implementation-neutral. `graph.py` and `create_app()` remain unwired.
- `tests/architecture/test_forecasting_failure_transition_boundary.py` — `fail_after_forecasting(state: WorkflowState) -> WorkflowState` is the only public surface of `forecasting_failure_transition.py`. The module may import stdlib `dataclasses.replace`, application errors, and orchestration state types. It imports none of infrastructure/ML/API, LangGraph/LangChain, FastAPI/Starlette, Redis/SQLAlchemy/Qdrant, HTTP clients, n8n, ML/LLM libraries, forecast agents/model ports, `ForecastingPlan`/`ForecastingSuccess`/`ForecastingExecutionPort`/`ForecastingWorkflowContextPort`/`ForecastingWorkflowStep`/`ParallelForecastingExecutionService`, `FailurePolicyPort`, asyncio, or exception types. There is no Protocol, factory, registry, generic state machine, exception parameter, or diagnostic construction. Valid source is `FORECASTING`/`RUNNING`; destination is `FORECASTING`/`FAILED`. `WorkflowState` still has no `fail()` / `mark_failed()` methods. The module remains LangGraph-free and separate from the success-transition module. `graph.py`, the executor, the workflow step, and `create_app()` remain unwired.
...
- `tests/architecture/test_in_memory_forecasting_context_boundary.py` — infrastructure `InMemoryForecastingWorkflowContext` structurally implements `ForecastingWorkflowContextPort` without inheriting it. The module may import stdlib, application errors, `WorkflowState`, `ForecastingPlan`, and `ForecastingSuccess`. It imports none of LangGraph/LangChain, FastAPI/Starlette, API, ML, Redis, CachePort, SQLAlchemy/psycopg/Alembic, Qdrant, HTTP clients, n8n, forecast agents, forecast model ports, `ForecastingExecutionPort`, `forecasting_transition`, `graph.py`, or `asyncio.gather`/`TaskGroup`. Constructor copies `Mapping[str, ForecastingPlan]`. Public operations are keyword-only `resolve_plan(*, state)` and `record_success(*, state, success)`. Writes use `asyncio.Lock`. `WorkflowState` remains seven fields. `create_app()` and the graph remain unwired; the graph still depends only on already-composed `ForecastingWorkflowStep`.
...
Broader ML/agent import rules remain for later chunks. Chunk 118 locked the first Consumer Load Forecast application↔ML port: `application model port ← future ml adapter`. Chunk 119 added `ConsumerLoadForecastAgent` as `application agent → application model port ← future ml adapter`. The agent contains no numerical forecasting logic. Chunk 120 locked the DAM Price Forecast application↔ML port. Chunk 121 added `DAMPriceForecastAgent` as `application agent → application model port ← future ml adapter`. Chunk 122 added application-owned `ForecastingPlan` as the typed pair of those already-published request DTOs. Chunk 123 added application-owned `ForecastingSuccess` as the typed pair of existing canonical `LoadForecastPoint` and `PriceForecastPoint` tuples. Chunk 124 added application-owned `ForecastingExecutionPort` as the async seam `execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Chunk 125 added application-owned `ForecastingWorkflowContextPort` as the typed external payload seam `resolve_plan(*, state) -> ForecastingPlan` and `record_success(*, state, success) -> None`. Chunk 126 added application-owned `ForecastingWorkflowStep` as the resolve → execute → record composition over those two ports. Chunk 127 added application-owned `advance_after_forecasting` as the separate pure success transition from `forecasting`/`running` to `risk_and_bid`/`running`. Chunk 129 wires LangGraph node `forecasting_success_transition` so a successful Phase-3 graph run applies `advance_after_forecasting` and ends at `risk_and_bid`/`running`. Both forecasting agents are thin application delegates; neither contains numerical ML logic. The DAM agent performs no currency conversion. The plan, success aggregate, execution port, context port, workflow step, and success transition do not execute either agent themselves. Chunk 150 added application-owned `ParallelForecastingExecutionService` as the first concrete `ForecastingExecutionPort` implementation: it injects the two published forecast agents and runs them concurrently with `asyncio.TaskGroup`. Chunk 152 added `ForecastingAgentFailure` attribution of ordinary branch exceptions at that executor boundary before native TaskGroup aggregation. Chunk 153 added application-owned `extract_forecasting_agent_failures` for nested ExceptionGroup attributed-leaf extraction. Chunk 154 added frozen `ForecastingFailureFact` and one-leaf `classify_forecasting_agent_failure`. Chunk 155 added `classify_forecasting_agent_failures` as tuple-level composition that delegates each already-attributed leaf independently to that one-leaf classifier. Chunk 156 added `ForecastingFailureSelectionPort` as the Phase-3-specific selection contract over already-sanitized `ForecastingFailureFact` tuples, with no winner rule. Chunk 157 added `StrictSingleForecastingFailureSelector` as the structural implementation of that contract for the unambiguous one-fact case only. Chunk 158 added `ForecastingAttemptNumberPort` as the Phase-3-specific read-only attempt-number contract over workflow identity. Chunk 162 added `InitialForecastingAttemptNumberSource` as the structural no-retry implementation that always returns `1`. Chunk 163 added `InitialForecastingFailurePolicy` as the structural no-retry implementation that always returns `FailureAction.FAIL`. Chunk 164 added `ForecastingFailureDecisionService` as the Phase-3-specific delegation of an already-prepared `FailurePolicyContext` to `FailurePolicyPort.decide`. Chunk 165 added `execute_forecasting_failure_action` as the Phase-3-specific application of an already-decided `FailureAction`: `FAIL` delegates to `fail_after_forecasting`, while `RETRY` and `FALLBACK` fail closed. Chunk 166 added `ForecastingFailureHandlingService` as the Phase-3-specific composition of an already-prepared `FailurePolicyContext` through that decision service and action executor. Chunk 159 added `build_forecasting_failure_policy_context` as the Phase-3-specific constructor of existing `FailurePolicyContext` from already-sanitized keyword-only fields, with no selection, attempt lookup, or policy decision. The workflow step exists and can be invoked by LangGraph; the concrete executor remains unwired from LangGraph/`create_app()` and does not own context. Chunk 151 added application-owned `fail_after_forecasting` as the separate pure terminal failure transition from `forecasting`/`running` to `forecasting`/`failed`. It does not accept exceptions, mutate diagnostics, or wire LangGraph. Chunk 130 adds infrastructure `InMemoryForecastingWorkflowContext` as an unwired local/dev structural implementation of `ForecastingWorkflowContextPort`. Concrete ML adapters remain outer implementations. Currency is explicit `CurrencyCode` and is not inferred from history.
...
│       │   │   ├── failure_policy.py
│       │   │   ├── forecasting_agent_failure.py
│       │   │   ├── forecasting_attempt_number.py
│       │   │   ├── forecasting_exception_group.py
│       │   │   ├── forecasting_executor.py
│       │   │   ├── forecasting_failure_action.py
│       │   │   ├── forecasting_failure_classification.py
│       │   │   ├── forecasting_failure_context.py
│       │   │   ├── forecasting_failure_context_resolution.py
│       │   │   ├── forecasting_failure_fact.py
│       │   │   ├── forecasting_failure_handling.py
│       │   │   ├── forecasting_failure_selection.py
│       │   │   ├── forecasting_strict_single_failure_selector.py
│       │   │   ├── forecasting_failure_transition.py
│       │   │   ├── forecasting_initial_attempt_number_source.py
│       │   │   ├── forecasting_initial_failure_policy.py
│       │   │   ├── graph.py
...
│       │   │   ├── consumer_load_forecast_model.py
│       │   │   ├── dam_price_forecast_model.py
│       │   │   ├── dlq.py
...
│       ├── ml/
│       │   └── consumer_load/
│       │       ├── previous_day_persistence.py
│       │       ├── previous_day_persistence_backtest.py
│       │       ├── previous_day_persistence_evaluation.py
│       │       └── lag_24h_168h_ols_forecast.py
│       ├── infrastructure/
│       │   ├── adapters/
│       │   │   ├── structured/
│       │   │   │   ├── csv/
...
Chunk 116 adds the application-owned Regulatory workflow-node adapter. `RegulatoryIntelligenceWorkflowNodeAdapter` injects the published context port and Chunk 114 workflow step. `async run(self, state: WorkflowState) -> WorkflowState` awaits `resolve_request(state=state)` once, awaits `workflow_step.run(request)` once with that exact request, awaits `record_result(state=state, result=result)` once with that exact result, and returns the original `WorkflowState` object unchanged. Exceptions propagate by identity with no retry, wrap, or local `try/except`. The adapter does not import LangGraph, does not add request/result slots to `WorkflowState`, and does not choose storage. `graph.py` remains Regulatory-unwired: no new node, edge, route, or factory dependency. Pricing & Sales remains absent.
...
| Concern | Owner |
| --- | --- |
| Load forecast numbers | `ml/consumer_load` behind application ports. Current unwired implementations: `PreviousDayPersistenceConsumerLoadForecastModel` (exact `T - 24h` persistence baseline, not a trained champion) and `Lag24h168hOLSConsumerLoadForecastModel` (two-feature OLS candidate live adapter, not a selected/best/champion model; negative finite live predictions fail closed because `LoadForecastPoint.value_mw` remains `NonNegativeMW`; Chunk 144 offline signed predictions are unchanged). Offline evaluation cases: `build_previous_day_persistence_backtest_cases` consumes canonical `ConsumptionRecord` history and returns ML `PreviousDayPersistenceBacktestCase` values. Offline MAE: `evaluate_previous_day_persistence_mae` consumes those already-built cases and returns `PreviousDayPersistenceMAEResult`; it is not application orchestration and not runtime inference. Offline supervised feature rows: `build_consumer_load_lag_24h_feature_rows` consumes canonical `ConsumptionRecord` history and returns ML `ConsumerLoadLag24hFeatureRow` values (`lag_24h_mw` feature, `target_value_mw` label) without routing through backtest or MAE artifacts. Offline chronological split: `split_consumer_load_feature_rows_chronologically` partitions those already-built rows at an explicit UTC cutoff into `training_rows` (`target_timestamp < cutoff`) and `evaluation_rows` (`target_timestamp >= cutoff`) without training a model. Offline OLS fit: `fit_consumer_load_lag_24h_linear_regression` consumes already-built training rows and returns `slope` plus signed `intercept_mw` without predicting or scoring. Offline OLS evaluation prediction: `predict_consumer_load_lag_24h_linear_regression` applies those already-fitted parameters to already-built evaluation rows and returns `ConsumerLoadLag24hLinearRegressionPrediction` values without calculating metrics. Offline trained OLS MAE: `evaluate_consumer_load_lag_24h_linear_regression_mae` consumes those already-produced prediction artifacts and returns `ConsumerLoadLag24hLinearRegressionMAEResult` (`case_count`, `mae_mw`) without fitting, repredicting, or comparing against persistence. Offline aligned comparison: `compare_consumer_load_persistence_vs_trained_ols_mae` aligns already-produced persistence cases with already-produced trained OLS predictions on the same consumer, timestamps, and actual MW labels, then delegates to the published MAE evaluators and returns `ConsumerLoadPersistenceVsTrainedOLSMAEComparison` without choosing a production model. Offline richer supervised feature rows: `build_consumer_load_lag_24h_168h_feature_rows` consumes canonical `ConsumptionRecord` history and returns ML `ConsumerLoadLag24h168hFeatureRow` values (`lag_24h_mw` and `lag_168h_mw` features, `target_value_mw` label) only when both exact lags exist, without routing through the published one-feature builder, split, training, or scoring. Offline richer chronological split: `split_consumer_load_lag_24h_168h_feature_rows_chronologically` partitions those already-built two-lag rows at an explicit UTC cutoff into `training_rows` (`target_timestamp < cutoff`) and `evaluation_rows` (`target_timestamp >= cutoff`) without rebuilding features or training a model. Offline richer OLS fit: `fit_consumer_load_lag_24h_168h_linear_regression` consumes already-built two-lag training rows and returns signed `lag_24h_coefficient`, `lag_168h_coefficient`, and `intercept_mw` without predicting or scoring. Offline richer OLS evaluation prediction: `predict_consumer_load_lag_24h_168h_linear_regression` applies those already-fitted parameters to already-built evaluation rows and returns `ConsumerLoadLag24h168hLinearRegressionPrediction` values without calculating metrics. Offline richer trained OLS MAE: `evaluate_consumer_load_lag_24h_168h_linear_regression_mae` consumes those already-produced prediction artifacts and returns `ConsumerLoadLag24h168hLinearRegressionMAEResult` (`case_count`, `mae_mw`) without fitting, repredicting, comparing against persistence or one-feature OLS, or selecting a production model. Offline aligned one-feature-versus-two-feature comparison: `compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae` aligns already-produced one-feature predictions with already-produced two-feature predictions on the same consumer, timestamps, and actual MW labels, then delegates to the published MAE evaluators and returns `ConsumerLoadLag24hVsLag24h168hOLSMAEComparison` without choosing a production model. Offline aligned persistence-versus-two-feature comparison: `compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae` aligns already-produced persistence cases with already-produced two-feature OLS predictions on the same consumer, timestamps, and actual MW labels, then delegates to the published persistence and two-feature MAE evaluators and returns `ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison` without choosing a production model. Offline aligned three-way comparison: `compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae` aligns already-produced persistence cases with already-produced one-feature and two-feature OLS predictions on the same consumer, timestamps, and actual MW labels, then delegates to the published persistence, one-feature, and two-feature MAE evaluators and returns `ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison` without choosing a production model and without calling pairwise comparison functions. Candidate live adapter: `Lag24h168hOLSConsumerLoadForecastModel` applies already-fitted two-feature OLS parameters to exact `T - 24h` and `T - 168h` request history and returns canonical `LoadForecastPoint` values; negative finite raw predictions fail closed before DTO construction; it is not a selected production model. Invoked by Consumer Load Forecast Agent only after future wiring. |
...
A LangGraph runtime now exists in the application orchestration layer. Application-owned `WorkflowState` from Chunk 27 remains the authoritative graph schema. Factory `build_workflow_graph(*, regulatory_intelligence_node: RegulatoryIntelligenceWorkflowNodeAdapter, parallel_ingestion_step: ParallelIngestionWorkflowStep, parallel_ingestion_failure_runtime_handler: ParallelIngestionFailureRuntimeHandlingService, forecasting_step: ForecastingWorkflowStep)` constructs and compiles a fresh graph on every call. The Regulatory node adapter, Phase 2 workflow step, outer runtime failure handler, and Phase 3 forecasting step are required through keyword-only dependency injection. Current topology is:
...
Entry routing is fail-closed: `CONTRACT` / `RUNNING` goes to Regulatory; `INGESTION` / `RUNNING` goes to Parallel Ingestion; `FORECASTING` / `RUNNING` goes to Forecasting; every other phase/status combination raises `InvalidRequestError`. `workflow_entry` is an internal async no-op: it does not call an agent, change phase or status, append diagnostics, perform I/O, persist state, or consult `FailurePolicyPort`. `regulatory_intelligence` is a thin async node whose only application behavior is `await regulatory_intelligence_node.run(state)`. That already-composed Chunk 116 adapter remains framework-neutral; LangGraph imports stay isolated to `graph.py`. Regulatory execution returns the original `CONTRACT` / `RUNNING` `WorkflowState` and the graph then ends. There is no Pricing & Sales node and no CONTRACT→INGESTION transition. `parallel_ingestion` is a thin async node whose application behavior is `await parallel_ingestion_step.run(state)` and, on `BaseExceptionGroup` only, `await parallel_ingestion_failure_runtime_handler.handle(state=state, failure_group=failure_group)`. `parallel_ingestion_success_transition` is a thin async node whose only application behavior is to apply `advance_after_parallel_ingestion(state)` and return that `WorkflowState`. `forecasting` is a thin async node whose only application behavior is `await forecasting_step.run(state)`. `forecasting_success_transition` is a thin async node whose only application behavior is to apply `advance_after_forecasting(state)` and return that `WorkflowState`. The graph does not construct plans, record success, call agents, inspect exception-group leaves, or reimplement phase/status assignment. A successful Phase-3 graph run therefore ends at `RISK_AND_BID` / `RUNNING`. A one-attributed-failure group terminates at `ingestion`/`failed`. Direct non-group exceptions still propagate. Compilation is plain (no checkpointer, store, cache, or interrupt). There is no API composition wiring. Direct project dependency is `langgraph>=1.2.11,<1.3`. Direct `langchain` and LLM-provider packages are not project dependencies. Project source must not import `langchain` or `langchain_core`.

Graph nodes must remain thin: they load canonical snapshots, call an agent or use case through application abstractions, write canonical snapshots, and record diagnostics. Nodes depend on application abstractions, not on concrete infrastructure or ML packages. The current Regulatory node depends only on `RegulatoryIntelligenceWorkflowNodeAdapter`. The current Phase 2 node depends only on `ParallelIngestionWorkflowStep` and `ParallelIngestionFailureRuntimeHandlingService`. The Phase-2 success-transition node depends only on `advance_after_parallel_ingestion`. The current Phase 3 node depends only on `ForecastingWorkflowStep`. The Phase-3 success-transition node depends only on `advance_after_forecasting`. The graph itself still does not know the executor, context implementation, five agents, forecast agents, model ports, infrastructure, storage, `fail_after_forecasting`, or lower-level failure internals. Conditional routing exists after `workflow_entry` and after Phase 2.
...
3. LangGraph runtime (`START → workflow_entry`, then contract/running → Regulatory → `END`, ingestion/running → parallel ingestion then success or terminal-failure routing, or forecasting/running → `forecasting` → `forecasting_success_transition` → `END`)
...
The function requires `WorkflowPhase.INGESTION` and `WorkflowStatus.RUNNING`. It returns a new frozen snapshot via `dataclasses.replace`, preserving `workflow_id`, `portfolio_id`, `delivery_date`, `correlation_id`, and `diagnostics`. Any other phase or status fails closed as `InvalidRequestError`. LangGraph node `parallel_ingestion_success_transition` applies this function after a successful workflow step. The successful Phase 2 graph path ends at `forecasting`/`running` and does not continue into the Chunk 128 `forecasting` node.
...
LangGraph ≠ agent interface. The skeleton does not invoke agents. Eight concrete agents exist and remain unwired from API composition. The five Phase 2 agents remain unwired from direct LangGraph calls. Regulatory Intelligence is also unwired from the graph. Consumer Load Forecast and DAM Price Forecast remain unwired from the graph. When later chunks add agent nodes, LangGraph must call them through `AgentPort`; it does not define agent contracts.
...
Chunk 119 introduced the seventh concrete application agent. Current path:

```text
ConsumerLoadForecastModelRequest
  → ConsumerLoadForecastAgent
  → ConsumerLoadForecastModelPort.forecast(...)
  → tuple[LoadForecastPoint, ...]
```

Current outer ML composition is:
...
The agent contains no numerical forecasting logic. It does not reconstruct `LoadForecastPoint` values, convert MW, fill missing points, retry, or call an LLM. Exact model-port exceptions propagate unchanged. `PreviousDayPersistenceConsumerLoadForecastModel` is an unwired deterministic exact `T - 24h` MW persistence baseline, not a trained champion model. `Lag24h168hOLSConsumerLoadForecastModel` is an unwired two-feature OLS candidate live adapter: it consumes already-fitted `ConsumerLoadLag24h168hLinearRegressionFit` and exact `T - 24h` / `T - 168h` history, does not fit during inference, fails closed on missing/duplicate lags and on non-finite or negative finite live predictions, admits exact `0.0`, does not clamp, and is not a selected/best/champion model. `build_previous_day_persistence_backtest_cases` is an unwired ML-owned offline evaluation transformation: it consumes canonical `ConsumptionRecord` history and returns chronological `PreviousDayPersistenceBacktestCase` values, not workflow state and not `LoadForecastPoint`. Missing exact historical pairs are skipped; mixed-consumer or duplicate-timestamp history fails closed. `evaluate_previous_day_persistence_mae` is an unwired ML-owned offline MAE evaluator: `PreviousDayPersistenceBacktestCase` → MAE evaluator → `PreviousDayPersistenceMAEResult`. Empty case tuples fail closed. MAE remains MW. RMSE/MSE/MAPE are not calculated. `build_consumer_load_lag_24h_feature_rows` is an unwired ML-owned offline supervised feature transformation:

```text
ConsumptionRecord history → ConsumerLoadLag24hFeatureRow
ConsumerLoadLag24hFeatureRow → explicit UTC cutoff → training_rows + evaluation_rows
training ConsumerLoadLag24hFeatureRow → OLS fit → slope + intercept_mw
fitted slope + intercept_mw + evaluation ConsumerLoadLag24hFeatureRow → OLS prediction
OLS prediction artifacts → MAE evaluation → case_count + mae_mw
aligned persistence cases + trained OLS predictions → MAE comparison → case_count + persistence_mae_mw + trained_ols_mae_mw
ConsumptionRecord history → ConsumerLoadLag24h168hFeatureRow
ConsumerLoadLag24h168hFeatureRow → explicit UTC cutoff → training_rows + evaluation_rows
training ConsumerLoadLag24h168hFeatureRow → two-feature OLS fit → lag_24h_coefficient + lag_168h_coefficient + intercept_mw
fitted two-feature parameters + evaluation ConsumerLoadLag24h168hFeatureRow → two-feature OLS prediction
two-feature OLS prediction artifacts → MAE evaluation → case_count + mae_mw
aligned one-feature OLS predictions + two-feature OLS predictions → MAE comparison → case_count + lag_24h_mae_mw + lag_24h_168h_mae_mw
aligned persistence cases + two-feature OLS predictions → MAE comparison → case_count + persistence_mae_mw + lag_24h_168h_mae_mw
aligned persistence cases + one-feature OLS predictions + two-feature OLS predictions → MAE comparison → case_count + persistence_mae_mw + lag_24h_mae_mw + lag_24h_168h_mae_mw
```

A row exists only for an exact pair: `lag_24h_mw` from `T - 24 hours` and `target_value_mw` from `T`. Missing exact historical pairs are skipped; mixed-consumer or duplicate-timestamp history fails closed. The feature builder consumes canonical `ConsumptionRecord` history directly and does not import backtest cases, MAE evaluation, or the Chunk 132 inference adapter. The chronological splitter consumes already-built `ConsumerLoadLag24hFeatureRow` values only: training is `target_timestamp < cutoff`; evaluation is `target_timestamp >= cutoff`; both partitions must be non-empty; malformed chronology and mixed consumers fail closed. It does not rebuild history, sort, shuffle, or split by ratio. The OLS fitter consumes already-built training `ConsumerLoadLag24hFeatureRow` values only: `lag_24h_mw` is the single feature, `target_value_mw` is the target, and the result is `slope` plus signed `intercept_mw`. Fewer than two rows and zero feature variance fail closed. A negative intercept is mathematically valid and is not clamped. The fitter does not import the split type, does not predict, and does not calculate metrics. The OLS predictor consumes already-fitted parameters plus already-built evaluation `ConsumerLoadLag24hFeatureRow` values only: `predicted_value_mw = slope * lag_24h_mw + intercept_mw`, evaluation identity and actual target are preserved, negative finite predictions are valid and are not clamped, and empty/mixed/malformed/non-finite inputs fail closed. The predictor does not rebuild history, fit, split, or calculate metrics. The OLS MAE evaluator consumes already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values only: MAE is the arithmetic mean of `|predicted_value_mw - actual_value_mw|`, `case_count` is the prediction tuple length, negative finite predictions are scored with ordinary absolute error, and empty/mixed/malformed/non-finite inputs fail closed. The evaluator does not fit, predict, rebuild features, split, inspect `ConsumptionRecord`, or compare against persistence. It does not reuse Chunk 134 persistence MAE types and does not introduce a generic metrics framework. The persistence-versus-trained OLS comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values and already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values: fair comparison requires the same consumer, the same target timestamps, the same actual MW labels, and equal cohort size; after alignment it delegates to `evaluate_previous_day_persistence_mae` and `evaluate_consumer_load_lag_24h_linear_regression_mae` and returns `case_count`, `persistence_mae_mw`, and `trained_ols_mae_mw`. It does not fit, predict, rebuild features, split, reimplement MAE, or choose a production model. The 24h+168h feature builder consumes canonical `ConsumptionRecord` history directly into `ConsumerLoadLag24h168hFeatureRow`: a row exists only when exact observations exist at `T`, `T - 24 hours`, and `T - 168 hours`; missing either lag omits that target; mixed-consumer or duplicate-timestamp history fails closed; canonical MW is copied unchanged. It does not call the published Chunk 135 one-feature builder, does not impute missing lags, and does not split, train, predict, score, or compare. The 24h+168h chronological splitter consumes already-built `ConsumerLoadLag24h168hFeatureRow` values only: training is `target_timestamp < cutoff`; evaluation is `target_timestamp >= cutoff`; both partitions must be non-empty; malformed chronology and mixed consumers fail closed. It does not rebuild features, inspect raw history, sort, shuffle, split by ratio, or modify the published Chunk 136 one-feature splitter. The 24h+168h OLS fitter consumes already-built training `ConsumerLoadLag24h168hFeatureRow` values only: `lag_24h_mw` and `lag_168h_mw` are the two features, `target_value_mw` is the target, and the result is signed `lag_24h_coefficient`, `lag_168h_coefficient`, and `intercept_mw`. Fewer than three rows and a non-positive centered two-feature determinant fail closed. Negative coefficients and a negative intercept are mathematically valid and are preserved. The fitter does not import the Chunk 142 split type, does not call the Chunk 137 one-feature fitter, does not predict, and does not calculate metrics. The 24h+168h OLS predictor consumes already-fitted two-feature parameters plus already-built evaluation `ConsumerLoadLag24h168hFeatureRow` values only: `predicted_value_mw = intercept_mw + lag_24h_coefficient * lag_24h_mw + lag_168h_coefficient * lag_168h_mw`, evaluation identity and actual target are preserved, negative finite predictions are valid and are not clamped, and empty/mixed/malformed/non-finite inputs fail closed. The predictor does not rebuild history, fit, split, calculate metrics, or call the published Chunk 138 one-feature predictor. The 24h+168h OLS MAE evaluator consumes already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values only: MAE is the arithmetic mean of `|predicted_value_mw - actual_value_mw|`, `case_count` is the prediction tuple length, negative finite predictions are scored with ordinary absolute error, and empty/mixed/malformed/non-finite inputs fail closed. The evaluator does not fit, predict, rebuild features, split, inspect `ConsumptionRecord`, compare against persistence or one-feature OLS, or reuse Chunk 139 one-feature MAE types. It does not introduce a generic metrics framework. The one-feature-versus-two-feature OLS comparison consumes already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values and already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values: fair comparison requires the same consumer, the same target timestamps, the same actual MW labels, and equal cohort size; after alignment it delegates to `evaluate_consumer_load_lag_24h_linear_regression_mae` and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae` and returns `case_count`, `lag_24h_mae_mw`, and `lag_24h_168h_mae_mw`. It does not fit, predict, rebuild features, split, reimplement MAE, or choose a production model. The persistence-versus-two-feature OLS comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values and already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values: fair comparison requires the same consumer, the same target timestamps, the same actual MW labels, and equal cohort size; after alignment it delegates to `evaluate_previous_day_persistence_mae` and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae` and returns `case_count`, `persistence_mae_mw`, and `lag_24h_168h_mae_mw`. It does not fit, predict, rebuild features, split, reimplement MAE, participate in one-feature OLS, form a three-way comparison, or choose a production model. The three-way persistence-versus-one-feature-versus-two-feature OLS comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values, already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values, and already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values: fair comparison requires the same consumer, the same target timestamps, the same actual MW labels, and equal cohort size across all three cohorts; after alignment it delegates to `evaluate_previous_day_persistence_mae`, `evaluate_consumer_load_lag_24h_linear_regression_mae`, and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae` and returns `case_count`, `persistence_mae_mw`, `lag_24h_mae_mw`, and `lag_24h_168h_mae_mw`. It does not fit, predict, rebuild features, split, reimplement MAE, call pairwise comparison functions, or choose a production model. There is no production LightGBM/XGBoost/sklearn adapter and no wider feature pipeline. The baseline, the backtest builder, the persistence MAE evaluator, the 24h feature-row builder, the chronological splitter, the OLS fitter, the OLS predictor, the trained OLS MAE evaluator, the aligned persistence-versus-trained comparison, the 24h+168h feature-row builder, the 24h+168h chronological splitter, the 24h+168h OLS fitter, the 24h+168h OLS predictor, the 24h+168h OLS MAE evaluator, the aligned one-feature-versus-two-feature comparison, the aligned persistence-versus-two-feature comparison, the aligned three-way persistence-versus-one-feature-versus-two-feature comparison, and the two-feature OLS candidate live adapter are not wired into the agent, API composition, FastAPI, LangGraph, or `ForecastingExecutionPort`. Infrastructure `InMemoryForecastingWorkflowContext` exists as an unwired local/dev structural implementation of `ForecastingWorkflowContextPort`. LangGraph injects already-composed `ForecastingWorkflowStep` on `FORECASTING`/`RUNNING` and then applies `advance_after_forecasting`.

Chunk 121 introduced the eighth concrete application agent. Current path:
...
The DAM agent contains no numerical forecasting logic. It does not reconstruct `PriceForecastPoint` or `EnergyPrice` values, convert currencies, fill missing timestamps, retry, or call an LLM. Exact model-port exceptions propagate unchanged. There is no production LightGBM/XGBoost/sklearn adapter and no feature pipeline. Infrastructure `InMemoryForecastingWorkflowContext` exists as an unwired local/dev structural implementation of `ForecastingWorkflowContextPort`. LangGraph injects already-composed `ForecastingWorkflowStep` on `FORECASTING`/`RUNNING` and then applies `advance_after_forecasting`.

Chunk 122 added the Phase-3 application plan contract only. Chunk 123 added the Phase-3 application success aggregate. Chunk 124 added the Phase-3 application execution port. Chunk 125 added the Phase-3 application workflow-context port. Chunk 126 added the Phase-3 application workflow step. Chunk 127 added the Phase-3 application success transition. Conceptual flow:
...
The workflow step now exists as `async run(self, state: WorkflowState) -> WorkflowState`. It injects the published context and execution ports, preserves typed object identity, and returns the original snapshot unchanged. It does not mutate phase or status. LangGraph injects that already-composed step on `FORECASTING`/`RUNNING` (`workflow_entry` → `forecasting` → `forecasting_success_transition` → `END`) and does not own concurrency. `advance_after_forecasting(state: WorkflowState) -> WorkflowState` is a separate pure function. Valid source is `FORECASTING`/`RUNNING`. Destination is `RISK_AND_BID`/`RUNNING`. It returns a new immutable snapshot and does not embed forecast payloads. LangGraph node `forecasting_success_transition` applies it after the forecasting step. The context port now exists as keyword-only `async resolve_plan(*, state: WorkflowState) -> ForecastingPlan` and keyword-only `async record_success(*, state: WorkflowState, success: ForecastingSuccess) -> None`. It externalizes Phase-3 payload ownership. Infrastructure `InMemoryForecastingWorkflowContext` is an unwired local/dev structural implementation and is not production persistence. The execution port now exists as keyword-only `async execute(*, plan: ForecastingPlan) -> ForecastingSuccess`. Application-owned `ParallelForecastingExecutionService` structurally implements that port by running Consumer Load and DAM agents concurrently with `asyncio.TaskGroup`. Ordinary branch failures become `ForecastingAgentFailure` leaves with canonical `AgentName` and chained causes; native TaskGroup exception/cancellation semantics still apply. Sibling cancellation is not attributed as forecast-agent failure. Application-owned `extract_forecasting_agent_failures` can extract those attributed leaves from a possibly nested exception group; the graph does not call it. Application-owned `classify_forecasting_agent_failure` can turn one attributed leaf into a sanitized `ForecastingFailureFact`; the graph does not call it. Application-owned `classify_forecasting_agent_failures` can classify an already-attributed failure tuple by delegating each element to that one-leaf classifier; the graph does not call it. Application-owned `ForecastingFailureSelectionPort` can resolve a sanitized fact tuple to one fact; the graph does not call it. Application-owned `StrictSingleForecastingFailureSelector` implements that contract for the unambiguous one-fact case only; zero or multiple facts fail closed, simultaneous multi-failure selection remains unresolved, and the graph does not call it. Application-owned `ForecastingAttemptNumberPort` can return the current 1-based forecasting attempt for a workflow identity; there is no concrete source, no increment/reset semantics on the port, and the graph does not call it. Application-owned `build_forecasting_failure_policy_context` can construct existing `FailurePolicyContext` from already-sanitized keyword-only fields; it does not select failures, look up attempts, invoke policy, or execute an action, and the graph does not call it. Graph wiring of that executor does not. `ForecastingSuccess` fields are exactly `consumer_load_forecast: tuple[LoadForecastPoint, ...]` and `dam_price_forecast: tuple[PriceForecastPoint, ...]`. Canonical forecast points are reused; empty tuples remain valid successful outputs. There is no duplicate result schema, no cross-result timestamp/horizon invariant, and no Consumer → DAM domain dependency. Everything after these seams remains absent: no production/LangGraph wiring of the executor, no production/durable context adapter, no LangGraph wiring of `fail_after_forecasting`, no LangGraph wiring of `StrictSingleForecastingFailureSelector`, no forecasting attempt tracking/policy/runtime handling, no selected ML adapter, no feature pipeline, and no runtime/API composition. Both aggregates retain supplied objects unchanged. `WorkflowState` remains the seven-field snapshot and does not carry the forecast payload. Phase 3 is not generally complete: the graph can invoke the abstract workflow step and apply the success transition, and local/dev can use the in-memory context adapter, but the concurrent executor remains unwired from LangGraph/`create_app()` and there is still no production context implementation.

Chunk 157 adds a LangGraph-free Phase-3-specific strict single-failure selector after the published selection contract. Application owns concrete `StrictSingleForecastingFailureSelector` that structurally satisfies `ForecastingFailureSelectionPort` without inheriting it:

```text
forecast-agent runtime failure
→ attribution
→ ExceptionGroup
...
→ attempt tracking still missing
→ FailurePolicyContext still missing
→ policy/action still missing
→ LangGraph routing still missing
```

Exactly one sanitized fact is returned unchanged. Zero facts and two-or-more facts raise the same sanitized `InvalidRequestError`. There is no first/last winner, no AgentName/error-code priority, no ranking, no sorting, no deduplication, and no aggregation. Duplicate identities remain multiple facts. Simultaneous multi-failure resolution remains deliberately unresolved. The selector is not wired into LangGraph. Attempt tracking, policy context, policy/action execution, `fail_after_forecasting` invocation, and retry/fallback remain absent.

Chunk 158 adds a LangGraph-free Phase-3-specific attempt-number source contract after the strict single-failure selector. Application owns non-generic `ForecastingAttemptNumberPort` with a single async operation `get_attempt_number(workflow_id: str) -> int`:
...
→ FailurePolicyContext construction
→ selection + attempt-number context resolution
→ concrete attempt tracking/source implementation still missing
→ exception-to-context preparation still missing
→ policy/action still missing
→ LangGraph routing still missing
```

The Protocol accepts only workflow identity and returns the current 1-based Phase 3 forecasting attempt. There is no concrete implementation, no increment/reset/set/record/begin/complete write API, no persistence, no retry ownership, and no empty or invalid-number production default. Attempt-number reading remains separate from classification, selection, context construction, failure policy, action execution, and LangGraph. The graph does not import or call this port. `WorkflowState` remains the seven-field snapshot.

Chunk 159 adds a LangGraph-free Phase-3-specific failure-policy context constructor after the attempt-number source contract. Application owns synchronous `build_forecasting_failure_policy_context(*, phase, error_code, attempt_number, agent_name=None) -> FailurePolicyContext`:
...
Explicit keyword-only inputs map one-for-one onto existing `FailurePolicyContext`. The caller supplies the sanitized error code, the attempt number, and the optional canonical agent identity. The builder forwards those values unchanged, relies on existing `FailurePolicyContext` validation, and does not inspect exceptions, `WorkflowState`, or failure facts. It does not select among failures, look up attempts, invoke policy, execute an action, or wire LangGraph. The graph does not import or call this function. `WorkflowState` remains the seven-field snapshot.

Chunk 160 adds a LangGraph-free Phase-3-specific failure-policy context resolution service after that constructor. Application owns concrete `ForecastingFailureContextResolutionService`:
...
Constructor injects exactly `ForecastingFailureSelectionPort` and `ForecastingAttemptNumberPort`. Keyword-only `async resolve(*, workflow_id, phase, facts) -> FailurePolicyContext` selects once with the incoming facts tuple, awaits attempt lookup once with the incoming workflow identity, then delegates construction to existing `build_forecasting_failure_policy_context`. Incoming `phase`, selected `error_code`, returned attempt number, and selected `agent_name` are forwarded unchanged. The service does not implement a selector, does not track attempts, does not duplicate published validation, does not inspect exceptions, does not invoke policy, does not execute an action, and does not wire LangGraph. The Phase 2 resolver remains separate. Exception-to-context preparation is owned by `ForecastingFailureContextPreparationService`. The graph does not import or call this service. `WorkflowState` remains the seven-field snapshot.

Chunk 161 adds a LangGraph-free Phase-3-specific failure-policy context preparation service after that resolver. Application owns concrete `ForecastingFailureContextPreparationService`:
...
→ FailurePolicyContext construction
→ selection + attempt-number context resolution
→ exception-to-context preparation
→ concrete attempt tracking/source implementation still missing
→ policy/action still missing
→ LangGraph routing still missing
```

Constructor injects exactly `ForecastingFailureContextResolutionService`. Keyword-only `async prepare(*, workflow_id, phase, error) -> FailurePolicyContext` extracts attributed leaves once, classifies that tuple once, then awaits the injected resolver once. Incoming `error`, extraction result, classified facts, `workflow_id`, and `phase` are forwarded unchanged. The service does not duplicate published extraction, classification, selection, or attempt behavior. It does not inspect exceptions, invoke policy, execute an action, mutate state, or wire LangGraph. The Phase 2 preparation service remains separate. Runtime integration remains later work. The graph does not import or call this service. `WorkflowState` remains the seven-field snapshot.

Chunk 162 adds a LangGraph-free Phase-3-specific initial attempt-number source after the published attempt-number contract. Application owns concrete `InitialForecastingAttemptNumberSource` that structurally satisfies `ForecastingAttemptNumberPort` without inheriting it:
...
→ selection + attempt-number context resolution
→ exception-to-context preparation
→ retry-capable attempt tracking still missing
→ policy/action still missing
→ LangGraph routing still missing
```

The source is async and stateless. It returns the literal integer `1` for every workflow identity because the published runtime has no retry execution. The workflow ID remains opaque. There is no increment, reset, persistence, per-workflow map, or graph wiring. It is not a tracker and is not suitable as a future retry tracker. The Phase 2 source remains separate. The existing resolver continues to depend on `ForecastingAttemptNumberPort`, not this concrete class. Future retry support requires separately reviewed tracking semantics.

Chunk 163 adds a LangGraph-free Phase-3-specific initial failure policy after the published failure-policy contract. Application owns concrete `InitialForecastingFailurePolicy` that structurally satisfies `FailurePolicyPort` without inheriting it:
...
→ selection + attempt-number context resolution
→ exception-to-context preparation
→ initial failure policy (`FAIL`)
→ retry-capable attempt tracking still missing
→ RETRY / FALLBACK execution still missing
→ prepared failure-handling composition still missing
→ runtime failure handling still missing
→ LangGraph routing still missing
```

The policy is async and stateless. It returns exactly `FailureAction.FAIL` for every valid context because the published runtime has no retry or fallback execution. It does not inspect phase, error code, attempt number, or agent. It does not execute the action, mutate state, or wire LangGraph. The Phase 2 policy remains separate. Future RETRY/FALLBACK behavior requires separately reviewed runtime mechanics.

Chunk 164 adds a LangGraph-free Phase-3-specific failure-policy decision service after the published failure-policy contract and the initial policy. Application owns concrete `ForecastingFailureDecisionService` that injects exactly `FailurePolicyPort` and does not construct `InitialForecastingFailurePolicy`:
...
→ exception-to-context preparation
→ initial failure policy (`FAIL`)
→ failure-policy decision service
→ retry-capable attempt tracking still missing
→ action execution still missing
→ prepared failure-handling composition still missing
→ runtime failure handling still missing
→ LangGraph routing still missing
```

The service is async. `decide(context)` forwards the exact supplied context to the injected port, awaits that decision exactly once, and returns the exact `FailureAction` unchanged. It does not inspect context fields, branch on the action, execute FAIL/RETRY/FALLBACK, mutate `WorkflowState`, instantiate a concrete policy, or wrap policy exceptions. The Phase 2 decision service remains separate.

Chunk 165 adds LangGraph-free Phase-3-specific terminal FAIL action execution after the published failure-policy decision. Application owns `execute_forecasting_failure_action(*, state, action)`:
...
→ initial failure policy (`FAIL`)
→ failure-policy decision service
→ terminal FAIL action execution (`fail_after_forecasting`)
→ retry-capable attempt tracking still missing
→ RETRY / FALLBACK execution still missing
→ prepared failure-handling composition still missing
→ runtime failure handling still missing
...
The function is synchronous. It receives an already-decided `FailureAction`. `FAIL` returns `fail_after_forecasting(state)` unchanged. `RETRY` and `FALLBACK` fail closed as `InvalidRequestError` with stable sanitized messages. The function does not consult policy, construct context, inspect exceptions, mutate diagnostics, or couple to LangGraph. The Phase 2 action executor remains separate. Terminal transition semantics remain owned by `fail_after_forecasting`.

Chunk 166 adds LangGraph-free Phase-3-specific prepared failure-handling composition after the published decision service and action executor. Application owns concrete `ForecastingFailureHandlingService` that injects exactly `ForecastingFailureDecisionService`:
...
→ failure-policy decision service
→ terminal FAIL action execution (`fail_after_forecasting`)
→ prepared failure-handling composition (`handle`)
→ retry-capable attempt tracking still missing
→ RETRY / FALLBACK execution still missing
→ outer runtime failure handling still missing
→ LangGraph routing still missing
```

The service is async. `handle(*, state, context)` awaits the injected decision service exactly once with the supplied context, forwards the returned `FailureAction` and supplied `WorkflowState` to `execute_forecasting_failure_action`, and returns that executor result unchanged. It does not construct context, inspect exceptions or context fields, branch on the action, call `fail_after_forecasting` directly, instantiate a concrete policy, or mutate diagnostics. Decision-service and action-executor exceptions propagate unchanged. The Phase 2 handling service remains separate.

Chunk 167 adds LangGraph-free Phase-3-specific runtime failure-handling composition after the published preparation and prepared-handling seams. Application owns concrete `ForecastingFailureRuntimeHandlingService` that injects exactly `ForecastingFailureContextPreparationService` and `ForecastingFailureHandlingService`:
...
→ terminal FAIL action execution (`fail_after_forecasting`)
→ prepared failure-handling composition (`handle`)
→ outer runtime failure handling (`WorkflowState` + `BaseExceptionGroup`)
→ retry-capable attempt tracking still missing
→ RETRY / FALLBACK execution still missing
→ LangGraph forecasting-failure capture/routing still missing
```

The service is async. `handle(*, state, failure_group)` awaits preparation exactly once with `state.workflow_id`, `state.phase`, and the supplied group as `error`, forwards the original state plus that context to existing handling, and returns that handling result unchanged. It does not extract, classify, select, resolve attempts, construct context, invoke policy, execute actions, branch on `FailureAction`, mutate diagnostics, or catch/wrap exceptions. The currently supported single-attributed-failure terminal path is executable entirely inside application code. The Phase 2 runtime handler remains separate. LangGraph does not inject or catch through this service.

Eight concrete agents exist. LangGraph does not import or call them. It injects `ParallelIngestionWorkflowStep` into node `parallel_ingestion` and awaits `run(state)`. That step can invoke the five Phase 2 agents only when a caller has composed `ParallelIngestionExecutionPort` (typically `ConcurrentParallelIngestionExecutor`) and a context port behind it. Application-owned `ParallelIngestionPlan` can hold the five existing typed Phase 2 requests together. Application-owned `ConcurrentParallelIngestionExecutor` structurally implements `ParallelIngestionExecutionPort`: it injects those five application agents, runs `agent.run(...)` concurrently with `asyncio.TaskGroup`, and returns `ParallelIngestionSuccess` only when all five succeed. Ordinary agent failures become `ParallelIngestionAgentFailure` leaves with canonical `AgentName` and chained causes; native TaskGroup exception/cancellation semantics still apply. Sibling cancellation is not attributed as agent failure. Application-owned `ParallelIngestionWorkflowContextPort` can resolve a prepared plan and record all-five-success output by workflow identity without expanding `WorkflowState`. Application-owned `ParallelIngestionWorkflowStep` composes that resolve → execute → record sequence and returns the original `WorkflowState` unchanged. Application-owned `advance_after_parallel_ingestion` can replace a valid `ingestion`/`running` snapshot with `forecasting`/`running`. LangGraph node `parallel_ingestion_success_transition` applies that function after a successful workflow step. Application-owned `fail_parallel_ingestion` can replace a valid `ingestion`/`running` snapshot with `ingestion`/`failed` without mutating diagnostics; the graph does not call it. Application-owned `ParallelIngestionFailureDecisionService` can decide a `FailureAction` from an already-constructed `FailurePolicyContext` without executing it; the graph does not call it. Application-owned `build_parallel_ingestion_failure_policy_context` can construct that published context from already-sanitized typed facts; the graph does not call it. Application-owned `execute_parallel_ingestion_failure_action` can apply terminal `FailureAction.FAIL` by delegating to `fail_parallel_ingestion` and rejects `RETRY` / `FALLBACK` as not implemented; the graph does not call it. Application-owned `ParallelIngestionFailureHandlingService` can compose an already-built `FailurePolicyContext` through the published decision service and action executor; the graph does not call it. Application-owned `extract_parallel_ingestion_agent_failures` can extract already-attributed `ParallelIngestionAgentFailure` leaves from a possibly nested exception group; the graph does not call it. Application-owned `classify_parallel_ingestion_agent_failure` can convert one attributed leaf into sanitized `ParallelIngestionFailureFact`; the graph does not call it. Application-owned `classify_parallel_ingestion_agent_failures` can classify an already-extracted attributed-failure tuple by delegating each element to that one-leaf classifier; it preserves order, cardinality, and duplicate agent identities, does not select a primary failure, and the graph does not call it. Application-owned `ParallelIngestionFailureSelectionPort` is the Phase-2-specific selection contract over that sanitized fact tuple; it has no first/last/priority winner semantics, and the graph does not call it. Application-owned `StrictSingleParallelIngestionFailureSelector` implements that contract for the unambiguous one-fact case only; zero or multiple facts fail closed, simultaneous multi-failure selection remains unresolved, and the graph does not call it. Application-owned `ParallelIngestionAttemptNumberPort` is the Phase-2-specific read-only attempt-number source contract over workflow identity; it has no increment/reset semantics on the port, and the graph does not call it. Application-owned `InitialParallelIngestionAttemptNumberSource` implements that contract for the current no-retry runtime by returning exactly `1`; it is not a tracker, and the graph does not call it. Application-owned `ParallelIngestionFailureContextResolutionService` composes that selection port, attempt-number port, and the published context builder into one `FailurePolicyContext`; it implements no selection algorithm, no attempt tracking, no policy decision, and no action execution, the strict selector and initial attempt source exist but are not auto-wired, and the graph does not call it. Application-owned `ParallelIngestionFailureContextPreparationService` composes a `BaseExceptionGroup` through extraction, tuple classification, and that resolution service into one `FailurePolicyContext`; it does not duplicate exception interpretation, does not invoke policy, does not execute an action, and the graph does not call it. Phase 3 is not generally complete: LangGraph can invoke `ForecastingWorkflowStep` and apply `advance_after_forecasting`, and application-owned `ParallelForecastingExecutionService` can concurrently execute both forecast agents and attribute ordinary branch failures as `ForecastingAgentFailure`, and application-owned `extract_forecasting_agent_failures` can extract those attributed leaves from a nested exception group, and application-owned `classify_forecasting_agent_failure` can classify one attributed leaf into `ForecastingFailureFact`, and application-owned `classify_forecasting_agent_failures` can classify an attributed-failure tuple by delegating independently to that one-leaf classifier, and application-owned `ForecastingFailureSelectionPort` can resolve a sanitized fact tuple to one fact, and application-owned `StrictSingleForecastingFailureSelector` implements that contract for the unambiguous one-fact case only, and application-owned `ForecastingAttemptNumberPort` can return the current 1-based forecasting attempt for a workflow identity, and application-owned `build_forecasting_failure_policy_context` can construct existing `FailurePolicyContext` from already-sanitized keyword-only fields, and application-owned `ForecastingFailureContextResolutionService` can compose selection, attempt lookup, and that builder into one `FailurePolicyContext`, and application-owned `ForecastingFailureContextPreparationService` can compose extraction, tuple classification, and that resolution service into one `FailurePolicyContext`, but that executor, extractor, classifiers, selection port, selector, attempt-number port, context builder, resolution service, preparation service, initial policy, decision service, action executor, prepared handling service, and runtime handling service remain unwired from LangGraph/`create_app()`, there is still no production/durable context implementation, no retry-capable forecasting attempt tracking, and Phase 3 still has no LangGraph forecasting-failure capture/routing even though `InitialForecastingFailurePolicy` exists as an unwired always-`FAIL` policy, `ForecastingFailureDecisionService` can obtain a policy decision from a prepared context, `execute_forecasting_failure_action` can apply terminal `FAIL` in application code, `ForecastingFailureHandlingService` can compose an already-prepared context through decision and terminal action execution, and `ForecastingFailureRuntimeHandlingService` can compose a `WorkflowState` plus `BaseExceptionGroup` through preparation then existing handling. Partial/failure/degraded fan-in semantics are not defined. Infrastructure `InMemoryParallelIngestionWorkflowContext` exists as unwired local/dev reference storage. Infrastructure `InMemoryForecastingWorkflowContext` exists as unwired local/dev reference storage for Phase 3 plan/success payloads. Production/durable workflow-context implementation does not exist. No weather, hydro, generation, news, or market provider exists. No weather, hydro, generation, news, or market persistence exists. No retry/fallback execution exists. LangGraph Phase 2 join (failure/degraded/runtime composition) remains future work.
...
Chunk 130 adds the matching Phase-3 local/dev adapter. Infrastructure `InMemoryForecastingWorkflowContext` copies constructor-prepared `ForecastingPlan` values keyed by `workflow_id`, fails closed on missing identity as `ResourceNotFoundError`, records `ForecastingSuccess` idempotently for equal retries, and rejects conflicting content as `ConflictError`. Coroutine-level writes use `asyncio.Lock`. The adapter does not execute forecasts, choose Consumer → DAM ordering, or mutate `WorkflowState`. It is not durable, not Redis/PostgreSQL, and not wired into LangGraph or `create_app()`.
