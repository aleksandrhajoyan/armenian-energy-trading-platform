...
The current Phase 3/4 orchestration foundation is five application-owned seams:
...
No current node calls the failure policy. No retries or fallbacks execute. Neither the plan, the execution port, nor the success aggregate is consumed by the graph. The execution port has no concrete implementation.
...
Five concrete agents exist. LangGraph still invokes none. Application-owned `ParallelIngestionPlan` can hold the five existing typed Phase 2 requests together. Application-owned `ParallelIngestionExecutionPort` can accept that plan and return `ParallelIngestionSuccess`. Application-owned `ParallelIngestionSuccess` can hold the five existing typed Phase 2 results for the all-success case. None of these executes agents. There is no concrete executor and no concurrency semantics. Partial/failure/degraded fan-in semantics are not defined. No weather, hydro, generation, news, or market provider exists. No weather, hydro, generation, news, or market persistence exists. No retry/fallback execution exists. Actual parallel execution and join remain future work.
...
There is no concrete executor. The port does not encode sequential vs parallel execution, `asyncio.gather` / `TaskGroup`, retries, cancellation, timeout, fallback, degraded mode, or partial success. It is not `WorkflowState` and is not wired into LangGraph. Actual parallel execution and join remain future work.
