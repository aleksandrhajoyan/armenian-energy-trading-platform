...
- **Available foundation (not a concrete orchestrator):** `AgentPort`; `WorkflowState`; minimal LangGraph skeleton; failure-policy hook (`FailurePolicyPort.decide` → `RETRY` | `FALLBACK` | `FAIL`).
- **Still absent:** concrete Chief Orchestrator; phase routing; parallel join; retry execution; fallback execution; concrete policy implementation. Conceptual failure behavior above is specification, not implemented runtime.
