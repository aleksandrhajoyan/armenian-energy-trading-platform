...
- **Available foundation (not a concrete orchestrator):** `AgentPort`; `WorkflowState`; minimal LangGraph skeleton; failure-policy hook (`FailurePolicyPort.decide` → `RETRY` | `FALLBACK` | `FAIL`); typed parallel-ingestion fan-out plan (`ParallelIngestionPlan` with the five existing Phase 2 request DTOs); typed all-five-success fan-in aggregate (`ParallelIngestionSuccess` with the five existing Phase 2 result DTOs).
- **Still absent:** concrete Chief Orchestrator; phase routing; parallel execution; parallel join; partial/degraded fan-in policy; retry execution; fallback execution; concrete policy implementation. Conceptual failure behavior above is specification, not implemented runtime. The graph still invokes zero agents.
