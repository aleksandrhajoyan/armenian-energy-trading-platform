...
- **Still absent:** concrete Chief Orchestrator; five-phase routing; parallel graph join for failure/degraded/runtime composition; production/durable workflow-context implementation; Phase 3 routing/execution; general status lifecycle transitions; partial/degraded fan-in policy; retry execution; fallback execution; concrete policy implementation; LangGraph wiring of `fail_parallel_ingestion`; exception-to-diagnostic mapping. Conceptual failure behavior above is specification, not implemented runtime. The graph invokes the Phase 2 workflow step and then the published success transition; it does not call `AgentPort` or the five agents directly, does not execute Phase 3, and does not execute the terminal failure transition. Runtime Phase 2 exceptions still propagate. The in-memory context adapter is not composed by a Chief Orchestrator and is not wired into the graph or API.
