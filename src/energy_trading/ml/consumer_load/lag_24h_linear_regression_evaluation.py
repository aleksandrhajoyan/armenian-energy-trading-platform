...
This outer ML transformation scores already-produced Chunk 138
``ConsumerLoadLag24hLinearRegressionPrediction`` values. It calculates
MAE only.
...
LangGraph, and ``ForecastingExecutionPort``. It does not depend on
previous-day persistence MAE evaluation.
"""
...
class ConsumerLoadLag24hLinearRegressionMAEResult:
...
def evaluate_consumer_load_lag_24h_linear_regression_mae(
    *,
    predictions: tuple[ConsumerLoadLag24hLinearRegressionPrediction, ...],
) -> ConsumerLoadLag24hLinearRegressionMAEResult:
    """Return MAE in MW over the supplied OLS evaluation predictions."""
    ...
    if not isfinite(mae_mw):
        raise InvalidRequestError(_NON_FINITE_MAE_MESSAGE)
    return ConsumerLoadLag24hLinearRegressionMAEResult(
