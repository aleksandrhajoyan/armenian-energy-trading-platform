...
previous day, rolling statistics, and calendar/time-of-day features are
not used. Duplicate timestamps or mixed consumer identity fail closed
before any row is returned. Canonical MW values are copied unchanged.
...
class ConsumerLoadLag24hFeatureRow:
    """One exact ``T`` / ``T - 24h`` Consumer Load supervised pair."""
...
def build_consumer_load_lag_24h_feature_rows(
    *,
    history: tuple[ConsumptionRecord, ...],
) -> tuple[ConsumerLoadLag24hFeatureRow, ...]:
