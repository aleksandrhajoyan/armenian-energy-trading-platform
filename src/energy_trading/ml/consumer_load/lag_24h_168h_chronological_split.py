...
This outer ML transformation partitions already-built Chunk 141
``ConsumerLoadLag24h168hFeatureRow`` values at an explicit UTC cutoff.
It does not train a model and does not rebuild history.
...
* Already-built two-lag feature rows are the only input. This module
  does not rebuild features and does not belong to application
  orchestration.
...
input, and empty partitions fail closed. Ratio-based partitions are not
used.

This contract is independent of the published one-feature chronological
split. The cutoff is caller-supplied and is not stored on the result.
...
class ConsumerLoadLag24h168hChronologicalFeatureSplit:
...
def split_consumer_load_lag_24h_168h_feature_rows_chronologically(
    *,
    rows: tuple[ConsumerLoadLag24h168hFeatureRow, ...],
    cutoff: UtcDateTime,
    ...
    return ConsumerLoadLag24h168hChronologicalFeatureSplit(
