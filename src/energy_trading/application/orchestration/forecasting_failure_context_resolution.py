"""Application-owned Phase 3 failure-policy context resolution.
...
* Application: owns ``ForecastingFailureContextResolutionService``.
* Injected ports: ``ForecastingFailureSelectionPort`` and
  ``ForecastingAttemptNumberPort``.
* Existing ``build_forecasting_failure_policy_context``: owns
...
class ForecastingFailureContextResolutionService:
    """Phase-3-specific composition of selection, attempt lookup, and context.
    ...
    async def resolve(
        ...
        *,
        ...
        delegated to ``build_forecasting_failure_policy_context``.
        """
        ...
        return build_forecasting_failure_policy_context(
            phase=phase,
