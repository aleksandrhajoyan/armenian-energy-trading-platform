...
and delegates Phase 2 ingestion to an injected
``ParallelIngestionWorkflowStep``. After Phase 2 it either applies
the published successful control-state transition or delegates a
caught ``BaseExceptionGroup`` to the injected
...
fallback, or Phase 3 execution in this module.
"""
...
_INVALID_ENTRY_ROUTE_MESSAGE = (
    "Workflow entry routing requires contract running or ingestion running status."
)
...
def _route_after_workflow_entry(state: WorkflowState) -> str:
    """Return the contract-phase or Phase 2 destination.

    Valid only for ``contract`` / ``running`` and ``ingestion`` / ``running``.
...
def build_workflow_graph(
    *,
    ...
    adapter, Phase 2 workflow step, and outer runtime failure handler are
    required through keyword-only dependency injection. Compilation is
    plain: no checkpointer, store, cache, interrupt, or durability
    configuration.
    """
