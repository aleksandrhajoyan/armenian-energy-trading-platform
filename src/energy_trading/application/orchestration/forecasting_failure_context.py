"""Application-owned Phase 3 failure-policy context construction.
...
* Application: owns ``build_forecasting_failure_policy_context``.
* Existing ``FailurePolicyContext``: reused unchanged.
* Selection, attempt lookup, policy decision, action execution,
...
def build_forecasting_failure_policy_context(
    *,
    phase: WorkflowPhase,
    error_code: str,
    attempt_number: int,
