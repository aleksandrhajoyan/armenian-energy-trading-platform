"""Application-owned Phase 3 initial terminal-fail failure policy.
...
* Application: owns ``InitialForecastingFailurePolicy``.
...
class InitialForecastingFailurePolicy:
    """Stateless Phase 3 failure policy for the current capability set.
    ...
    async def decide(self, context: FailurePolicyContext) -> FailureAction:
        """Return the only currently supported Phase 3 failure action."""
