"""Application-owned Phase 3 sanitized one-leaf failure classification.

This module converts one already-attributed ``ForecastingAgentFailure``
...
* Application: owns ``ForecastingFailureFact`` and
...
class ForecastingFailureFact:
    """Immutable sanitized fact for one already-attributed Phase 3 failure.
...
def classify_forecasting_agent_failure(
    failure: ForecastingAgentFailure,
) -> ForecastingFailureFact:
    """Return a sanitized Phase 3 failure fact for one attributed leaf.
