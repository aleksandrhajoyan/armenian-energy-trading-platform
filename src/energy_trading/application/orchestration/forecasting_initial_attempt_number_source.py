"""Application-owned Phase 3 initial attempt-number source.

This module implements the published attempt-number contract for the
current no-retry runtime: the only supported Phase 3 execution is
attempt ``1``.

Ownership:

* Application: owns ``InitialForecastingAttemptNumberSource``.
* Existing ``ForecastingAttemptNumberPort``: satisfied structurally.
...
class InitialForecastingAttemptNumberSource:
    """Stateless Phase 3 attempt source for the current no-retry runtime.
    ...
    async def get_attempt_number(self, workflow_id: str) -> int:
        """Return the currently supported Phase 3 forecasting attempt number."""
