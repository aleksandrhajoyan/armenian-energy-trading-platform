"""Application-owned Phase 3 agent-failure attribution.

This module preserves canonical ``AgentName`` identity when one concurrent
Phase 3 forecast execution fails. The original Python exception remains the
causal failure through standard exception chaining.

Ownership:

* Application: owns ``ForecastingAgentFailure``.
...
_MESSAGE_TEMPLATE = "Forecasting agent failed: {agent_name}."
...
class ForecastingAgentFailure(Exception):
    """Sanitized attribution wrapper for one failed Phase 3 forecast task.
