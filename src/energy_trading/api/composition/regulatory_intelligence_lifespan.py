...
settings-loaded Regulatory runtime. It does not install that lifespan into
``create_app()``, expose the composed service, or invoke provider operations.
...
enters Chunk 75. The composed service is intentionally unused.
"""
...
def build_regulatory_intelligence_lifespan(
    *,
    ...
    async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
        async with loaded_regulatory_intelligence_runtime(env_file=env_file):
            yield
