...
def _evaluate(
    ...
    return evaluate_consumer_load_lag_24h_168h_linear_regression_mae(predictions=predictions)
...
def test_result_is_frozen_slotted_with_exactly_two_fields() -> None:
    ...
    assert ConsumerLoadLag24h168hLinearRegressionMAEResult.__slots__ == _RESULT_FIELDS
...
def test_accumulated_absolute_error_overflow_fails_closed() -> None:
    first_predicted = 1e308
    second_predicted = 1e308
    assert isfinite(first_predicted)
    assert isfinite(second_predicted)
    assert isfinite(abs(first_predicted - 0.0))
    assert isfinite(abs(second_predicted - 0.0))
    assert not isfinite(abs(first_predicted - 0.0) + abs(second_predicted - 0.0))
    predictions = (
        _prediction(hour=10, predicted_value_mw=first_predicted, actual_value_mw=0.0),
        _prediction(hour=11, predicted_value_mw=second_predicted, actual_value_mw=0.0),
    )
    with pytest.raises(InvalidRequestError, match=_NON_FINITE_MAE_MESSAGE) as caught:
        _evaluate(predictions)
    assert caught.value.code == "invalid_request"
...
def test_return_contract_uses_int_count_and_finite_float_mae() -> None:
    ...
    assert isinstance(result, ConsumerLoadLag24h168hLinearRegressionMAEResult)
