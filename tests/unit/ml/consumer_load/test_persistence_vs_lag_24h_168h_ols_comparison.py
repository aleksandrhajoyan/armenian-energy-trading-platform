...
from energy_trading.ml.consumer_load.persistence_vs_lag_24h_168h_ols_comparison import (
...
def _compare(
    ...
    return compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae(
...
def test_result_is_frozen_slotted_with_exactly_three_fields() -> None:
    ...
    assert ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison.__slots__ == _RESULT_FIELDS
...
def test_empty_persistence_and_nonempty_two_feature_fail_closed() -> None:
...
def test_nonempty_persistence_and_empty_two_feature_fail_closed() -> None:
...
def test_input_tuples_are_not_mutated() -> None:
    ...
    first_prediction = _lag_24h_168h_prediction(
        hour=10,
        predicted_value_mw=3.0,
        actual_value_mw=2.0,
    )
    second_prediction = _lag_24h_168h_prediction(
        hour=12,
        predicted_value_mw=4.0,
        actual_value_mw=3.0,
    )
    persistence_cases = (first_case, second_case)
    lag_24h_168h_predictions = (first_prediction, second_prediction)
    original_case_ids = tuple(id(item) for item in persistence_cases)
    original_prediction_ids = tuple(id(item) for item in lag_24h_168h_predictions)
    _compare(persistence_cases, lag_24h_168h_predictions)
    assert persistence_cases == (first_case, second_case)
    assert lag_24h_168h_predictions == (first_prediction, second_prediction)
    assert tuple(id(item) for item in persistence_cases) == original_case_ids
    assert tuple(id(item) for item in lag_24h_168h_predictions) == original_prediction_ids
...
def test_repeated_equal_input_returns_value_equal_result() -> None:
    ...
    first = _compare(persistence_cases, lag_24h_168h_predictions)
    second = _compare(persistence_cases, lag_24h_168h_predictions)
    assert first == second
...
def test_return_contract_uses_int_count_and_finite_float_mae() -> None:
    ...
    result = _compare(persistence_cases, lag_24h_168h_predictions)
    assert isinstance(result, ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison)
