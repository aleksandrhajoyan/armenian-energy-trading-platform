"""Phase 3 failure-policy context builder constructs the published contract."""
...
def test_builder_constructs_published_context_with_exact_field_values() -> None:
    ...
    context = build_forecasting_failure_policy_context(
        phase=phase,
...
def test_builder_does_not_fabricate_an_agent_identity() -> None:
    ...
    assert context.agent_name is None
    assert context.agent_name is not AgentName.CONSUMER_LOAD_FORECAST
    assert context.agent_name is not AgentName.DAM_PRICE_FORECAST
...
def test_builder_preserves_caller_supplied_forecasting_agent_identities(
...
def test_builder_is_deterministic_for_equivalent_typed_inputs() -> None:
    first = build_forecasting_failure_policy_context(
        phase=WorkflowPhase.FORECASTING,
        error_code="schema_invalid",
        attempt_number=3,
        agent_name=AgentName.DAM_PRICE_FORECAST,
    )
    second = build_forecasting_failure_policy_context(
        phase=WorkflowPhase.FORECASTING,
        error_code="schema_invalid",
        attempt_number=3,
        agent_name=AgentName.DAM_PRICE_FORECAST,
...
def test_builder_preserves_published_phase_type_rejection() -> None:
    with pytest.raises(TypeError, match="phase must be a WorkflowPhase"):
        build_forecasting_failure_policy_context(
            phase="forecasting",  # type: ignore[arg-type]
            error_code="dependency_unavailable",
            attempt_number=1,
        )
...
def test_builder_signature_is_keyword_only_without_speculative_defaults() -> None:
    signature = inspect.signature(build_forecasting_failure_policy_context)
    ...
    assert not inspect.iscoroutinefunction(build_forecasting_failure_policy_context)


def test_builder_parameter_annotations_match_published_context_fields() -> None:
    builder = inspect.signature(build_forecasting_failure_policy_context)
