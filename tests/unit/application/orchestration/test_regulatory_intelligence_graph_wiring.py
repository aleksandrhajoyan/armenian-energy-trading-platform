...
def _compile(
    *,
    ...
    return compiled, injected_regulatory, injected_step, injected_handler


def test_factory_requires_three_keyword_only_dependencies() -> None:
...
def test_topology_includes_terminal_regulatory_slice() -> None:
    compiled, _regulatory, _step, _handler = _compile()
...
async def test_contract_running_invokes_regulatory_and_terminates() -> None:
    ...
    compiled, regulatory, step, handler = _compile()
    node_order: list[str] = []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkflowPhase.FORECASTING, WorkflowStatus.RUNNING),
    ],
)
...
async def test_regulatory_failure_propagates_without_phase2_or_retry() -> None:
    ...
    compiled, injected, step, handler = _compile(regulatory=regulatory)
    original = _state()
