"""Phase 3 forecasting attempt-number source is a structural Protocol only."""
...
class _StructuralForecastingAttemptNumberFake:
...
def _as_attempt_number_port(
    fake: _StructuralForecastingAttemptNumberFake,
) -> ForecastingAttemptNumberPort:
...
def test_structural_fake_does_not_inherit_production_base() -> None:
    assert ForecastingAttemptNumberPort not in (_StructuralForecastingAttemptNumberFake.__mro__)
    assert not any(
        base.__name__ in {"ForecastingAttemptNumberPort", "Protocol"}
        for base in _StructuralForecastingAttemptNumberFake.__bases__
    )


def test_structural_fake_can_be_assigned_to_attempt_number_port() -> None:
    port = _as_attempt_number_port(_StructuralForecastingAttemptNumberFake(1))
    assert isinstance(port, _StructuralForecastingAttemptNumberFake)


def test_get_attempt_number_signature_is_async_workflow_id_to_int() -> None:
    parameters = inspect.signature(ForecastingAttemptNumberPort.get_attempt_number).parameters
    assert tuple(parameters) == ("self", "workflow_id")
    hints = get_type_hints(ForecastingAttemptNumberPort.get_attempt_number)
    assert hints["workflow_id"] is str
    assert hints["return"] is int
    assert inspect.iscoroutinefunction(ForecastingAttemptNumberPort.get_attempt_number)
    public_operations = [
        name
        for name, value in vars(ForecastingAttemptNumberPort).items()
...
async def test_get_attempt_number_receives_exact_workflow_id() -> None:
    ...
    fake = _StructuralForecastingAttemptNumberFake(1)
...
async def test_fake_may_return_later_positive_attempt_without_transformation() -> None:
    ...
    fake = _StructuralForecastingAttemptNumberFake(3)
    port = _as_attempt_number_port(fake)
    result = await port.get_attempt_number(workflow_id)
    assert fake.received is workflow_id
    assert result == 3
...
def test_get_attempt_number_does_not_require_state_failure_or_policy_types() -> None:
    ...
    assert "failure_fact" not in parameters
