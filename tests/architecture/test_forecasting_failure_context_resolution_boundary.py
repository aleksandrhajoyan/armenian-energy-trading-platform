"""Forecasting failure-policy context resolution stays application-owned."""
...
ALLOWED_INIT_ANNOTATIONS = {
    "selection_port": "ForecastingFailureSelectionPort",
    "attempt_number_port": "ForecastingAttemptNumberPort",
}
...
def _resolve_method(class_def: ast.ClassDef) -> ast.AsyncFunctionDef:
    for node in class_def.body:
        if isinstance(node, ast.AsyncFunctionDef) and node.name == "resolve":
            return node
    msg = "async method 'resolve' not found on ForecastingFailureContextResolutionService"
    raise AssertionError(msg)
...
def test_resolution_service_does_not_import_outer_layers_or_vendors() -> None:
    ...
    assert "ForecastingFailureSelectionPort" in names
    assert "ForecastingAttemptNumberPort" in names
    assert "build_forecasting_failure_policy_context" in names
...
def test_resolution_module_exposes_exactly_one_production_class() -> None:
    ...
    assert _module_class_names(RESOLUTION_MODULE) == ["ForecastingFailureContextResolutionService"]
    class_def = _class_def(RESOLUTION_MODULE, "ForecastingFailureContextResolutionService")
    ...
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        ...
        for node in parsed.body:
            if (
                ...
                and node.name == "ForecastingFailureContextResolutionService"
            ):
                production_classes.append(path.relative_to(SRC_ROOT).as_posix())
    assert production_classes == [
        "energy_trading/application/orchestration/forecasting_failure_context_resolution.py"
    ]
    ...
    tree = ast.parse(RESOLUTION_MODULE.read_text(encoding="utf-8"), filename=str(RESOLUTION_MODULE))
    protocols = [
        node.name
        for node in tree.body
...
def test_resolve_delegates_once_in_order_without_selection_or_tracking_logic() -> None:
    ...
    for node in ast.walk(resolve_fn):
        ...
        if name == "build_forecasting_failure_policy_context":
            builder_calls += 1
            assert node.args == []
            keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in node.keywords}
            assert keywords["phase"] == "phase"
            assert keywords["error_code"].endswith(".error_code")
            assert keywords["agent_name"].endswith(".agent_name")
            assert set(keywords) == {
                "phase",
                "error_code",
                "attempt_number",
    ...
    assert _call_name(third.value) == "build_forecasting_failure_policy_context"
    indexed = [
        ast.unparse(node)
        for node in ast.walk(resolve_fn)
...
def test_graph_policy_and_lower_layers_remain_unwired_to_the_resolution_service() -> None:
    for path in UNWIRED_MODULES:
        ...
        assert "forecasting_failure_context_resolution" not in source


def test_workflow_state_shape_is_unchanged_by_context_resolution() -> None:
...
def test_api_composition_does_not_import_or_construct_the_resolution_service() -> None:
    ...
    assert "forecastingfailurecontextresolutionservice" not in app_source
    assert "forecasting_failure_context_resolution" not in app_source
