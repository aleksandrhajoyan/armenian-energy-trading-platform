"""Parallel-ingestion success transition stays application-owned and LangGraph-free."""
...
TRANSITION_MODULE = ORCHESTRATION_ROOT / "parallel_ingestion_transition.py"
...
def test_transition_module_exposes_exactly_one_public_function() -> None:
    ...
    assert [node.name for node in public_functions] == ["advance_after_parallel_ingestion"]
    ...
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        ...
        for node in parsed.body:
            if (
                ...
                and node.name == "advance_after_parallel_ingestion"
            ):
                production_functions.append(path.relative_to(SRC_ROOT).as_posix())
    assert production_functions == [
        "energy_trading/application/orchestration/parallel_ingestion_transition.py"
    ]
...
def test_transition_uses_immutable_replacement_and_is_not_a_framework() -> None:
    tree = ast.parse(TRANSITION_MODULE.read_text(encoding="utf-8"), filename=str(TRANSITION_MODULE))
    function = _public_function_defs(TRANSITION_MODULE)[0]
    replace_calls = 0
...
def test_workflow_step_and_graph_remain_unwired_to_the_transition() -> None:
    for path in (
        ...
        assert (
            "energy_trading.application.orchestration.parallel_ingestion_transition" not in modules
        )
...
def test_api_composition_does_not_import_or_construct_the_transition() -> None:
    ...
    assert "advance_after_parallel_ingestion" not in app_source
    assert "parallel_ingestion_transition" not in app_source
