"""Generation Availability agent and source port remain application-owned and provider-neutral."""
...
AGENT_MODULE = PRODUCTION_ROOT / "application" / "agents" / "generation_availability.py"
PORT_MODULE = PRODUCTION_ROOT / "application" / "ports" / "generation_availability_records.py"
...
def test_generation_modules_do_not_import_outer_layers_or_vendors() -> None:
...
def test_generation_agent_does_not_couple_to_hydro() -> None:
    for path in (AGENT_MODULE, PORT_MODULE):
        ...
        for hydro_name in HYDRO_COUPLING_NAMES:
            assert hydro_name not in names
        source = path.read_text(encoding="utf-8")
        for hydro_name in HYDRO_COUPLING_NAMES:
            assert hydro_name not in source


def test_generation_source_port_is_nongeneric_protocol_with_async_fetch() -> None:
    class_def = _class_def(PORT_MODULE, "GenerationAvailabilityRecordSourcePort")
    ...
    assert ast.unparse(fetch_fn.returns) == "tuple[GenerationAvailabilityRecord, ...]"
    names = annotation_type_names(PORT_MODULE)
    leaked = sorted(name for name in names if name in FORBIDDEN_TYPE_NAMES)
    assert leaked == []
    assert "GenerationAvailabilityRecord" in names
    assert "EntityId" in names
    assert "UtcDateTime" in names


def test_generation_source_port_module_has_no_concrete_implementation() -> None:
    assert _module_class_names(PORT_MODULE) == ["GenerationAvailabilityRecordSourcePort"]


def test_generation_agent_public_surface_is_name_and_run() -> None:
    class_def = _class_def(AGENT_MODULE, "GenerationAvailabilityAgent")
    ...
    assert ast.unparse(run_fn.args.args[1].annotation) == "GenerationAvailabilityRequest"
    assert ast.unparse(run_fn.returns) == "GenerationAvailabilityResult"
    ...
    assert "HydroRecord" not in imported_names(AGENT_MODULE)
    assert "HydroRecordSourcePort" not in imported_names(AGENT_MODULE)
    assert "WeatherAndRenewableForecastAgent" not in imported_names(AGENT_MODULE)


def test_generation_request_and_result_have_exact_fields() -> None:
    assert _annassign_field_names(AGENT_MODULE, "GenerationAvailabilityRequest") == (
        "asset_id",
        "horizon_start",
        "horizon_end",
    )
    assert _annassign_field_names(AGENT_MODULE, "GenerationAvailabilityResult") == ("records",)


def test_graph_module_does_not_import_or_invoke_weather_hydro_or_generation_agents() -> None:
    ...
    assert "GenerationAvailabilityRequest" not in names
    ...
    assert "HydroRecordSourcePort" not in graph_source
    assert "GenerationAvailabilityAgent" not in graph_source
    assert "GenerationAvailabilityRecordSourcePort" not in graph_source


def test_api_composition_does_not_import_or_construct_weather_hydro_or_generation_agents() -> None:
    forbidden_wiring = (
    ...
    assert "hydrorecordsourceport" not in app_source
    assert "generationavailabilityagent" not in app_source
    assert "generationavailabilityrecordsourceport" not in app_source
