"""Consumer Load 24h+168h chronological feature split stays a narrow ML seam."""
...
SPLIT_MODULE = CONSUMER_LOAD_ML_ROOT / "lag_24h_168h_chronological_split.py"
...
def test_split_module_lives_under_ml_consumer_load() -> None:
    assert SPLIT_MODULE.is_relative_to(CONSUMER_LOAD_ML_ROOT)
    assert SPLIT_MODULE.name == "lag_24h_168h_chronological_split.py"
    assert _module_class_names(SPLIT_MODULE) == ["ConsumerLoadLag24h168hChronologicalFeatureSplit"]
    assert _module_function_names(SPLIT_MODULE) == [
...
def test_result_is_frozen_slotted_with_exactly_two_fields() -> None:
    class_def = _class_def(SPLIT_MODULE, "ConsumerLoadLag24h168hChronologicalFeatureSplit")
    ...
    assert (
        _annassign_field_names(SPLIT_MODULE, "ConsumerLoadLag24h168hChronologicalFeatureSplit")
        == ALLOWED_RESULT_FIELDS
    )
...
def test_splitter_is_keyword_only_over_chunk_141_rows_and_utc_cutoff() -> None:
    tree = ast.parse(SPLIT_MODULE.read_text(encoding="utf-8"), filename=str(SPLIT_MODULE))
    splitter = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "split_consumer_load_lag_24h_168h_feature_rows_chronologically"
    )
    ...
    assert ast.unparse(splitter.returns) == "ConsumerLoadLag24h168hChronologicalFeatureSplit"


def test_partition_uses_strict_less_than_and_greater_or_equal_cutoff() -> None:
    source = SPLIT_MODULE.read_text(encoding="utf-8")
    assert "target_timestamp < cutoff" in source
    assert "target_timestamp >= cutoff" in source
    called = _called_names(SPLIT_MODULE)
    ...
    assert "shuffle" not in identifiers


def test_split_depends_only_on_allowed_inward_contracts() -> None:
    leaked = sorted(
        module
...
def test_split_does_not_generate_identity_or_use_io() -> None:
    source = SPLIT_MODULE.read_text(encoding="utf-8")
...
def test_split_does_not_introduce_generic_or_ratio_frameworks() -> None:
    class_names = _module_class_names(SPLIT_MODULE)
    ...
    leaked_tokens = [token for token in UNAUTHORIZED_SPLIT_TOKENS if token in source]
...
def test_application_agents_do_not_import_the_two_lag_split() -> None:
    ...
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        ...
        assert "lag_24h_168h_chronological_split" not in source
...
def test_orchestration_and_langgraph_do_not_import_the_two_lag_split() -> None:
    ...
    assert "split_consumer_load_lag_24h_168h_feature_rows_chronologically" not in graph_names
    assert "ConsumerLoadLag24h168hChronologicalFeatureSplit" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_does_not_reference_the_two_lag_split() -> None:
    ...
    assert "ConsumerLoadLag24h168hChronologicalFeatureSplit" not in app_source
    assert "split_consumer_load_lag_24h_168h_feature_rows_chronologically" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "lag_24h_168h_chronological_split" not in source
    assert "lag_168h_mw" not in source


def test_no_dam_imports_in_two_lag_feature_split() -> None:
    names = imported_names(SPLIT_MODULE)
    source = SPLIT_MODULE.read_text(encoding="utf-8")
    ...
    assert "energy_trading.ml.dam" not in imported_modules(SPLIT_MODULE)
