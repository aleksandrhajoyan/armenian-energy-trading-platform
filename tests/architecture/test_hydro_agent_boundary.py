"""Hydro agent and source port remain application-owned and provider-neutral."""
...
AGENT_MODULE = PRODUCTION_ROOT / "application" / "agents" / "hydro_resources.py"
PORT_MODULE = PRODUCTION_ROOT / "application" / "ports" / "hydro_records.py"
...
def test_hydro_modules_do_not_import_outer_layers_or_vendors() -> None:
...
def test_hydro_source_port_is_nongeneric_protocol_with_async_fetch() -> None:
    class_def = _class_def(PORT_MODULE, "HydroRecordSourcePort")
    ...
    assert ast.unparse(fetch_fn.returns) == "tuple[HydroRecord, ...]"
    names = annotation_type_names(PORT_MODULE)
    leaked = sorted(name for name in names if name in FORBIDDEN_TYPE_NAMES)
    assert leaked == []
    assert "HydroRecord" in names
    assert "EntityId" in names
    assert "UtcDateTime" in names


def test_hydro_source_port_module_has_no_concrete_implementation() -> None:
    assert _module_class_names(PORT_MODULE) == ["HydroRecordSourcePort"]


def test_hydro_agent_public_surface_is_name_and_run() -> None:
    class_def = _class_def(AGENT_MODULE, "HydroResourcesAgent")
    ...
    assert ast.unparse(run_fn.args.args[1].annotation) == "HydroResourcesRequest"
    assert ast.unparse(run_fn.returns) == "HydroResourcesResult"
    ...
    assert "GenerationAvailabilityRecord" not in imported_names(AGENT_MODULE)
    assert "WeatherAndRenewableForecastAgent" not in imported_names(AGENT_MODULE)


def test_hydro_request_and_result_have_exact_fields() -> None:
    assert _annassign_field_names(AGENT_MODULE, "HydroResourcesRequest") == (
        "resource_id",
        "horizon_start",
        "horizon_end",
    )
    assert _annassign_field_names(AGENT_MODULE, "HydroResourcesResult") == ("records",)


def test_graph_module_does_not_import_or_invoke_weather_or_hydro_agents() -> None:
...
def test_api_composition_does_not_import_or_construct_weather_or_hydro_agents() -> None:
    forbidden_wiring = (
