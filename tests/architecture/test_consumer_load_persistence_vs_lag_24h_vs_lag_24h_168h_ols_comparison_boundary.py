"""Consumer Load three-way persistence-versus-OLS MAE comparison stays a narrow ML seam."""
...
COMPARISON_MODULE = CONSUMER_LOAD_ML_ROOT / (
...
UNAUTHORIZED_COMPARISON_TOKENS = (
    "sklearn",
    "numpy",
    "pandas",
    "polars",
...
def test_comparison_module_lives_under_ml_consumer_load() -> None:
    assert COMPARISON_MODULE.is_relative_to(CONSUMER_LOAD_ML_ROOT)
    assert COMPARISON_MODULE.name == "persistence_vs_lag_24h_vs_lag_24h_168h_ols_comparison.py"
    assert _module_class_names(COMPARISON_MODULE) == [
        "ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison"
    ]
    assert _module_function_names(COMPARISON_MODULE) == [
...
def test_comparison_is_keyword_only_over_published_artifacts() -> None:
    tree = ast.parse(COMPARISON_MODULE.read_text(encoding="utf-8"), filename=str(COMPARISON_MODULE))
    comparison = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae"
    )
    assert not isinstance(comparison, ast.AsyncFunctionDef)
    assert tuple(arg.arg for arg in comparison.args.args) == ()
    assert tuple(arg.arg for arg in comparison.args.kwonlyargs) == (
        "persistence_cases",
        "lag_24h_predictions",
        "lag_24h_168h_predictions",
    )
    assert comparison.args.vararg is None
    assert comparison.args.kwarg is None
    persistence_arg = comparison.args.kwonlyargs[0]
    lag_24h_arg = comparison.args.kwonlyargs[1]
    lag_24h_168h_arg = comparison.args.kwonlyargs[2]
    ...
    assert comparison.returns is not None
    assert (
        ast.unparse(persistence_arg.annotation) == "tuple[PreviousDayPersistenceBacktestCase, ...]"
    )
    assert (
        ast.unparse(lag_24h_arg.annotation)
        == "tuple[ConsumerLoadLag24hLinearRegressionPrediction, ...]"
    )
    assert (
        ast.unparse(lag_24h_168h_arg.annotation)
        == "tuple[ConsumerLoadLag24h168hLinearRegressionPrediction, ...]"
    )
    assert (
        ast.unparse(comparison.returns)
        == "ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison"
    )


def test_alignment_checks_consumer_timestamp_and_actual_then_delegates() -> None:
    source = COMPARISON_MODULE.read_text(encoding="utf-8")
    identifiers = _identifier_names(COMPARISON_MODULE)
    called = _called_names(COMPARISON_MODULE)
    assert "consumer_id" in identifiers
    assert "target_timestamp" in identifiers
    assert "actual_value_mw" in identifiers
    assert "evaluate_previous_day_persistence_mae" in called
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" in called
    assert "evaluate_consumer_load_lag_24h_168h_linear_regression_mae" in called
    assert "compare_consumer_load_persistence_vs_trained_ols_mae" not in called
    assert "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae" not in called
    assert "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae" not in called
    ...
    leaked_calls = sorted(name for name in called if name in FORBIDDEN_CALL_NAMES)
    assert leaked_calls == []
...
def test_comparison_depends_only_on_allowed_inward_contracts() -> None:
    ...
    assert "ConsumerLoadLag24hLinearRegressionPrediction" in names
    assert "ConsumerLoadLag24h168hLinearRegressionPrediction" in names
    assert "evaluate_previous_day_persistence_mae" in names
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" in names
    assert "evaluate_consumer_load_lag_24h_168h_linear_regression_mae" in names
    ...
    assert "compare_consumer_load_persistence_vs_trained_ols_mae" not in names
    assert "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae" not in names
    assert "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae" not in names
...
def test_comparison_does_not_generate_identity_or_use_io() -> None:
    source = COMPARISON_MODULE.read_text(encoding="utf-8")
...
def test_comparison_does_not_introduce_generic_or_selection_frameworks() -> None:
    class_names = _module_class_names(COMPARISON_MODULE)
    ...
    assert leaked_ids == []
    source = COMPARISON_MODULE.read_text(encoding="utf-8").lower()
    leaked_tokens = [token for token in UNAUTHORIZED_COMPARISON_TOKENS if token in source]
    assert leaked_tokens == []
    ...
    assert "mape" not in identifiers
...
def test_application_agents_do_not_import_the_comparison() -> None:
    ...
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        ...
        assert "ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison" not in names
        assert "compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae" not in names
        source = path.read_text(encoding="utf-8")
        assert "persistence_vs_lag_24h_vs_lag_24h_168h_ols_comparison" not in source
...
def test_orchestration_and_langgraph_do_not_import_the_comparison() -> None:
    ...
    assert "compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae" not in graph_names
    assert "ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_does_not_reference_the_comparison() -> None:
    ...
    assert "ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison" not in app_source
    assert "compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "persistence_vs_lag_24h_vs_lag_24h_168h_ols_comparison" not in source
    assert "lag_24h_mae_mw" not in source
    assert "lag_24h_168h_mae_mw" not in source


def test_no_dam_imports_in_consumer_load_comparison() -> None:
    names = imported_names(COMPARISON_MODULE)
    source = COMPARISON_MODULE.read_text(encoding="utf-8")
    ...
    assert "energy_trading.ml.dam" not in imported_modules(COMPARISON_MODULE)
