"""Forecasting success stays typed, application-only, and LangGraph-free."""
...
def test_forecasting_success_lives_in_application_orchestration() -> None:
    assert SUCCESS_MODULE.is_relative_to(ORCHESTRATION_ROOT)
    assert SUCCESS_MODULE.name == "forecasting_success.py"
    assert SUCCESS_MODULE.exists()


def test_forecasting_success_does_not_import_outer_layers_or_vendors() -> None:
    leaked = sorted(
        module
        for module in imported_modules(SUCCESS_MODULE)
        if is_forbidden(module, FORBIDDEN_PREFIXES)
    )
    assert leaked == []
    extras = imported_modules(SUCCESS_MODULE) - ALLOWED_MODULE_IMPORTS
    assert extras == set()
    leaked_names = sorted(
        name for name in imported_names(SUCCESS_MODULE) if name in FORBIDDEN_TYPE_NAMES
    )
    assert leaked_names == []


def test_forecasting_success_is_frozen_slotted_dataclass_with_exactly_two_typed_fields() -> None:
    class_def = _class_def(SUCCESS_MODULE, "ForecastingSuccess")
    ...
    fields = _annassign_field_names(SUCCESS_MODULE, "ForecastingSuccess")
    assert fields == ALLOWED_SUCCESS_FIELDS
    leaked = sorted(name for name in fields if name in FORBIDDEN_FIELD_NAMES)
    assert leaked == []
    annotations = _annassign_field_annotations(SUCCESS_MODULE, "ForecastingSuccess")
    assert annotations == ALLOWED_SUCCESS_ANNOTATIONS
    leaked_types = sorted(
        name
        for annotation in annotations.values()
        for name in {annotation}
        if name in FORBIDDEN_TYPE_NAMES
    )
    assert leaked_types == []
    duplicate_result_types = {
        "ConsumerLoadForecastResult",
        "DAMPriceForecastResult",
        "ForecastingResult",
        "ForecastingOutcome",
    ...
    assert duplicate_result_types.isdisjoint(annotations.values())
    defined = [
        node.name
...
def test_forecasting_success_module_exposes_only_the_success_contract() -> None:
    class_names = _module_class_names(SUCCESS_MODULE)
    assert class_names == ["ForecastingSuccess"]
    tree = ast.parse(SUCCESS_MODULE.read_text(encoding="utf-8"), filename=str(SUCCESS_MODULE))
    protocols = [
        node.name
    ...
    identifiers = _identifier_names(SUCCESS_MODULE)
...
def test_forecasting_success_public_contract_excludes_payload_runtime_and_execution() -> None:
    names = annotation_type_names(SUCCESS_MODULE)
    ...
    assert "ForecastingPlan" not in identifiers
    ...
    source = SUCCESS_MODULE.read_text(encoding="utf-8")
...
def test_workflow_state_shape_is_unchanged_by_the_success_aggregate() -> None:
    fields = _annassign_field_names(STATE_MODULE, "WorkflowState")
    assert fields == WORKFLOW_STATE_FIELDS
    leaked = sorted(name for name in fields if name in ALLOWED_SUCCESS_FIELDS)
    assert leaked == []
...
def test_graph_remains_unwired_to_the_forecasting_success() -> None:
...
def test_api_composition_does_not_import_or_construct_forecasting_success() -> None:
    forbidden_wiring = ("energy_trading.application.orchestration.forecasting_success",)
...
def test_forecasting_success_stays_isolated_from_ml_implementations() -> None:
    modules = imported_modules(SUCCESS_MODULE)
