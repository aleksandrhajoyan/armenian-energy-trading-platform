"""Consumer Load persistence-versus-two-feature OLS MAE comparison stays a narrow ML seam."""
...
COMPARISON_MODULE = CONSUMER_LOAD_ML_ROOT / "persistence_vs_lag_24h_168h_ols_comparison.py"
...
def test_comparison_module_lives_under_ml_consumer_load() -> None:
    ...
    assert COMPARISON_MODULE.name == "persistence_vs_lag_24h_168h_ols_comparison.py"
...
def test_comparison_is_keyword_only_over_published_artifacts() -> None:
    ...
    comparison = next(
        ...
        and node.name == "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae"
    )
    ...
    lag_24h_168h_arg = comparison.args.kwonlyargs[1]
    ...
    assert ast.unparse(comparison.returns) == "ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison"
...
def test_alignment_checks_consumer_timestamp_and_actual_then_delegates() -> None:
    ...
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" not in called
...
def test_comparison_depends_only_on_allowed_inward_contracts() -> None:
    ...
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" not in names
...
def test_application_agents_do_not_import_the_comparison() -> None:
    ...
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        ...
        assert "ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison" not in names
        assert "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae" not in names
        source = path.read_text(encoding="utf-8")
        assert "persistence_vs_lag_24h_168h_ols_comparison" not in source
...
def test_orchestration_and_langgraph_do_not_import_the_comparison() -> None:
    ...
    assert "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae" not in graph_names
    assert "ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison" not in graph_names
...
def test_api_does_not_reference_the_comparison() -> None:
    ...
    assert "ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison" not in app_source
    assert "compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae" not in app_source
