"""Consumer Load one-feature versus two-feature OLS MAE comparison stays a narrow ML seam."""
...
COMPARISON_MODULE = CONSUMER_LOAD_ML_ROOT / "lag_24h_vs_lag_24h_168h_ols_comparison.py"
...
def test_result_is_frozen_slotted_with_exactly_three_fields() -> None:
    class_def = _class_def(COMPARISON_MODULE, "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison")
    ...
    assert (
        _annassign_field_names(COMPARISON_MODULE, "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison")
        == ALLOWED_RESULT_FIELDS
    )
...
def test_comparison_is_keyword_only_over_published_artifacts() -> None:
    ...
    comparison = next(
        ...
        and node.name == "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae"
    )
    ...
    lag_24h_arg = comparison.args.kwonlyargs[0]
    ...
    assert ast.unparse(comparison.returns) == "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison"
...
def test_application_agents_do_not_import_the_comparison() -> None:
    ...
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        ...
        assert "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison" not in names
        assert "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae" not in names
        source = path.read_text(encoding="utf-8")
        assert "lag_24h_vs_lag_24h_168h_ols_comparison" not in source
...
def test_orchestration_and_langgraph_do_not_import_the_comparison() -> None:
    ...
    assert "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae" not in graph_names
    assert "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison" not in graph_names
...
def test_api_does_not_reference_the_comparison() -> None:
    ...
    assert "ConsumerLoadLag24hVsLag24h168hOLSMAEComparison" not in app_source
    assert "compare_consumer_load_lag_24h_vs_lag_24h_168h_ols_mae" not in app_source
