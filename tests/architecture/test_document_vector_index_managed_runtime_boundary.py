"""Chunk 90 managed document vector index runtime stays an outer lifecycle layer."""
...
BUILDER_MODULE = COMPOSITION_ROOT / "document_vector_index_managed_runtime.py"
...
def test_builder_exposes_exactly_one_public_async_context_manager() -> None:
    ...
    assert [node.name for node in public] == ["managed_document_vector_index_runtime"]
    ...
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        ...
        for node in parsed.body:
            if (
                ...
                and node.name == "managed_document_vector_index_runtime"
            ):
...
def test_builder_registers_cleanup_then_delegates_to_chunk_89() -> None:
    builder = _managed_function(BUILDER_MODULE)
    control = [
        type(node).__name__
...
def test_http_create_app_does_not_invoke_the_managed_runtime() -> None:
    ...
    assert "managed_document_vector_index_runtime" not in app_source
    assert "managed_document_vector_index_runtime" not in CONFIGURED_RUNTIME_MODULE.read_text(
        encoding="utf-8"
    )
    assert "managed_document_vector_index_runtime" not in PROVIDER_RUNTIME_MODULE.read_text(
        encoding="utf-8"
    )
    assert "managed_document_vector_index_runtime" not in NEUTRAL_BUILDER_MODULE.read_text(
        encoding="utf-8"
    )
