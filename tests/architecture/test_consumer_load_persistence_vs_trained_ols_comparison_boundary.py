"""Consumer Load persistence-versus-trained OLS MAE comparison stays a narrow ML seam."""
...
COMPARISON_MODULE = CONSUMER_LOAD_ML_ROOT / "persistence_vs_trained_ols_comparison.py"
...
def test_result_is_frozen_slotted_with_exactly_three_fields() -> None:
    class_def = _class_def(COMPARISON_MODULE, "ConsumerLoadPersistenceVsTrainedOLSMAEComparison")
    ...
    assert (
        _annassign_field_names(
            COMPARISON_MODULE, "ConsumerLoadPersistenceVsTrainedOLSMAEComparison"
        )
        == ALLOWED_RESULT_FIELDS
    )
...
def test_comparison_is_keyword_only_over_published_artifacts() -> None:
    ...
    comparison = next(
        ...
        and node.name == "compare_consumer_load_persistence_vs_trained_ols_mae"
    )
...
def test_application_agents_do_not_import_the_comparison() -> None:
    ...
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        ...
        assert "ConsumerLoadPersistenceVsTrainedOLSMAEComparison" not in names
...
def test_orchestration_and_langgraph_do_not_import_the_comparison() -> None:
    ...
    assert "compare_consumer_load_persistence_vs_trained_ols_mae" not in graph_names
    assert "ConsumerLoadPersistenceVsTrainedOLSMAEComparison" not in graph_names
...
def test_api_does_not_reference_the_comparison() -> None:
    ...
    assert "ConsumerLoadPersistenceVsTrainedOLSMAEComparison" not in app_source
    assert "compare_consumer_load_persistence_vs_trained_ols_mae" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "persistence_vs_trained_ols_comparison" not in source
    assert "trained_ols_mae_mw" not in source
