"""Consumer Load lag-24h OLS MAE evaluation stays a narrow ML metric seam."""
...
EVALUATION_MODULE = CONSUMER_LOAD_ML_ROOT / "lag_24h_linear_regression_evaluation.py"
...
def test_evaluation_module_lives_under_ml_consumer_load() -> None:
    ...
    assert _module_class_names(EVALUATION_MODULE) == ["ConsumerLoadLag24hLinearRegressionMAEResult"]
...
def test_result_is_frozen_slotted_with_exactly_case_count_and_mae_mw() -> None:
    class_def = _class_def(EVALUATION_MODULE, "ConsumerLoadLag24hLinearRegressionMAEResult")
    ...
    assert (
        _annassign_field_names(EVALUATION_MODULE, "ConsumerLoadLag24hLinearRegressionMAEResult")
        == ALLOWED_RESULT_FIELDS
    )
...
def test_evaluator_is_keyword_only_sync_over_prediction_artifacts() -> None:
    ...
    evaluator = next(
        ...
        and node.name == "evaluate_consumer_load_lag_24h_linear_regression_mae"
    )
    ...
    assert ast.unparse(evaluator.returns) == "ConsumerLoadLag24hLinearRegressionMAEResult"
...
def test_application_agents_do_not_import_the_ols_mae() -> None:
...
def test_orchestration_and_langgraph_do_not_import_the_ols_mae() -> None:
    ...
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" not in graph_names
    assert "ConsumerLoadLag24hLinearRegressionMAEResult" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_does_not_reference_the_ols_mae() -> None:
    ...
    assert "ConsumerLoadLag24hLinearRegressionMAEResult" not in app_source
    assert "evaluate_consumer_load_lag_24h_linear_regression_mae" not in app_source
...
def test_no_dam_imports_in_consumer_load_ols_mae() -> None:
    names = imported_names(EVALUATION_MODULE)
    source = EVALUATION_MODULE.read_text(encoding="utf-8")
