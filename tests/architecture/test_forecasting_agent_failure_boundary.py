"""Phase 3 agent-failure attribution stays application-owned and LangGraph-free."""
...
FAILURE_MODULE = ORCHESTRATION_ROOT / "forecasting_agent_failure.py"
...
def test_attribution_module_exposes_exactly_one_production_class() -> None:
    assert _module_class_names(FAILURE_MODULE) == ["ForecastingAgentFailure"]
    class_def = _class_def(FAILURE_MODULE, "ForecastingAgentFailure")
    assert _base_names(class_def) == {"Exception"}
    production_classes: list[str] = []
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        parsed = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        for node in parsed.body:
            if isinstance(node, ast.ClassDef) and node.name == "ForecastingAgentFailure":
                production_classes.append(path.relative_to(SRC_ROOT).as_posix())
    assert production_classes == [
        "energy_trading/application/orchestration/forecasting_agent_failure.py"
    ]
...
def test_policy_graph_and_handling_remain_unwired_to_attribution() -> None:
    ...
    assert "ForecastingAgentFailure" in executor_names
    executor_modules = imported_modules(EXECUTOR_MODULE)
    assert "energy_trading.application.orchestration.forecasting_agent_failure" in executor_modules
...
def test_api_composition_does_not_import_or_construct_attribution() -> None:
    ...
    assert "forecastingagentfailure" not in app_source
    assert "forecasting_agent_failure" not in app_source
