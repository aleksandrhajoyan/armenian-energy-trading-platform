"""Forecasting ExceptionGroup extraction stays application-owned."""
...
def test_extraction_module_exposes_exactly_one_public_function() -> None:
    ...
    assert [node.name for node in public_functions] == ["extract_forecasting_agent_failures"]
...
def test_extractor_signature_is_exception_group_to_attributed_tuple() -> None:
    ...
    assert "BaseExceptionGroup" in parameter_annotation
    assert ast.unparse(extractor.returns) == "tuple[ForecastingAgentFailure, ...]"
...
def test_extractor_traverses_groups_without_rebuilding_or_inspecting_text() -> None:
    ...
    assert "extract_forecasting_agent_failures" in call_names
    assert "ForecastingAgentFailure" not in call_names
...
def test_workflow_state_shape_is_unchanged_by_extraction() -> None:
...
def test_api_composition_does_not_import_or_construct_the_extractor() -> None:
    ...
    assert "extract_forecasting_agent_failures" not in app_source
    assert "forecasting_exception_group" not in app_source
