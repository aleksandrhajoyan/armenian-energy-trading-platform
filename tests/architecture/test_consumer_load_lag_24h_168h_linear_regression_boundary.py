"""Consumer Load 24h+168h OLS fit stays a narrow ML fitting seam."""
...
FIT_MODULE = CONSUMER_LOAD_ML_ROOT / "lag_24h_168h_linear_regression.py"
...
UNAUTHORIZED_FIT_TOKENS = (
    ...
    "lag_24h_168h_chronological_split",
    "split_consumer_load_feature_rows_chronologically",
    "split_consumer_load_lag_24h_168h_feature_rows_chronologically",
    "fit_consumer_load_lag_24h_linear_regression",
    "build_consumer_load_lag_24h_168h_feature_rows",
...
def test_fit_module_lives_under_ml_consumer_load() -> None:
    assert FIT_MODULE.is_relative_to(CONSUMER_LOAD_ML_ROOT)
    assert FIT_MODULE.name == "lag_24h_168h_linear_regression.py"
    assert _module_class_names(FIT_MODULE) == ["ConsumerLoadLag24h168hLinearRegressionFit"]
    assert _module_function_names(FIT_MODULE) == [
        "fit_consumer_load_lag_24h_168h_linear_regression"
    ]


def test_result_is_frozen_slotted_with_exactly_three_float_fields() -> None:
    class_def = _class_def(FIT_MODULE, "ConsumerLoadLag24h168hLinearRegressionFit")
    ...
    assert (
        _annassign_field_names(FIT_MODULE, "ConsumerLoadLag24h168hLinearRegressionFit")
        == ALLOWED_RESULT_FIELDS
    )
...
def test_fitter_is_keyword_only_over_chunk_141_rows() -> None:
    tree = ast.parse(FIT_MODULE.read_text(encoding="utf-8"), filename=str(FIT_MODULE))
    fitter = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "fit_consumer_load_lag_24h_168h_linear_regression"
    )
    ...
    assert ast.unparse(fitter.returns) == "ConsumerLoadLag24h168hLinearRegressionFit"


def test_fit_uses_both_lag_features_and_finite_result_checking() -> None:
    identifiers = _identifier_names(FIT_MODULE)
    assert "lag_24h_mw" in identifiers
    assert "lag_168h_mw" in identifiers
    assert "target_value_mw" in identifiers
    assert "isfinite" in identifiers
    assert "determinant" in identifiers
    called = _called_names(FIT_MODULE)
...
def test_fit_depends_only_on_allowed_inward_contracts() -> None:
    leaked = sorted(
        module
    ...
    assert "ConsumerLoadLag24hLinearRegressionFit" not in names
...
def test_fit_does_not_generate_identity_or_use_io() -> None:
...
def test_fit_does_not_introduce_generic_or_numerical_frameworks() -> None:
    class_names = _module_class_names(FIT_MODULE)
    ...
    leaked_tokens = [token for token in UNAUTHORIZED_FIT_TOKENS if token in source]
    assert leaked_tokens == []
    assert "predict" not in identifiers
...
def test_application_agents_do_not_import_the_two_lag_ols_fit() -> None:
    assert collect_import_violations(AGENTS_ROOT, ("energy_trading.ml",)) == []
    for path in sorted(AGENTS_ROOT.rglob("*.py")):
        names = imported_names(path)
        assert "ConsumerLoadLag24h168hLinearRegressionFit" not in names
        assert "fit_consumer_load_lag_24h_168h_linear_regression" not in names
        source = path.read_text(encoding="utf-8")
        assert "lag_24h_168h_linear_regression" not in source


def test_orchestration_and_langgraph_do_not_import_the_two_lag_ols_fit() -> None:
    ...
    assert "fit_consumer_load_lag_24h_168h_linear_regression" not in graph_names
    assert "ConsumerLoadLag24h168hLinearRegressionFit" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_does_not_reference_the_two_lag_ols_fit() -> None:
    ...
    assert "ConsumerLoadLag24h168hLinearRegressionFit" not in app_source
    assert "fit_consumer_load_lag_24h_168h_linear_regression" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "lag_24h_168h_linear_regression" not in source
    assert "lag_168h_coefficient" not in source


def test_no_dam_imports_in_two_lag_ols_fit() -> None:
    names = imported_names(FIT_MODULE)
    source = FIT_MODULE.read_text(encoding="utf-8")
    ...
    assert "energy_trading.ml.dam" not in imported_modules(FIT_MODULE)
