"""Chunk 87 provider-aware document vector index composition stays an outer builder."""
...
BUILDER_MODULE = COMPOSITION_ROOT / "document_vector_index_runtime.py"
...
def _builder_function(path: Path) -> ast.FunctionDef:
    for node in _module_function_defs(path):
        if isinstance(node, ast.FunctionDef) and node.name == (
            ...
            return node
    msg = "build_document_vector_index_provider_runtime not found"
...
def test_builder_imports_only_approved_provider_and_chunk_86_surfaces() -> None:
    leaked = sorted(
        module
    ...
    assert "build_document_vector_index_execution" in names
...
def test_builder_exposes_exactly_one_public_synchronous_function() -> None:
    ...
    assert [node.name for node in public] == ["build_document_vector_index_provider_runtime"]
    ...
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        ...
        for node in parsed.body:
            if (
                ...
                and node.name == "build_document_vector_index_provider_runtime"
            ):
...
def test_builder_signature_is_keyword_only_injected_clients_config_and_model() -> None:
...
def test_builder_constructs_two_adapters_and_delegates_to_chunk_86() -> None:
    builder = _builder_function(BUILDER_MODULE)
    control = [
        type(node).__name__
    ...
    assert _call_name(first.value) == "OpenAIDocumentEmbeddingAdapter"
    first_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in first.value.keywords}
    assert first_keywords == {"client": "openai_client", "model": "document_embedding_model"}
    assert isinstance(second, ast.Assign)
    assert isinstance(second.value, ast.Call)
    assert _call_name(second.value) == "QdrantDocumentVectorIndex"
    ...
    assert _call_name(third.value) == "build_document_vector_index_execution"
    return_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in third.value.keywords}
    ...
    leaked_dam = sorted(name for name in identifiers if name in DAM_RULE_IDENTIFIERS)
    assert leaked_dam == []
...
def test_application_and_domain_do_not_import_the_provider_builder() -> None:
    forbidden = (
        "energy_trading.api",
...
def test_http_create_app_does_not_invoke_the_provider_builder() -> None:
    forbidden_wiring = (
        "openai",
        "qdrant_client",
    ...
    assert "build_document_vector_index_provider_runtime" not in app_source
    lowered = app_source.lower()
    assert "build_document_vector_index_provider_runtime" not in lowered
    assert "build_document_vector_index_provider_runtime" not in NEUTRAL_BUILDER_MODULE.read_text(
        encoding="utf-8"
    )


def test_graph_extraction_and_adapters_remain_unwired_to_the_provider_builder() -> None:
    ...
    for path in (OPENAI_DOCUMENT_EMBEDDING, QDRANT_INDEX, EXTRACTION_PORT):
        ...
        assert "build_document_vector_index_provider_runtime" not in path_names
        path_modules = imported_modules(path)
        assert "energy_trading.api.composition.document_vector_index_runtime" not in path_modules


def test_regulatory_composition_remains_unwired_to_the_provider_builder() -> None:
    for path in REGULATORY_COMPOSITION_MODULES:
        names = imported_names(path)
        assert "build_document_vector_index_provider_runtime" not in names
        modules = imported_modules(path)
        assert "energy_trading.api.composition.document_vector_index_runtime" not in modules
        source = path.read_text(encoding="utf-8")
        assert "build_document_vector_index_provider_runtime" not in source
