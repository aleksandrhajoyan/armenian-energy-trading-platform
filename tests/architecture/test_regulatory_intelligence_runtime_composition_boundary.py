"""Chunk 71 provider-aware Regulatory composition stays an outer builder."""
...
BUILDER_MODULE = COMPOSITION_ROOT / "regulatory_intelligence_runtime.py"
...
def _builder_function(path: Path) -> ast.FunctionDef:
    for node in _module_function_defs(path):
        if isinstance(node, ast.FunctionDef) and node.name == (
            ...
            return node
    msg = "build_regulatory_intelligence_provider_runtime not found"
...
def test_builder_imports_only_approved_provider_and_chunk_67_surfaces() -> None:
    leaked = sorted(
        module
    ...
    assert "OpenAIDocumentQueryEmbeddingAdapter" in names
    assert "OpenAIRegulatoryConstraintInferenceAdapter" in names
    assert "build_regulatory_intelligence_query_execution" in names
...
def test_builder_exposes_exactly_one_public_synchronous_function() -> None:
    ...
    assert [node.name for node in public] == ["build_regulatory_intelligence_provider_runtime"]
...
def test_builder_signature_is_keyword_only_injected_clients_config_and_models() -> None:
...
def test_builder_constructs_three_adapters_and_delegates_to_chunk_67() -> None:
    builder = _builder_function(BUILDER_MODULE)
    control = [
        type(node).__name__
    ...
    assert _call_name(first.value) == "OpenAIDocumentQueryEmbeddingAdapter"
    first_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in first.value.keywords}
    assert first_keywords == {"client": "openai_client", "model": "query_embedding_model"}
    assert isinstance(second, ast.Assign)
    assert isinstance(second.value, ast.Call)
    assert _call_name(second.value) == "QdrantDocumentVectorSearch"
    ...
    assert _call_name(third.value) == "OpenAIRegulatoryConstraintInferenceAdapter"
    third_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in third.value.keywords}
    assert third_keywords == {"client": "openai_client", "model": "constraint_inference_model"}
    assert isinstance(fourth, ast.Return)
    assert isinstance(fourth.value, ast.Call)
    assert _call_name(fourth.value) == "build_regulatory_intelligence_query_execution"
    return_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in fourth.value.keywords}
...
def test_http_create_app_does_not_invoke_the_provider_builder() -> None:
    ...
    assert "build_regulatory_intelligence_provider_runtime" not in app_source
    lowered = app_source.lower()
    assert "openai" not in lowered
    assert "qdrant" not in lowered
    assert "build_regulatory_intelligence_provider_runtime" not in NEUTRAL_BUILDER_MODULE.read_text(
        encoding="utf-8"
    )


def test_graph_remains_unwired_to_the_provider_builder() -> None:
