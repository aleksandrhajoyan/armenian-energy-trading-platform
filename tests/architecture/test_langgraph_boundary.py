...
FORECASTING_FAILURE_FACT_MODULE = ORCHESTRATION_ROOT / "forecasting_failure_fact.py"
...
FORECASTING_STRICT_SINGLE_FAILURE_SELECTOR_MODULE = (
    ORCHESTRATION_ROOT / "forecasting_strict_single_failure_selector.py"
)
...
def test_forecasting_workflow_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_WORKFLOW_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_WORKFLOW_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )
...
def test_forecasting_failure_transition_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_FAILURE_TRANSITION_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_FAILURE_TRANSITION_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )


def test_forecasting_agent_failure_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_AGENT_FAILURE_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_AGENT_FAILURE_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )
...
def test_forecasting_failure_fact_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_FAILURE_FACT_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_FAILURE_FACT_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )


def test_forecasting_failure_classification_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_FAILURE_CLASSIFICATION_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_FAILURE_CLASSIFICATION_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )
...
def test_forecasting_strict_single_failure_selector_module_remains_langgraph_free() -> None:
    names = imported_names(FORECASTING_STRICT_SINGLE_FAILURE_SELECTOR_MODULE)
    assert "langgraph" not in names
    assert "langchain" not in names
    assert "langchain_core" not in names
    modules = imported_modules(FORECASTING_STRICT_SINGLE_FAILURE_SELECTOR_MODULE)
    assert not any(
        is_forbidden(module, ("langgraph", "langchain", "langchain_core")) for module in modules
    )
...
def test_forecasting_node_delegates_once_to_step_run_without_try_except() -> None:
    ...
    class_def = next(
        node
        for node in ast.walk(tree)
        if isinstance(node, ast.ClassDef) and node.name == "_ForecastingNode"
    )
    call_method = next(
        node
