"""Chunk 88 document vector index runtime settings stay shared, SDK-free, and unwired."""
...
SETTINGS_MODULE = CONFIG_ROOT / "document_vector_index.py"
...
def _settings_class() -> ast.ClassDef:
    ...
    return next(
        ...
        if isinstance(node, ast.ClassDef) and node.name == "DocumentVectorIndexRuntimeSettings"
    )
...
def test_settings_module_lives_under_shared_config() -> None:
    ...
    assert relative == "shared/config/document_vector_index.py"


def test_exact_class_and_three_fields() -> None:
    settings = _settings_class()
    assert settings.name == "DocumentVectorIndexRuntimeSettings"
...
def test_dedicated_environment_prefix_and_exact_variable_names() -> None:
    ...
    assert 'env_prefix="ENERGY_DOCUMENT_INDEX_"' in source
...
def test_inner_layers_do_not_import_document_index_runtime_settings() -> None:
...
def test_no_runtime_objects_or_generic_frameworks() -> None:
    ...
    assert class_names == {"DocumentVectorIndexRuntimeSettings"}
...
def test_loader_signature_is_keyword_only_and_uncached() -> None:
    ...
    loader = next(
        ...
        and node.name == "load_document_vector_index_runtime_settings"
    )
    assert [arg.arg for arg in loader.args.args] == []
    assert [arg.arg for arg in loader.args.kwonlyargs] == ["env_file"]
    assert ast.unparse(loader.returns) == "DocumentVectorIndexRuntimeSettings"
    source = SETTINGS_MODULE.read_text(encoding="utf-8")
...
def test_create_app_remains_unwired() -> None:
    ...
    assert "load_document_vector_index_runtime_settings" not in call_names
    assert "DocumentVectorIndexRuntimeSettings" not in call_names
...
def test_chunk_87_builder_does_not_load_settings() -> None:
    ...
    assert "energy_trading.shared.config.document_vector_index" not in modules
...
def test_regulatory_runtime_settings_are_unchanged() -> None:
    source = REGULATORY_SETTINGS_MODULE.read_text(encoding="utf-8")
    assert "document_embedding_model" not in source
    assert "ENERGY_DOCUMENT_INDEX_" not in source
    assert "DocumentVectorIndexRuntimeSettings" not in source
    tree = ast.parse(source, filename=str(REGULATORY_SETTINGS_MODULE))
    settings = next(
        node
        for node in tree.body
