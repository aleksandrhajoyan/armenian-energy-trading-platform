"""Consumer Load chronological feature split stays a narrow ML preparation seam."""
...
SPLIT_MODULE = CONSUMER_LOAD_ML_ROOT / "chronological_feature_split.py"
...
def test_split_module_lives_under_ml_consumer_load() -> None:
    ...
    assert _module_class_names(SPLIT_MODULE) == ["ConsumerLoadChronologicalFeatureSplit"]
...
def test_result_is_frozen_slotted_with_exactly_two_fields() -> None:
    class_def = _class_def(SPLIT_MODULE, "ConsumerLoadChronologicalFeatureSplit")
    ...
    assert _annassign_field_names(SPLIT_MODULE, "ConsumerLoadChronologicalFeatureSplit") == (
        ALLOWED_RESULT_FIELDS
    )
...
def test_splitter_is_keyword_only_sync_over_feature_rows_and_cutoff() -> None:
    tree = ast.parse(SPLIT_MODULE.read_text(encoding="utf-8"), filename=str(SPLIT_MODULE))
    splitter = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "split_consumer_load_feature_rows_chronologically"
    )
    ...
    assert ast.unparse(splitter.returns) == "ConsumerLoadChronologicalFeatureSplit"
...
def test_split_does_not_introduce_generic_dataset_or_training() -> None:
...
def test_application_agents_do_not_import_the_feature_split() -> None:
...
def test_orchestration_and_langgraph_do_not_import_the_feature_split() -> None:
    ...
    assert "split_consumer_load_feature_rows_chronologically" not in graph_names
    assert "ConsumerLoadChronologicalFeatureSplit" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_composition_does_not_import_or_construct_the_feature_split() -> None:
    ...
    assert "ConsumerLoadChronologicalFeatureSplit" not in app_source
    assert "split_consumer_load_feature_rows_chronologically" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "chronological_feature_split" not in source
    assert "training_rows" not in source
    assert "evaluation_rows" not in source


def test_no_dam_imports_in_consumer_load_feature_split() -> None:
    names = imported_names(SPLIT_MODULE)
    source = SPLIT_MODULE.read_text(encoding="utf-8")
