"""Consumer Load lag-24h OLS fit stays a narrow ML fitting seam."""
...
FIT_MODULE = CONSUMER_LOAD_ML_ROOT / "lag_24h_linear_regression.py"
...
def test_fit_module_lives_under_ml_consumer_load() -> None:
    ...
    assert _module_class_names(FIT_MODULE) == ["ConsumerLoadLag24hLinearRegressionFit"]
    assert _module_function_names(FIT_MODULE) == ["fit_consumer_load_lag_24h_linear_regression"]
    common_root = ML_ROOT / "common"
    common_python = sorted(common_root.rglob("*.py")) if common_root.exists() else []
    assert common_python == []


def test_result_is_frozen_slotted_with_exactly_two_float_fields() -> None:
    class_def = _class_def(FIT_MODULE, "ConsumerLoadLag24hLinearRegressionFit")
    ...
    assert _annassign_field_names(FIT_MODULE, "ConsumerLoadLag24hLinearRegressionFit") == (
        ALLOWED_RESULT_FIELDS
    )
...
def test_fitter_is_keyword_only_sync_over_feature_rows() -> None:
    tree = ast.parse(FIT_MODULE.read_text(encoding="utf-8"), filename=str(FIT_MODULE))
    fitter = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "fit_consumer_load_lag_24h_linear_regression"
    )
    ...
    assert ast.unparse(fitter.returns) == "ConsumerLoadLag24hLinearRegressionFit"
...
def test_fit_does_not_introduce_generic_trainer_prediction_or_metrics() -> None:
...
def test_application_agents_do_not_import_the_ols_fit() -> None:
...
def test_orchestration_and_langgraph_do_not_import_the_ols_fit() -> None:
    ...
    assert "fit_consumer_load_lag_24h_linear_regression" not in graph_names
    assert "ConsumerLoadLag24hLinearRegressionFit" not in graph_names
    assert "ForecastingExecutionPort" not in graph_names


def test_api_composition_does_not_import_or_construct_the_ols_fit() -> None:
    ...
    assert "ConsumerLoadLag24hLinearRegressionFit" not in app_source
    assert "fit_consumer_load_lag_24h_linear_regression" not in app_source
...
def test_workflow_state_shape_is_unchanged() -> None:
    ...
    assert "lag_24h_linear_regression" not in source
    assert "intercept_mw" not in source


def test_no_dam_imports_in_consumer_load_ols_fit() -> None:
    names = imported_names(FIT_MODULE)
    source = FIT_MODULE.read_text(encoding="utf-8")
