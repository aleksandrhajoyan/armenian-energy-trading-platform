"""Chunk 110 configured document-index collection ensure stays a narrow composition seam."""
...
from tests.architecture.import_inspection import (
    DOCUMENT_VECTOR_INDEX_COLLECTION_ENSURE_COMPOSITION_RELATIVE,
    SRC_ROOT,
...
BUILDER_MODULE = COMPOSITION_ROOT / "document_vector_index_collection_ensure.py"
...
def _builder_function(path: Path) -> ast.AsyncFunctionDef:
    for node in _module_function_defs(path):
        if isinstance(node, ast.AsyncFunctionDef) and node.name == (
            ...
            return node
    msg = "ensure_configured_document_vector_index_collection_ready not found"
...
def test_builder_lives_in_api_composition_package() -> None:
    ...
    assert relative == DOCUMENT_VECTOR_INDEX_COLLECTION_ENSURE_COMPOSITION_RELATIVE
    ...
    for path in sorted(PRODUCTION_ROOT.rglob("*.py")):
        ...
        for node in parsed.body:
            if (
                ...
                and node.name == "ensure_configured_document_vector_index_collection_ready"
            ):
                production_builders.append(path.relative_to(SRC_ROOT).as_posix())
    assert production_builders == [DOCUMENT_VECTOR_INDEX_COLLECTION_ENSURE_COMPOSITION_RELATIVE]
...
def test_builder_constructs_one_config_maps_once_and_awaits_ensure_once() -> None:
    builder = _builder_function(BUILDER_MODULE)
    control = [
        type(node).__name__
    ...
    assert _call_name(awaits[0].value) == "ensure_qdrant_document_collection_ready"
    ...
    ensure_call = awaits[0].value
    ensure_keywords = {keyword.arg: ast.unparse(keyword.value) for keyword in ensure_call.keywords}
    assert ensure_keywords == {
        "client": "client",
...
def test_composition_init_exports_the_configured_ensure() -> None:
    ...
    assert "ensure_configured_document_vector_index_collection_ready" in source
    assert "map_qdrant_document_vector_distance" not in names
    assert "ensure_qdrant_document_collection_ready" not in names


def test_application_and_domain_do_not_import_the_configured_ensure() -> None:
    forbidden = (
        "energy_trading.api",
...
def test_http_create_app_and_runtimes_remain_unwired() -> None:
    ...
    assert "ensure_configured_document_vector_index_collection_ready" not in app_source
    for path in UNWIRED_RUNTIME_MODULES:
        path_names = imported_names(path)
        assert "ensure_configured_document_vector_index_collection_ready" not in path_names
