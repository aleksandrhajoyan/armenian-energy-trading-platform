"""Market Monitoring agent and source port remain application-owned and provider-neutral."""
...
AGENT_MODULE = PRODUCTION_ROOT / "application" / "agents" / "market_monitoring.py"
PORT_MODULE = PRODUCTION_ROOT / "application" / "ports" / "market_price_records.py"
...
ALLOWED_AGENT_IMPORTS = frozenset(
    {
        ...
        "energy_trading.application.ports.market_price_records",
...
def test_market_modules_do_not_import_outer_layers_or_vendors() -> None:
...
def test_market_source_port_is_nongeneric_protocol_with_async_fetch() -> None:
    class_def = _class_def(PORT_MODULE, "MarketPriceRecordSourcePort")
    ...
    assert ast.unparse(fetch_fn.returns) == "tuple[MarketPriceRecord, ...]"
...
def test_market_source_port_module_has_no_concrete_implementation() -> None:
    assert _module_class_names(PORT_MODULE) == ["MarketPriceRecordSourcePort"]


def test_market_agent_public_surface_is_name_and_run() -> None:
    class_def = _class_def(AGENT_MODULE, "MarketMonitoringAgent")
    ...
    assert ast.unparse(run_fn.args.args[1].annotation) == "MarketMonitoringRequest"
    assert ast.unparse(run_fn.returns) == "MarketMonitoringResult"
    ...
    assert "PriceForecastPoint" not in imported_names(AGENT_MODULE)
...
def test_market_request_and_result_have_exact_fields() -> None:
    assert _annassign_field_names(AGENT_MODULE, "MarketMonitoringRequest") == (
        "market_id",
        "horizon_start",
        "horizon_end",
    )
    assert _annassign_field_names(AGENT_MODULE, "MarketMonitoringResult") == ("records",)


def test_market_modules_have_no_dam_business_rule_constants() -> None:
    for path in (AGENT_MODULE, PORT_MODULE):
...
def test_graph_module_does_not_import_or_invoke_ingestion_agents() -> None:
    ...
    for agent_name in INGESTION_AGENTS:
    ...
    assert "MarketPriceRecordSourcePort" not in names
    assert "MarketMonitoringRequest" not in names
    ...
    assert "energy_trading.application.ports.market_price_records" not in modules
    graph_source = GRAPH_MODULE.read_text(encoding="utf-8")
    for agent_name in INGESTION_AGENTS:
        assert agent_name not in graph_source
    assert "MarketPriceRecordSourcePort" not in graph_source
...
def test_api_composition_does_not_import_or_construct_ingestion_agents() -> None:
    ...
    assert "marketmonitoringagent" not in app_source
    assert "marketpricerecordsourceport" not in app_source
