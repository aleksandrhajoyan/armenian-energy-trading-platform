...
- **Consequences:** Application call sites can ingest Consumption `.xlsx` through the existing port. REST adapters, other domain Excel readers, unit conversion, timezone inference, time-series cleaning, and formula engines remain later chunks. A later normalization stage can interpret kW headers and naive Excel clocks because those facts never become canonical MW or UTC here.
