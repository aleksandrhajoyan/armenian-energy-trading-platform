...
- **Decision:**
  ...
  - Concrete Qdrant `Distance.COSINE` / `DOT` / `EUCLID` / `MANHATTAN` remain confined to the Chunk 109 mapper. This composition imports `AsyncQdrantClient` only as an injected type.
  - `managed_regulatory_intelligence_runtime`, `loaded_regulatory_intelligence_runtime`, Regulatory/production lifespans, `create_app()`, HTTP routers, Document Index runtime, PDF extraction/index execution, API dependencies, and LangGraph do not call this function. Regulatory managed-runtime wiring, Regulatory loaded-runtime distance settings loading, and Regulatory production startup verification remain future architectural decisions. Collection migration, delete/recreate/update, race retry/locking, payload indexes, aliases, schema versioning, shared Regulatory/Document Index collection policy, OCR, automatic corpus scanning/indexing, Pricing & Sales, and Regulatory LangGraph wiring remain deferred.
- **Consequences:** Explicit callers can verify an existing Regulatory collection from already-constructed settings without inventing a default metric, creating an empty corpus, or a generic collection manager. Regulatory production readiness verification is not active. Regulatory Intelligence parent capability remains incomplete.
