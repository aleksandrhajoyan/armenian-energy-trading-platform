...
- **Decision:**
  ...
  - Production Qdrant modules still do not recreate, update, or delete collections and still do not hardcode distance enum members. Chunk 107 may inspect `VectorParams` and the `Distance` type in the read-only collection-readiness verifier to confirm unnamed dense-vector dimension and exact expected distance; it must not construct `VectorParams` or hardcode `Distance` members. Chunk 106 may construct unnamed dense `VectorParams` and call `create_collection` once from `collection_creation.py` using a caller-supplied `Distance`; it must not default a metric, probe existence, or ensure/reconcile collections. Automatic production collection provisioning and production distance policy remain deferred.
...
- **Decision:**
  ...
  - Inputs are an already-created `AsyncQdrantClient` plus existing `QdrantDocumentVectorConfig`. The function does not construct or close the client, load settings, or inspect the environment. Chunk 107 later added a required caller-supplied expected `Distance` without changing those ownership rules.
  ...
  - The module may import/inspect `VectorParams` to recognize the unnamed dense shape. It must not construct `VectorParams` and must not create, update, or delete collections or payload indexes. Chunk 107 later required a caller-supplied expected `Distance` without making this verifier choose a production metric.
...
- **Consequences:** Explicit callers can preflight an already-provisioned document collection without mutating Qdrant. Chunk 106 later added a separate create-only mutation; the verifier remains independent and still does not create collections. Chunk 107 later required exact expected distance alongside vector size. Automatic production collection provisioning, production distance selection, payload-index provisioning, automatic startup checks, and Pricing & Sales remain deferred. Regulatory Intelligence parent capability remains incomplete.

---

## ADR-116 — Explicit Qdrant Document Collection Creation

- **Status:** Accepted
- **Context:** ADR-005/032/033/034/115 remain the Qdrant direction: official async client, insert-only document adapters, test-only live-fixture provisioning, and a separate read-only readiness verifier. Callers had no infrastructure-only way to create a new unnamed dense document collection from an explicit vector size plus an explicit caller-selected Qdrant distance without inventing collection managers, defaulting a production metric, probing existence, or wiring startup/HTTP/LangGraph.
- **Decision:**
  - ADR-005, ADR-032, ADR-033, ADR-034, and ADR-115 remain Accepted and are not superseded.
  - Chunk 105 remains verify-only. Chunk 106 is create-only. They must not call each other. This chunk does not introduce `ensure_qdrant_document_collection`.
  - Infrastructure owns `create_qdrant_document_collection` in `infrastructure/vector_store/qdrant/collection_creation.py`. It is a keyword-only async function, not an application port and not a manager/registry/lifecycle/provisioner service.
  ...
  - Production constructs exactly one unnamed/default dense `VectorParams` with `size=config.vector_size` and `distance=distance`, then calls `create_collection` exactly once with `collection_name=config.collection_name` and that `vectors_config`. The function returns `None`.
  - There is no existence lookup (`get_collection` / `get_collections` / `collection_exists`), no recreate/update/delete, no payload-index mutation, and no idempotent reconciliation. An already-existing collection is a provider creation failure.
  ...
  - Provider creation failures become sanitized `DependencyUnavailableError`. The public message must not leak collection name, host, API key, provider text, `VectorParams` repr, or the distance value.
  - Index/search adapters remain unchanged and do not call this function. `create_app()`, production/document-index/Regulatory lifespans and runtimes, Chunk 104 execution, Chunk 105 readiness, HTTP routers, and LangGraph remain unwired. Production still performs zero automatic collection creation.
- **Consequences:** Explicit callers can create a document collection once when they already know the vector size and have chosen a Qdrant distance. Automatic ensure/startup provisioning, production distance policy, collection migration, payload indexes, and Pricing & Sales remain deferred. Regulatory Intelligence parent capability remains incomplete.

---

## ADR-117 — Explicit Expected Distance in Qdrant Collection Readiness

- **Status:** Accepted
- **Context:** ADR-005/032/033/034/115/116 remain the Qdrant direction: official async client, insert-only document adapters, test-only live-fixture provisioning, create-only collection mutation with explicit `Distance`, and a separate read-only readiness verifier. After Chunk 106, creation required `(config.vector_size, explicit distance)` while readiness still checked size only. That left an invariant gap: a collection created with one metric could pass readiness even if the caller expected another.
- **Decision:**
  - ADR-005, ADR-032, ADR-033, ADR-034, ADR-115, and ADR-116 remain Accepted and are not superseded.
  - Chunk 107 evolves the existing `verify_qdrant_document_collection_ready` function. It does not add a second verifier, an application port, a manager, or `verify_collection_compatible`.
  - The public signature is keyword-only `client`, `config`, and required `distance: Distance` with no default. The caller supplies expected Qdrant `Distance`. Production does not default COSINE/DOT/EUCLID/MANHATTAN, does not translate metrics, and does not add distance to settings, env, `QdrantDocumentVectorConfig`, `QdrantSettings`, `DocumentVectorIndexRuntimeSettings`, or `RegulatoryIntelligenceRuntimeSettings`.
  - Readiness remains one `get_collection(collection_name=config.collection_name)` metadata lookup. Compatible unnamed dense configuration is SDK `VectorParams` whose `size` equals `config.vector_size` and whose `distance` equals the caller-supplied expected `Distance`. The function returns `None` only when both match exactly.
  - Named-vector mappings, sparse-only/missing dense configuration, malformed metadata, size mismatch, distance mismatch, missing collections, and provider call failures fail closed as the same sanitized `DependencyUnavailableError("Document vector collection is unavailable.")`. Local compatibility failures do not fabricate a cause. Provider exceptions remain chained `from exc`.
  - The module may import `Distance` and `VectorParams` because it is Qdrant infrastructure. It must not construct `VectorParams`, must not hardcode `Distance` members, and must not create, update, delete, or recreate collections.
  - Chunk 106 remains create-only. Chunk 107 remains verify-only. They must not call each other. This chunk does not introduce `ensure_qdrant_document_collection` or any verify-then-create composition.
  - Symmetry:
    - Chunk 106 creation: `(config.vector_size, explicit distance)`
    - Chunk 107 readiness: `(config.vector_size, explicit expected distance)`
  - Index/search adapters remain unchanged and do not call this verifier. `create_app()`, production/document-index/Regulatory lifespans and runtimes, Chunk 104 execution, Chunk 106 creation, HTTP routers, and LangGraph remain unwired. No automatic startup readiness occurs.
...
## ADR-122 — Regulatory Collection Readiness Is Verify-Existing and Unwired

- **Status:** Accepted
- **Context:** ADR-005/032/033/034/115/116/117/118/119/120/121 remain the Qdrant direction: official async client, insert-only document adapters, test-only live-fixture provisioning, create-only mutation, verify-only readiness, create-if-missing / verify-if-present ensure, explicit provider-neutral distance configuration with no default, a configured document-index ensure composition, and Document Vector Index managed-runtime ownership of that ensure. The Document Index writer may create its target collection because it owns indexing. The Regulatory query path reads an authoritative regulatory corpus. Automatically creating a missing empty Regulatory collection could make corpus absence look like a valid empty retrieval result. Callers who already hold an `AsyncQdrantClient`, `RegulatoryIntelligenceRuntimeSettings`, and `QdrantDocumentVectorDistanceSettings` had no API composition seam that constructed `QdrantDocumentVectorConfig`, mapped the configured metric through Chunk 109, and delegated once to Chunk 107 verify-existing readiness.
- **Decision:**
  - ADR-005, ADR-032, ADR-033, ADR-034, ADR-115, ADR-116, ADR-117, ADR-118, ADR-119, ADR-120, and ADR-121 remain Accepted and are not superseded.
  - Chunk 112 introduces API composition `verify_configured_regulatory_intelligence_collection_ready` in `api/composition/regulatory_intelligence_collection_readiness.py`. It is a keyword-only async function, not a manager, registry, factory hierarchy, or DI container.
  - Inputs are an already-created `AsyncQdrantClient`, already-constructed `RegulatoryIntelligenceRuntimeSettings`, and already-constructed `QdrantDocumentVectorDistanceSettings`. The function does not load environment or settings, does not accept `env_file`, and does not construct or close clients.
  - It constructs exactly one existing `QdrantDocumentVectorConfig` from `runtime_settings.qdrant_collection_name` and `runtime_settings.qdrant_vector_size`. `query_embedding_model` and `constraint_inference_model` do not participate.
  - It maps `distance_settings.document_vector_distance` through published `map_qdrant_document_vector_distance` once. It does not locally reproduce the mapping table or default COSINE or any other metric.
  - It awaits `verify_qdrant_document_collection_ready` once with the exact injected client, the newly constructed config, and the mapped Qdrant `Distance`, then returns `None`.
  - It does not call `ensure_qdrant_document_collection_ready`, `create_qdrant_document_collection`, `collection_exists`, `create_collection`, `recreate_collection`, `delete_collection`, or `update_collection`. Published sanitized errors, including `DependencyUnavailableError`, propagate unchanged.
  - Concrete Qdrant `Distance.COSINE` / `DOT` / `EUCLID` / `MANHATTAN` remain confined to the Chunk 109 mapper. This composition imports `AsyncQdrantClient` only as an injected type.
  - `create_app()`, production/document-index/Regulatory lifespans, loaded runtime, HTTP routers, Document Index runtime, PDF extraction/index execution, API dependencies, and LangGraph do not call this function directly. Chunk 113 later authorized the existing Regulatory managed runtime to await it on the managed Qdrant client. Collection migration, delete/recreate/update, race retry/locking, payload indexes, aliases, schema versioning, shared Regulatory/Document Index collection policy, OCR, automatic corpus scanning/indexing, Pricing & Sales, and Regulatory LangGraph wiring remain deferred.
- **Consequences:** Explicit callers can verify an existing Regulatory collection from already-constructed settings without inventing a default metric, creating an empty corpus, or a generic collection manager. Regulatory Intelligence parent capability remains incomplete.

---

## ADR-123 — Regulatory Managed Runtime Owns Fail-Closed Collection Readiness

- **Status:** Accepted
- **Context:** ADR-005/032/033/034/115/116/117/118/119/120/121/122 remain the Qdrant direction: official async client, insert-only document adapters, test-only live-fixture provisioning, create-only mutation, verify-only readiness, create-if-missing / verify-if-present ensure, explicit provider-neutral distance configuration with no default, a configured document-index ensure composition, Document Vector Index managed-runtime ownership of that ensure, and a configured Regulatory verify-existing composition. Chunk 112 left that Regulatory composition unwired from runtime. A second standalone verification client or generic collection manager would duplicate the existing managed Qdrant client and violate single-ownership lifecycle discipline. Automatically creating a missing empty Regulatory collection remains forbidden because Regulatory Intelligence reads an authoritative corpus.
- **Decision:**
  - ADR-005, ADR-032, ADR-033, ADR-034, ADR-115, ADR-116, ADR-117, ADR-118, ADR-119, ADR-120, ADR-121, and ADR-122 remain Accepted and are not superseded.
  - Chunk 113 extends the existing Regulatory runtime rather than introducing a new lifecycle abstraction.
  - `loaded_regulatory_intelligence_runtime` loads exactly one additional typed settings object through the published loader `load_qdrant_document_vector_distance_settings`, using the same existing `env_file` argument already forwarded to `load_openai_settings`, `load_qdrant_settings`, and `load_regulatory_intelligence_runtime_settings`.
  - `managed_regulatory_intelligence_runtime` receives those four already-constructed settings objects. It does not load environment values.
  - After constructing the existing OpenAI and Qdrant clients and registering their cleanup immediately, the managed runtime awaits `verify_configured_regulatory_intelligence_collection_ready` once with that exact managed Qdrant client, the injected `RegulatoryIntelligenceRuntimeSettings`, and the injected `QdrantDocumentVectorDistanceSettings`.
  - Only after successful verification does it delegate to `build_regulatory_intelligence_configured_runtime` and yield `RegulatoryIntelligenceQueryExecutionService`.
  - If collection verification raises, the exact exception propagates unchanged, the configured builder is not called, no service is yielded, and both already-created clients still close according to existing `AsyncExitStack` cleanup. There is no local wrap, translation, retry, fallback, or collection creation.
  - There is no second Qdrant client, no standalone verification runtime, and no direct call to mapper / Chunk 107 / ensure / create from the managed or loaded runtime.
  - `create_app()`, `production_lifespan.py`, and `regulatory_intelligence_lifespan.py` are not given a second verify call. Production application lifespan therefore performs Regulatory collection readiness verification indirectly through the already-published chain: production lifespan → Regulatory lifespan → loaded runtime → managed runtime.
  - Document Index writer: missing collection → ensure/create-if-missing may be valid through the already-published Chunk 111 path. Regulatory reader: missing or incompatible collection → fail closed. These policies are intentional and independent. There is no shared collection name, vector-size, or lifecycle invariant.
  - Regulatory collection automatic provisioning, shared Regulatory/index collection identity or vector-size policy, collection migration, recreate/update/delete, race retry/locking, aliases, payload indexes, schema versioning, OCR, automatic corpus scanning/indexing, Pricing & Sales, and Regulatory LangGraph wiring remain out of scope. There is no default distance.
...
## ADR-138 — Forecasting LangGraph invocation does not choose execution order or apply the success transition

- **Status:** Accepted
- **Context:** ADR-128 through ADR-137 remain Accepted and are not superseded. They published two agent-specific forecast model request DTOs, two application agents that delegate through those ports, an unwired `ForecastingPlan`, an unwired `ForecastingSuccess`, an unwired `ForecastingExecutionPort`, an unwired `ForecastingWorkflowContextPort`, an application-owned `ForecastingWorkflowStep` that composes resolve → execute → record and returns the original `WorkflowState` unchanged, and a separate pure success transition `advance_after_forecasting` from `FORECASTING`/`RUNNING` to `RISK_AND_BID`/`RUNNING`. Phase 2 previously separated workflow-step invocation from success-transition wiring. Embedding execution-order choice, Consumer → DAM sequencing, or the next-phase mutation inside the graph node would mix routing with forecasting strategy and control-state policy. A generic node wrapper, router registry, or `WorkflowStep` hierarchy would hide the Phase-3-specific injected type.
- **Decision:**
  - ADR-128, ADR-129, ADR-130, ADR-131, ADR-132, ADR-133, ADR-134, ADR-135, ADR-136, and ADR-137 remain Accepted and are not superseded. ADR-037 remains Accepted: `WorkflowState` stays the seven-field snapshot and is not expanded with forecast payloads.
  - Chunk 128 extends existing `build_workflow_graph` with one additional required keyword-only dependency: `forecasting_step: ForecastingWorkflowStep`. There is no default. The factory does not construct the step.
...
## ADR-140 — In-memory forecasting workflow context is a local/dev reference adapter

- **Status:** Accepted
- **Context:** ADR-128 through ADR-139 remain Accepted and are not superseded. Chunk 125 published `ForecastingWorkflowContextPort` without choosing storage. Chunk 126 published `ForecastingWorkflowStep` as resolve → execute → record. Chunks 128–129 wired LangGraph to inject that already-composed step and apply `advance_after_forecasting`. Fixture-driven Phase-3 tests and local/dev orchestration still needed a deterministic context adapter before Redis or PostgreSQL is reviewed. Putting that adapter in application would mix a storage choice into the port owner. Wiring it into LangGraph or `create_app()` would freeze composition before a production store and a concrete `ForecastingExecutionPort` implementation are chosen. Encoding Consumer → DAM sequencing or `asyncio.gather`/`TaskGroup` forecast execution would decide the still-unresolved execution-order question.
- **Decision:**
  - ADR-128, ADR-129, ADR-130, ADR-131, ADR-132, ADR-133, ADR-134, ADR-135, ADR-136, ADR-137, ADR-138, and ADR-139 remain Accepted and are not superseded. ADR-037 remains Accepted: `WorkflowState` stays the seven-field snapshot and is not expanded with forecast payloads. ADR-054 remains Accepted for the Phase-2 in-memory precedent and is not superseded.
  - `ForecastingWorkflowContextPort` remains the application-facing contract. Chunk 130 adds infrastructure `InMemoryForecastingWorkflowContext` in `infrastructure/orchestration/forecasting_context.py`. It structurally implements the published port and does not inherit it, an ABC, or a generic repository/UoW/`WorkflowContextPort`.
  - The constructor accepts `plans: Mapping[str, ForecastingPlan]`, copies that mapping, and keeps the supplied plan objects by identity. Lookup and success recording key only on `WorkflowState.workflow_id`. Diagnostics, phase, status, portfolio, delivery date, and correlation ID are not part of the key. Plans are not derived from those fields or from providers.
  - `resolve_plan(*, state)` returns the stored plan for a known workflow ID. An unknown identity fails closed as existing `ResourceNotFoundError` with a stable sanitized message that does not echo workflow identity or plan contents. No default plan is invented.
  - `record_success(*, state, success)` stores the first `ForecastingSuccess` for a workflow ID. Equal retries, compared by value, are idempotent no-ops and retain the originally stored object. A different success for the same identity is existing `ConflictError` with a stable sanitized message. There is no last-write-win and no merge.
  - An infrastructure-local `asyncio.Lock` serializes check-and-write inside one process, matching the Chunk 44 Phase-2 adapter. This is not threading, multiprocessing, Redis locking, or database CAS. There is no durability or cross-process guarantee.
  - The adapter does not execute Consumer Load or DAM Price agents, does not import forecast model ports or ML, does not import LangGraph or FastAPI, does not choose forecast execution order, and does not perform a phase transition. Public operations remain only `resolve_plan` and `record_success`.
  - Graph still receives only the already-constructed `ForecastingWorkflowStep`. The graph, `create_app()`, Redis, and PostgreSQL remain unwired to this adapter. Redis/Postgres forecasting-context adapters are not authorized.
...
- **Consequences:** Already-produced Consumer Load OLS predictions can be scored as MW MAE without becoming live application inference and without comparing against persistence. Fair aligned comparison against persistence MAE is a later accepted decision (ADR-150). LightGBM/XGBoost and Phase 3 execution remain future work.

---

## ADR-150 — Consumer Load persistence-versus-trained OLS MAE comparison requires aligned artifacts, not aggregate shortcuts

- **Status:** Accepted
- **Context:** ADR-149 remains Accepted and is not superseded. Chunk 134 published persistence MAE over backtest cases. Chunk 139 published trained OLS MAE over prediction artifacts. Comparing those two aggregate MAE result objects alone does not prove they were scored over the same consumer, the same target timestamps, and the same actual MW labels. Introducing a generic `Comparator`/`Metric`/`ml/common` framework, a winner/champion field, or percentage improvement would invent model-selection policy this repository does not own. Recalculating MAE inside the comparison would mix transformation ownership.
- **Decision:**
  - ADR-008, ADR-037, ADR-128 through ADR-149 remain Accepted and are not superseded.
  - Chunk 140 adds ML-owned `ConsumerLoadPersistenceVsTrainedOLSMAEComparison` and `compare_consumer_load_persistence_vs_trained_ols_mae` under `energy_trading.ml.consumer_load`. The comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values and already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values. It does not import the Chunk 132 live inference adapter, Chunk 135 feature builder, Chunk 136 splitter, Chunk 137 fitter, or Chunk 138 predictor function.
  - Fair comparison requires a non-empty equal-length cohort, exactly one shared consumer, strictly increasing target timestamps, pairwise timestamp identity, and pairwise actual-MW identity. Predicted values are intentionally allowed to differ, including a negative finite trained prediction. The function does not sort, truncate with `zip`, intersect timestamps, apply tolerances, or drop unmatched cases.
  - After alignment succeeds, scoring is delegated to published `evaluate_previous_day_persistence_mae` and `evaluate_consumer_load_lag_24h_linear_regression_mae`. Chunk 140 does not reimplement MAE. The result fields are exactly `case_count`, `persistence_mae_mw`, and `trained_ols_mae_mw`.
  - Winner/champion selection, percentage improvement, acceptance thresholds, RMSE/MSE/MAPE/R², model serialization, and runtime wiring are not introduced. No sklearn/NumPy/pandas, LightGBM/XGBoost, generic comparison/`ml/common` framework, or live inference adapter is introduced. The comparison remains unwired from agents, API composition, FastAPI, LangGraph, and `ForecastingExecutionPort`.
...
- **Consequences:** Consumer Load now has an offline two-feature OLS parameter fit independent of split ownership, prediction, and evaluation, without rewriting the published one-feature fit. Applying those parameters to evaluation rows is a later accepted decision (ADR-154). Two-lag versus one-lag or persistence comparison, live inference, and Phase 3 execution remain future work.

---

## ADR-154 — Consumer Load 24h+168h OLS prediction is a separate offline artifact, not Chunk 138 reuse

- **Status:** Accepted
- **Context:** ADR-148 and ADR-153 remain Accepted and are not superseded. Chunk 138 published a one-feature OLS evaluation predictor over `ConsumerLoadLag24hFeatureRow` values. Chunk 143 published a two-feature OLS fit over `ConsumerLoadLag24h168hFeatureRow` training rows. Routing the richer fit through `predict_consumer_load_lag_24h_linear_regression`, widening `ConsumerLoadLag24hLinearRegressionPrediction`, clamping negative predictions, or introducing a generic `Predictor`/`Model`/`ml/common` inference framework would rewrite a published experiment contract or invent an abstraction this repository does not own. Coupling prediction to MAE, persistence comparison, or live `ConsumerLoadForecastModelPort` inference would mix later ownership.
- **Decision:**
  - ADR-008, ADR-037, ADR-128 through ADR-153 remain Accepted and are not superseded.
  - Chunk 144 adds ML-owned `ConsumerLoadLag24h168hLinearRegressionPrediction` and `predict_consumer_load_lag_24h_168h_linear_regression` under `energy_trading.ml.consumer_load`. The predictor consumes an already-constructed `ConsumerLoadLag24h168hLinearRegressionFit` plus already-built `ConsumerLoadLag24h168hFeatureRow` evaluation rows. It does not rebuild history, import the Chunk 142 splitter, or call the Chunk 143 fitter.
  - The published Chunk 138 one-feature prediction contract remains unchanged. Chunk 144 does not call `predict_consumer_load_lag_24h_linear_regression` and does not reuse `ConsumerLoadLag24hLinearRegressionPrediction`.
  - The formula is exactly `predicted_value_mw = fit.intercept_mw + fit.lag_24h_coefficient * row.lag_24h_mw + fit.lag_168h_coefficient * row.lag_168h_mw`. Evaluation identity (`consumer_id`, `target_timestamp`) and `actual_value_mw` from `row.target_value_mw` are copied. Input order is preserved.
  - `predicted_value_mw` is a signed finite `float`. A negative finite prediction is valid experimental evidence and is not clamped, discarded, or converted to `NonNegativeMW`. This artifact is not a live `LoadForecastPoint`.
  - Empty evaluation input, mixed consumers, duplicate/out-of-order target timestamps, non-finite fit parameters, and non-finite calculated predictions fail closed as existing `InvalidRequestError`. The function does not sort, group, return a partial tuple, or silently repair input.
  - Metrics, persistence or one-feature comparison, model serialization, and runtime wiring are not introduced. No sklearn/NumPy/pandas, LightGBM/XGBoost, generic `Predictor`/`Model`/`ml/common` framework, or live inference adapter is introduced. The predictor remains unwired from agents, API composition, FastAPI, LangGraph, and `ForecastingExecutionPort`.
...
## ADR-157 — Consumer Load persistence versus two-feature OLS MAE comparison requires aligned artifacts, not aggregate shortcuts

- **Status:** Accepted
- **Context:** ADR-149, ADR-150, ADR-155, and ADR-156 remain Accepted and are not superseded. Chunk 134 published persistence MAE over backtest cases. Chunk 145 published two-feature OLS MAE over prediction artifacts. Chunk 140 already established aligned persistence-versus-one-feature comparison using `PreviousDayPersistenceBacktestCase` and `evaluate_previous_day_persistence_mae`. Comparing persistence and two-feature aggregate MAE result objects alone does not prove they were scored over the same consumer, the same target timestamps, and the same actual MW labels. Widening the published Chunk 140 or Chunk 146 comparisons, introducing a generic `Comparator`/`Metric`/`ml/common` framework, a winner field, a relative-change field, one-feature participation, or a three-way comparison would rewrite a published experiment contract or invent model-selection policy this repository does not own. Recalculating MAE inside the comparison would mix transformation ownership.
- **Decision:**
  - ADR-008, ADR-037, ADR-128 through ADR-156 remain Accepted and are not superseded.
  - Chunk 147 adds ML-owned `ConsumerLoadPersistenceVsLag24h168hOLSMAEComparison` and `compare_consumer_load_persistence_vs_lag_24h_168h_ols_mae` under `energy_trading.ml.consumer_load`. The comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values and already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values. It reuses the published Chunk 140 persistence-side artifact type and Chunk 134 evaluator. It does not import feature builders, splitters, fitters, prediction functions, one-feature OLS prediction/evaluation, the Chunk 140 comparison, or the Chunk 146 comparison.
  - Fair comparison requires a non-empty equal-length cohort, exactly one shared consumer, strictly increasing target timestamps, pairwise timestamp identity, and pairwise actual-MW identity. Predicted values are intentionally allowed to differ, including a negative finite two-feature prediction. The function does not sort, truncate with `zip`, intersect timestamps, apply tolerances, or drop unmatched cases.
  - After alignment succeeds, scoring is delegated to published `evaluate_previous_day_persistence_mae` and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae`. Chunk 147 does not reimplement MAE. The result fields are exactly `case_count`, `persistence_mae_mw`, and `lag_24h_168h_mae_mw`.
  - Winner selection, relative change, acceptance thresholds, RMSE/MSE/MAPE/R², one-feature participation, three-way comparison, model serialization, and runtime wiring are not introduced. No sklearn/NumPy/pandas, LightGBM/XGBoost, generic comparison/`ml/common` framework, or live inference adapter is introduced. The comparison remains unwired from agents, API composition, FastAPI, LangGraph, and `ForecastingExecutionPort`.
- **Consequences:** Persistence and two-feature OLS MAE can be reported side by side only after identity and actual-label alignment. The comparison is evidence machinery, not a production-model choice and not empirical proof that either model is better. LightGBM/XGBoost and Phase 3 execution remain future work.

---

## ADR-158 — Consumer Load three-way persistence versus one-feature versus two-feature OLS MAE comparison requires aligned artifacts, not pairwise composition

- **Status:** Accepted
- **Context:** ADR-150, ADR-156, and ADR-157 remain Accepted and are not superseded. Chunk 134 published persistence MAE over backtest cases. Chunk 139 published one-feature OLS MAE over prediction artifacts. Chunk 145 published two-feature OLS MAE over prediction artifacts. Chunks 140, 146, and 147 each prove only a pairwise aligned cohort. Comparing three aggregate MAE result objects, or composing those pairwise comparison functions, does not prove that persistence, one-feature OLS, and two-feature OLS were scored over the same consumer, the same target timestamps, and the same actual MW labels as a single three-way artifact. Widening the published pairwise comparisons, introducing a generic `Comparator`/`Metric`/`ml/common` framework, a winner field, a relative-change field, or a ranking would rewrite a published experiment contract or invent model-selection policy this repository does not own. Recalculating MAE inside the comparison would mix transformation ownership.
- **Decision:**
  - ADR-008, ADR-037, ADR-128 through ADR-157 remain Accepted and are not superseded.
  - Chunk 148 adds ML-owned `ConsumerLoadPersistenceVsLag24hVsLag24h168hOLSMAEComparison` and `compare_consumer_load_persistence_vs_lag_24h_vs_lag_24h_168h_ols_mae` under `energy_trading.ml.consumer_load`. The comparison consumes already-produced `PreviousDayPersistenceBacktestCase` values, already-produced `ConsumerLoadLag24hLinearRegressionPrediction` values, and already-produced `ConsumerLoadLag24h168hLinearRegressionPrediction` values. It does not import feature builders, splitters, fitters, prediction functions, or the published Chunk 140, Chunk 146, or Chunk 147 comparison functions.
  - Fair comparison requires a non-empty equal-length cohort, exactly one shared consumer across all three tuples, strictly increasing target timestamps in each tuple, positional timestamp identity across all three tuples, and positional actual-MW identity across all three tuples. Predicted values are intentionally allowed to differ, including a negative finite OLS prediction. The function does not sort, truncate with `zip`, intersect timestamps, apply tolerances, or drop unmatched cases.
  - After alignment succeeds, scoring is delegated to published `evaluate_previous_day_persistence_mae`, `evaluate_consumer_load_lag_24h_linear_regression_mae`, and `evaluate_consumer_load_lag_24h_168h_linear_regression_mae`. Chunk 148 does not reimplement MAE. The result fields are exactly `case_count`, `persistence_mae_mw`, `lag_24h_mae_mw`, and `lag_24h_168h_mae_mw`.
  - Winner selection, ranking, relative change, acceptance thresholds, RMSE/MSE/MAPE/R², model serialization, and runtime wiring are not introduced. No sklearn/NumPy/pandas, LightGBM/XGBoost, generic comparison/`ml/common` framework, or live inference adapter is introduced. The comparison remains unwired from agents, API composition, FastAPI, LangGraph, and `ForecastingExecutionPort`.
...
## ADR-161 — Forecasting terminal failure transition is application-owned and state-only

- **Status:** Accepted
- **Context:** Chunk 150 published `ParallelForecastingExecutionService`, which concurrently executes Consumer Load and DAM agents and lets native `asyncio.TaskGroup` exception aggregation propagate. Chunk 127 already published `advance_after_forecasting` as the success replacement `forecasting`/`running` → `risk_and_bid`/`running`. Chunk 45 published `fail_parallel_ingestion` as a Phase-2-specific terminal replacement that consumes only `WorkflowState`, preserves diagnostics, and does not accept an exception argument. Putting failure methods on `WorkflowState`, catching executor exceptions inside `ForecastingWorkflowStep`, appending `AdapterDiagnostic` values from raw TaskGroup leaves, or inventing Consumer Load versus DAM precedence would freeze diagnostic authorship, classification, and branch-winner policy before those concerns are reviewed. Reusing `ParallelIngestionFailureFact` for Phase 3 would mix Phase-2 identity into forecasting. A second generic failure-policy hierarchy would duplicate ADR-029.
- **Decision:**
  - ADR-008, ADR-037, ADR-055, ADR-128 through ADR-160 remain Accepted and are not superseded.
  - Application owns `fail_after_forecasting(state: WorkflowState) -> WorkflowState` in `forecasting_failure_transition.py`. It is one Phase-3-specific function, not a method on `WorkflowState`, not a Protocol, not a factory, and not a generic state machine. It stays separate from `advance_after_forecasting`. It consumes only an already-valid forecasting/running `WorkflowState` and performs the terminal control-state replacement. It does not consume a sanitized failure fact, a forecasting failure fact, or classified branch-failure information.
  - The function is valid only when `state.phase is WorkflowPhase.FORECASTING` and `state.status is WorkflowStatus.RUNNING`. Any other phase or status fails closed as existing `InvalidRequestError` with a stable sanitized message that does not include workflow identity, correlation ID, diagnostics, or exception internals. A previously failed snapshot cannot be failed again through this function.
  - A valid call returns a **new** frozen `WorkflowState` with `phase = WorkflowPhase.FORECASTING` and `status = WorkflowStatus.FAILED`. Phase remains forecasting because this is a terminal outcome of the current Phase 3 attempt, not a progression into risk and bid. Replacement uses `dataclasses.replace`. The input object is not mutated. `workflow_id`, `portfolio_id`, `delivery_date`, `correlation_id`, and `diagnostics` are preserved exactly.
  - Diagnostics remain untouched because converting runtime failures into canonical diagnostics is a separate orchestration concern and has not been specified for Phase 3. The function does not accept an exception, TaskGroup, `ExceptionGroup`, or error argument. Raw TaskGroup/exception objects do not enter `WorkflowState`.
  - Chunk 150 executor semantics remain intact: it continues to propagate failures and does not call this transition. `ForecastingWorkflowStep` continues to own resolve → execute → record and does not catch executor failures. There is no branch-precedence rule and no Consumer Load versus DAM winner.
  - No forecasting-specific failure-fact DTO is introduced. Phase 2 already owns sanitized `ParallelIngestionFailureFact` for attributed ingestion leaves. Classification of forecasting TaskGroup failures, failure-policy decision, retry/fallback, context write, and LangGraph wiring remain separately reviewed chunks.
  - `WorkflowState` remains the published seven-field snapshot. `WorkflowPhase` and `WorkflowStatus` members are unchanged. LangGraph remains `forecasting` → `forecasting_success_transition` → `END` on the success path and does not import or call `fail_after_forecasting`.
...
## ADR-172 — The no-retry Phase 3 runtime resolves the initial attempt as one

- **Status:** Accepted
- **Context:** `FailurePolicyContext` requires an attempt number. The application now has a Phase 3 attempt-number port, but runtime retry execution and attempt tracking do not exist. Introducing mutable or persistent tracking before retries exist would design unsupported semantics. Reusing `InitialParallelIngestionAttemptNumberSource` as the production Phase 3 class would mix Phase 2 identity into forecasting.
- **Decision:**
  - ADR-008, ADR-037, ADR-055, ADR-057, ADR-068, ADR-128 through ADR-171 remain Accepted and are not superseded.
  - Application owns LangGraph-free `InitialForecastingAttemptNumberSource` in `forecasting_initial_attempt_number_source.py`.
  - The class satisfies existing `ForecastingAttemptNumberPort` structurally and does not inherit the Protocol.
  - The method remains async `get_attempt_number(self, workflow_id: str) -> int`.
  - The source is stateless and returns exactly `1` for every workflow identity.
  - The workflow ID remains opaque: it is not stripped, normalized, validated, hashed, stored, or mapped.
  - There is no increment, reset, persistence, per-workflow map, or retry ownership.
  - The Phase 2 source remains a separately named class.
- **Consequences:** The existing single-failure Phase 3 application pipeline can construct a real `FailurePolicyContext` using concrete selection and concrete initial-attempt resolution when a caller injects this source behind the published port. This source is intentionally insufficient for future retry execution. Retry-capable attempt tracking requires separate architectural review.

---

## ADR-173 — The current no-retry Phase 3 failure policy always terminates

- **Status:** Accepted
- **Context:** Phase 3 can now prepare a real `FailurePolicyContext`. Retry execution does not exist, fallback execution does not exist, retry-capable attempt tracking does not exist, and retryability semantics are not approved. Reusing `InitialParallelIngestionFailurePolicy` as the production Phase 3 class would mix Phase 2 identity into forecasting.
- **Decision:**
  - ADR-008, ADR-037, ADR-055, ADR-057, ADR-068, ADR-070, ADR-128 through ADR-172 remain Accepted and are not superseded.
  - Application owns LangGraph-free `InitialForecastingFailurePolicy` in `forecasting_initial_failure_policy.py`.
  - The class satisfies existing `FailurePolicyPort` structurally and does not inherit the Protocol.
  - The method remains async `decide(self, context: FailurePolicyContext) -> FailureAction`.
  - The policy is stateless and returns exactly `FailureAction.FAIL` for every valid context.
  - It does not branch on phase, error code, attempt number, or agent.
...
- **Consequences:** The current no-retry/no-fallback Phase 3 runtime has an honest initial policy decision. The system does not silently pretend retries or fallback are supported. Future RETRY/FALLBACK behavior requires separate architectural review plus supporting runtime mechanics. Outer application raw-runtime composition now exists independently of LangGraph. LangGraph forecasting-failure routing remains deferred.
...
- **Decision:**
  ...
  - `WorkflowState` remains the existing seven-field snapshot.
- **Consequences:** Phase 3 can obtain a policy decision from a prepared context. Production still lacks LangGraph forecasting-failure routing, RETRY execution, FALLBACK execution, and retry-capable attempt tracking.
...
- **Decision:**
  ...
  - `WorkflowState` remains the existing seven-field snapshot.
- **Consequences:** Application callers can apply a terminal Phase 3 `FAIL` decision in application code without a graph runtime. Production still lacks LangGraph forecasting-failure routing, RETRY execution, FALLBACK execution, and retry-capable attempt tracking.

---

## ADR-176 — Prepared Phase 3 failure handling is composed before runtime exception routing

- **Status:** Accepted
- **Context:** Chunk 164 published `ForecastingFailureDecisionService`, which returns an already-decided `FailureAction` from a prepared `FailurePolicyContext`. Chunk 165 published `execute_forecasting_failure_action`, which applies terminal `FAIL` and rejects `RETRY`/`FALLBACK`. Folding those two steps into context preparation, raw-exception routing, or LangGraph would freeze independently reviewable concerns: how a prepared context is decided, how that action is applied, and when graph catch/conditional routing exists. Reusing `ParallelIngestionFailureHandlingService` as the production Phase 3 class would mix Phase 2 identity into forecasting. Injecting `FailurePolicyPort` or constructing `InitialForecastingFailurePolicy` inside the handling service would couple composition to one concrete policy and skip the published decision seam.
- **Decision:**
  - ADR-008, ADR-037, ADR-055, ADR-058, ADR-128 through ADR-175 remain Accepted and are not superseded.
  - Application owns `ForecastingFailureHandlingService` in `forecasting_failure_handling.py`. It is one Phase-3-specific concrete class, not a Protocol, ABC, dataclass, generic, registry, factory, policy implementation, or Phase 2 alias.
  - Constructor injects exactly `ForecastingFailureDecisionService`. It does not inject `FailurePolicyPort`, does not construct `InitialForecastingFailurePolicy`, and does not own an action-executor object.
  - The only public operation is `async handle(self, *, state: WorkflowState, context: FailurePolicyContext) -> WorkflowState`. The caller must supply an already-prepared published context and a `WorkflowState`. The service does not construct `FailurePolicyContext`, inspect raw exceptions or `BaseExceptionGroup`, or inspect `context.phase`, `context.error_code`, `context.attempt_number`, or `context.agent_name`.
  - `handle` awaits the injected `ForecastingFailureDecisionService.decide(context)` exactly once, forwards the returned `FailureAction` unchanged to `execute_forecasting_failure_action(state=state, action=action)` together with the exact supplied state, and returns the exact resulting `WorkflowState`. There is no action branching, no direct `fail_after_forecasting` call, no diagnostics mutation, and no retry/fallback mechanics.
  - Decision-service exceptions propagate unchanged. Action-execution errors propagate unchanged. The service does not catch, wrap, translate, retry, or return a default action.
  - The Phase 2 handling service remains a separately named class. LangGraph, FastAPI, infrastructure, and ML remain unwired.
  - `WorkflowState` remains the existing seven-field snapshot.
- **Consequences:** Phase 3 now supports complete handling of an already-prepared context through decision and terminal action execution. Production still lacks LangGraph forecasting-failure capture/routing, RETRY execution, FALLBACK execution, and retry-capable attempt tracking. Composition from a raw forecasting exception group through context preparation into handling is owned separately from this prepared-context seam.

---

## ADR-177 — Phase 3 raw failure handling is composed inside application before LangGraph routing

- **Status:** Accepted
- **Context:** Chunk 161 published `ForecastingFailureContextPreparationService`, which maps a `BaseException` into `FailurePolicyContext`. Chunk 166 published `ForecastingFailureHandlingService`, which maps an already-prepared context plus `WorkflowState` through decision and terminal action execution. Folding raw exception-group capture into those inner seams, or into LangGraph, would freeze independently reviewable concerns: how a group becomes a context, how a prepared context becomes a state replacement, and when graph catch/conditional routing exists. Reusing `ParallelIngestionFailureRuntimeHandlingService` as the production Phase 3 class would mix Phase 2 identity into forecasting. Injecting policy, selector, attempt source, or action-executor objects into the outer composer would skip the published preparation and handling seams.
- **Decision:**
  - ADR-008, ADR-037, ADR-055, ADR-058, ADR-071, ADR-128 through ADR-176 remain Accepted and are not superseded.
  - Application owns `ForecastingFailureRuntimeHandlingService` in `forecasting_failure_runtime_handling.py`. It is one Phase-3-specific concrete class, not a Protocol, ABC, dataclass, generic, registry, factory, policy implementation, or Phase 2 alias.
  - Constructor injects exactly `ForecastingFailureContextPreparationService` and `ForecastingFailureHandlingService`. It does not inject `FailurePolicyPort`, a selector, an attempt source, a concrete policy, or an action-executor object, and it does not depend on LangGraph.
  - The only public operation is `async handle(self, *, state: WorkflowState, failure_group: BaseExceptionGroup) -> WorkflowState`.
  - `handle` awaits the injected preparation service exactly once with `workflow_id=state.workflow_id`, `phase=state.phase`, and `error=failure_group`, forwards the exact original `state` plus the exact returned `FailurePolicyContext` to the injected handling service, and returns the exact resulting `WorkflowState`.
  - The service does not extract forecasting failures, classify failures, inspect nested groups, inspect `__cause__`, inspect exception text or class names, select facts, resolve attempt numbers, construct `FailurePolicyContext`, invoke `FailurePolicyPort` or `ForecastingFailureDecisionService`, call `execute_forecasting_failure_action` or `fail_after_forecasting`, branch on `FailureAction`, mutate diagnostics, mutate or replace state, implement RETRY/FALLBACK, implement retry counters, or catch/wrap/translate exceptions.
