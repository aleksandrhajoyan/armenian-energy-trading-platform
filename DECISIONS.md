...
- **Decision:**
  ...
  - Production Qdrant modules still do not recreate, update, or delete collections and still do not hardcode distance enum members. Chunk 105 may inspect `VectorParams` in the read-only collection-readiness verifier to confirm unnamed dense-vector dimension; it must not construct `VectorParams` or import `Distance`. Chunk 106 may construct unnamed dense `VectorParams` and call `create_collection` once from `collection_creation.py` using a caller-supplied `Distance`; it must not default a metric, probe existence, or ensure/reconcile collections. Automatic production collection provisioning and production distance policy remain deferred.
...
- **Consequences:** Explicit callers can preflight an already-provisioned document collection without mutating Qdrant or choosing a distance. Chunk 106 later added a separate create-only mutation; the verifier remains independent and still does not create collections. Automatic production collection provisioning, production distance selection, payload-index provisioning, automatic startup checks, and Pricing & Sales remain deferred. Regulatory Intelligence parent capability remains incomplete.
