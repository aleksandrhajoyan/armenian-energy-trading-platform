...
- Architecture test (`tests/architecture/test_qdrant_client_boundary.py`): domain/application/API/ML import neither `qdrant_client` nor Qdrant infrastructure. `QdrantSettings` imports none of the Qdrant SDK, FastAPI, Redis, SQLAlchemy, or LLM libraries. Qdrant infrastructure may import `qdrant_client` and settings but not FastAPI, SQLAlchemy, Redis, agents, LangGraph, ML, NumPy, FastEmbed, PDF/OCR, or application document embedding/index/search contracts. `create_app()` remains unwired. Production source rejects `cloud_inference=True`, `models.Document`, and FastEmbed.
- Qdrant live tests still do not exist.
