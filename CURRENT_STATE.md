...
Everything after Chunk 31 in `ROADMAP.md`. Next is **next Phase 4 implementation slice to be selected after Chunk 31 review**. Phase 4 has begun; it is not complete. Two concrete application agents exist and both remain provider-neutral and unwired. Orchestration still invokes no agents. Do not claim the trading workflow runs end-to-end. Do not install embedding SDKs or ML stacks until those chunks.
...
- Agent invocation is application-owned: `AgentName` plus structurally typed `AgentPort[TRequest, TResult]`; only `name` and async `run(request)` are shared; no registry, retry, or fallback. Two concrete agents exist (`WeatherAndRenewableForecastAgent`, `HydroResourcesAgent`); the other 11 remain absent
...
Two concrete application agents exist:
...
The other 11 concrete agents remain absent. Thirteen canonical identities are captured by application `AgentName`. Future agents will structurally satisfy `AgentPort` and consume canonical contracts through application ports; they must not parse raw source schemas.
