...
- Query-preparation wiring into Regulatory Intelligence
...
- Query-text embedding composition into Regulatory Intelligence
...
- Application-owned `DocumentVectorSearchQueryPreparationService` (injects `DocumentQueryEmbeddingPort`; keyword-only `async prepare(*, query_text, limit) -> DocumentVectorSearchQuery`; query text forwarded unchanged; existing query-contract limit validation preserved; no search invocation; unwired from Regulatory Intelligence, graph, and API)
- Query-preparation unit and architecture-boundary tests (structural embedding fake; no live provider, search, or Regulatory wiring)
...
- Document vector-search query preparation architecture tests (service injects only `DocumentQueryEmbeddingPort`; `prepare` returns `DocumentVectorSearchQuery`; no search/Regulatory/provider/graph/API wiring)
...
Everything after Chunk 65 in `ROADMAP.md`. Next is **next Phase 5 Regulatory + Pricing slice or remaining Phase 4 parallel-ingestion work after Chunk 65 review**. Phase 5 has begun. Phase 4's parallel-ingestion parent remains incomplete: remaining failure/retry/degraded gaps did not disappear because Phase 5 work started.
...
- Query-search-query preparation is an application-owned `DocumentVectorSearchQueryPreparationService` over that port plus an explicit caller-supplied `limit`, returning existing `DocumentVectorSearchQuery`; it does not search, rewrite query text, default/clamp limits, or wire Regulatory Intelligence, graph, or API; no embedding provider exists
...
- Regulatory Intelligence is application-owned: `RegulatoryIntelligenceAgent` consumes `RegulatoryIntelligenceRequest` (`search_query: DocumentVectorSearchQuery`), calls existing `DocumentVectorSearchPort.search` once, and — when chunks exist — calls `RegulatoryConstraintInferencePort.infer` once; empty retrieval returns no constraints and does not invoke inference. The agent does not construct `RegulatoryConstraint` values, hardcode Armenian DAM rules, embed query text, or import Qdrant/LLM SDKs. No concrete inference provider, PDF/OCR, graph node, or API wiring
...
6. Regulatory Intelligence Agent (`RegulatoryIntelligenceAgent`). It is provider-neutral and unwired: it consumes `RegulatoryIntelligenceRequest`, calls existing `DocumentVectorSearchPort` with the supplied `DocumentVectorSearchQuery`, and — when retrieved chunks exist — calls `RegulatoryConstraintInferencePort`. Empty retrieval returns `constraints=()` and does not invoke inference. The agent does not construct `RegulatoryConstraint` values, hardcode Armenian DAM rules, embed query text, or call an LLM SDK. Query-text embedding exists as a separate unwired `DocumentQueryEmbeddingPort`. Query preparation exists as a separate unwired `DocumentVectorSearchQueryPreparationService` that can compose query text plus an explicit limit into `DocumentVectorSearchQuery`; this agent still expects an already-built `DocumentVectorSearchQuery` and does not inject that service. No concrete inference provider, PDF/OCR, actual RAG runtime, graph/API wiring, or verified Armenian rule extraction exists.
