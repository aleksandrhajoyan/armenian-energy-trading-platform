...
- Concrete query-text embedding provider / implementation
...
Everything after Chunk 67 in `ROADMAP.md`. Next is **next Phase 5 Regulatory + Pricing slice or remaining Phase 4 parallel-ingestion work after Chunk 67 review**. Phase 5 has begun. Phase 4's parallel-ingestion parent remains incomplete: remaining failure/retry/degraded gaps did not disappear because Phase 5 work started.
...
- Query-text embedding is an application-owned `DocumentQueryEmbeddingPort` over already-normalized query text returning `DocumentQueryEmbedding`; it is not document-chunk embedding, not vector search, not Qdrant, not a domain contract, and has no concrete provider/model
- Query-search-query preparation is an application-owned `DocumentVectorSearchQueryPreparationService` over that port plus an explicit caller-supplied `limit`, returning existing `DocumentVectorSearchQuery`; it does not search, rewrite query text, default/clamp limits, or inject into `RegulatoryIntelligenceAgent`; no embedding provider exists
- Regulatory query execution is an application-owned `RegulatoryIntelligenceQueryExecutionService` over that preparation service plus the unchanged `RegulatoryIntelligenceAgent`; keyword-only `execute(*, query_text, limit)` constructs existing `RegulatoryIntelligenceRequest` and returns existing `RegulatoryIntelligenceResult` unchanged; it does not embed, search, infer, rewrite query text, or default/clamp limits; graph and `create_app()` remain unwired; no embedding or inference provider exists
- Regulatory runtime composition is an API-owned synchronous `build_regulatory_intelligence_query_execution` over the three published application ports; it constructs query preparation, the agent, and query execution without provider construction, settings/environment access, I/O, or runtime method calls; application does not import this builder; `create_app()` does not invoke it
