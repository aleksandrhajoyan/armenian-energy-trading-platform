...
- Production OpenAI client lifecycle, adapter injection, model runtime selection, and Regulatory composition for query embedding and inference
- Graph/API/LangGraph wiring of `RegulatoryIntelligenceQueryExecutionService`
- `create_app()` wiring of `build_regulatory_intelligence_query_execution`
...
- Regulatory Intelligence graph/API wiring
...
- OpenAI query-embedding architecture tests (SDK confined to the three approved OpenAI infrastructure modules; application/domain/API OpenAI-free; `create_app()` and `graph.py` unwired)
...
- OpenAI regulatory-inference architecture tests (SDK confined to the three approved OpenAI infrastructure modules; application/domain/API OpenAI-free; `create_app()`, Chunk 67 builder, and `graph.py` unwired)
...
- OpenAI query-embedding adapter architecture tests (`OpenAIDocumentQueryEmbeddingAdapter` lives in `infrastructure/embeddings`; OpenAI SDK imports are confined to the three approved OpenAI infrastructure modules; application/domain/API remain OpenAI-free; the adapter does not construct the client; `create_app()` and `graph.py` remain unwired)
- OpenAI regulatory-constraint inference adapter architecture tests (`OpenAIRegulatoryConstraintInferenceAdapter` lives in `infrastructure/regulatory`; OpenAI SDK imports are confined to the three approved OpenAI infrastructure modules; public result is `tuple[RegulatoryConstraint, ...]`; the adapter does not construct the client; evidence checking is infrastructure-local; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired)
- OpenAI client-foundation architecture tests (`OpenAISettings` lives in `shared/config` and imports no OpenAI SDK; `create_openai_client` lives in `infrastructure/openai`; SDK retries disabled; no env access in the factory; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired)
...
Six concrete application agents exist. The five Phase 2 ingestion agents remain provider-neutral. LangGraph does not import them; they are invoked only if a caller composes the execution port behind `ParallelIngestionWorkflowStep`. They remain unwired to API, providers, persistence, and n8n. Regulatory Intelligence is also provider-neutral at the application layer and unwired from LangGraph and `create_app()`: it composes existing document-vector retrieval with an injected inference port. Infrastructure now has an OpenAI inference adapter behind that port, but the application agent does not import OpenAI. Application can compose query text + limit → `DocumentVectorSearchQueryPreparationService` → `RegulatoryIntelligenceRequest` → `RegulatoryIntelligenceAgent` → `RegulatoryIntelligenceResult` through `RegulatoryIntelligenceQueryExecutionService`. The API-owned `build_regulatory_intelligence_query_execution` constructs that stack from three injected application ports. Infrastructure `OpenAIDocumentQueryEmbeddingAdapter` can satisfy the query-embedding port when an `AsyncOpenAI` client and model are supplied. Infrastructure `OpenAIRegulatoryConstraintInferenceAdapter` can satisfy the inference port when an `AsyncOpenAI` client and model are supplied. That is object composition plus unwired provider adapters, not production RAG readiness: OpenAI client **construction capability** now exists as typed `OpenAISettings` plus lazy `create_openai_client()`, but production client lifecycle, adapter injection, model runtime selection, Regulatory composition, Qdrant/API provider wiring, and LangGraph Phase 1 wiring remain absent. Do not claim the trading workflow runs end-to-end.
...
- Query-search-query preparation is an application-owned `DocumentVectorSearchQueryPreparationService` over that port plus an explicit caller-supplied `limit`, returning existing `DocumentVectorSearchQuery`; it does not search, rewrite query text, default/clamp limits, or inject into `RegulatoryIntelligenceAgent`; OpenAI adapter injection and model runtime selection remain deferred
- Regulatory query execution is an application-owned `RegulatoryIntelligenceQueryExecutionService` over that preparation service plus the unchanged `RegulatoryIntelligenceAgent`; keyword-only `execute(*, query_text, limit)` constructs existing `RegulatoryIntelligenceRequest` and returns existing `RegulatoryIntelligenceResult` unchanged; it does not embed, search, infer, rewrite query text, or default/clamp limits; graph and `create_app()` remain unwired; OpenAI adapter injection and model runtime selection remain deferred
...
- Regulatory Intelligence is application-owned: `RegulatoryIntelligenceAgent` consumes `RegulatoryIntelligenceRequest` (`search_query: DocumentVectorSearchQuery`), calls existing `DocumentVectorSearchPort.search` once, and — when chunks exist — calls `RegulatoryConstraintInferencePort.infer` once; empty retrieval returns no constraints and does not invoke inference. The agent does not construct `RegulatoryConstraint` values, hardcode Armenian DAM rules, embed query text, or import Qdrant/LLM SDKs. Query text + limit reach this unchanged agent through `RegulatoryIntelligenceQueryExecutionService`. Outer `build_regulatory_intelligence_query_execution` wires those objects from three injected ports. Infrastructure `OpenAIDocumentQueryEmbeddingAdapter` can implement the query-embedding port; it is not constructed by the builder or `create_app()`. Infrastructure `OpenAIRegulatoryConstraintInferenceAdapter` can implement the inference port; it is not constructed by the builder or `create_app()`. No PDF/OCR, graph node, or `create_app()` runtime wiring
...
6. Regulatory Intelligence Agent (`RegulatoryIntelligenceAgent`). It is provider-neutral and unwired from graph and `create_app()`: it consumes `RegulatoryIntelligenceRequest`, calls existing `DocumentVectorSearchPort` with the supplied `DocumentVectorSearchQuery`, and — when retrieved chunks exist — calls `RegulatoryConstraintInferencePort`. Empty retrieval returns `constraints=()` and does not invoke inference. The agent does not construct `RegulatoryConstraint` values, hardcode Armenian DAM rules, embed query text, or call an LLM SDK. Query-text embedding exists as a separate `DocumentQueryEmbeddingPort`. Query preparation exists as a separate `DocumentVectorSearchQueryPreparationService`. Application query execution composition exists as `RegulatoryIntelligenceQueryExecutionService`. Outer `build_regulatory_intelligence_query_execution` constructs that stack from the three injected application ports without provider construction or I/O. This agent still expects an already-built `DocumentVectorSearchQuery` and does not inject those services. Infrastructure `OpenAIDocumentQueryEmbeddingAdapter` structurally implements the query-embedding port from an injected `AsyncOpenAI` client and explicit model; it is not wired into the builder, `create_app()`, or LangGraph. Infrastructure `OpenAIRegulatoryConstraintInferenceAdapter` structurally implements the inference port from an injected `AsyncOpenAI` client and explicit model; it converts structured provider output into canonical `RegulatoryConstraint` after an evidence-reference check and is not wired into the builder, `create_app()`, or LangGraph. An offline typed OpenAI client-construction foundation exists (`OpenAISettings`, `create_openai_client`) and is not used by this agent, either adapter, the builder, `create_app()`, or LangGraph. The application agent still depends only on `RegulatoryConstraintInferencePort` and does not import OpenAI. No PDF/OCR, actual RAG runtime, graph/`create_app()` wiring, or verified Armenian rule extraction exists.
