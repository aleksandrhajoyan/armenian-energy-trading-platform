...
- Regulatory Intelligence query-execution architecture tests (service injects exactly `DocumentVectorSearchQueryPreparationService` and `RegulatoryIntelligenceAgent`; `execute` returns `RegulatoryIntelligenceResult`; no embedding/search/inference ports, providers, graph, or API wiring)
...
Everything after Chunk 66 in `ROADMAP.md`. Next is **next Phase 5 Regulatory + Pricing slice or remaining Phase 4 parallel-ingestion work after Chunk 66 review**. Phase 5 has begun. Phase 4's parallel-ingestion parent remains incomplete: remaining failure/retry/degraded gaps did not disappear because Phase 5 work started.
...
Six concrete application agents exist. The five Phase 2 ingestion agents remain provider-neutral. LangGraph does not import them; they are invoked only if a caller composes the execution port behind `ParallelIngestionWorkflowStep`. They remain unwired to API, providers, persistence, and n8n. Regulatory Intelligence is also provider-neutral and unwired: it composes existing document-vector retrieval with an injected inference port, is not on the LangGraph graph, and has no concrete LLM or Qdrant composition. Application can compose query text + limit → `DocumentVectorSearchQueryPreparationService` → `RegulatoryIntelligenceRequest` → `RegulatoryIntelligenceAgent` → `RegulatoryIntelligenceResult` through `RegulatoryIntelligenceQueryExecutionService` when dependencies are injected. That is application composition, not production RAG readiness: there is no concrete embedding provider, no concrete inference provider, no Qdrant/API composition, and no LangGraph Phase 1 wiring. Do not claim the trading workflow runs end-to-end. Do not install embedding SDKs or ML stacks until those chunks.
