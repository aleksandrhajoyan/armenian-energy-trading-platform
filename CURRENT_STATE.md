...
- **Current phase:** Phase 3 — Application orchestration foundation (begun; not complete)
...
Everything after Chunk 28 in `ROADMAP.md`. Next is **Retries / fallback policy hooks**. Phase 3 has begun; it is not complete. Do not claim the trading workflow runs end-to-end. Do not claim a concrete agent exists. Do not install embedding SDKs or ML stacks until those chunks.
...
- LangGraph skeleton is application-owned: `build_workflow_graph()` compiles `START → workflow_entry → END`; `workflow_entry` is async and no-op; no agents, phase routing, retries, fallback, checkpointer, store, cache, or API wiring
