...
- Multi-failure selection policy after ExceptionGroup extraction
...
- Parallel-ingestion plan/success/execution-port/executor/context-port/workflow-step/success-transition/failure-transition/failure-decision/failure-context/failure-action/failure-handling/agent-failure/exception-group-extraction/failure-fact-classification/tuple-failure-classification unit and architecture tests (real request/result DTOs; real agents behind source fakes; structural context/execution/policy fakes; no live services)
...
→ multi-failure selection still missing
