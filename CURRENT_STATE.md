...
- LangGraph node / wiring for `ParallelIngestionWorkflowStep`
- Workflow phase/status transition after Phase 2
- Concrete parallel-ingestion workflow-context implementation
...
- Factory `build_workflow_graph()` compiling a fresh graph over `WorkflowState`
- Single async no-op node `workflow_entry` with topology `START → workflow_entry → END`
- LangGraph skeleton unit and architecture tests (async `ainvoke`, state/diagnostic preservation, import ownership)
...
- Concrete application `ConcurrentParallelIngestionExecutor` (injects the five Phase 2 application agents; `asyncio.TaskGroup`; all-success only; LangGraph-free)
- Application-owned non-generic `ParallelIngestionWorkflowContextPort` (`async resolve_plan(workflow_id: str) -> ParallelIngestionPlan` and `async record_success(workflow_id: str, success: ParallelIngestionSuccess) -> None`; Protocol only; storage-neutral; no concrete implementation)
...
- Parallel-ingestion plan/success/execution-port/executor/context-port/workflow-step unit and architecture tests (real request/result DTOs; real agents behind source fakes; structural context/execution fakes; no live services)
...
Everything after Chunk 40 in `ROADMAP.md`. Next is **next Phase 4 implementation slice to be selected after Chunk 40 review**. Phase 4 has begun; it is not complete.

NOW implemented: concurrent invocation of the five Phase 2 application agents through `ConcurrentParallelIngestionExecutor`, all-success collection into `ParallelIngestionSuccess`, a typed workflow-context boundary (`ParallelIngestionWorkflowContextPort`) for resolving a Phase 2 plan and recording all-success output, and a framework-neutral workflow step (`ParallelIngestionWorkflowStep`) composing resolve → execute → record.

STILL absent: concrete context implementation; LangGraph Phase 2 node; LangGraph invocation of the executor or workflow step; graph join; workflow phase/status transition; retry/fallback execution; degraded-mode semantics; concrete Chief Orchestrator; API/composition wiring; providers/persistence/n8n workflows.

Five concrete application agents remain provider-neutral and unwired to LangGraph, API, providers, persistence, and n8n. Do not claim the trading workflow runs end-to-end. Do not install embedding SDKs or ML stacks until those chunks.
...
- Parallel Phase 2 execution is application-owned: non-generic `ParallelIngestionExecutionPort` exposes only `async execute(plan: ParallelIngestionPlan) -> ParallelIngestionSuccess`. Concrete `ConcurrentParallelIngestionExecutor` injects the five application agents and uses `asyncio.TaskGroup` for the all-success path only. Native TaskGroup failure propagation applies. There is no retry/fallback/degraded policy, no LangGraph wiring, and no `WorkflowState` expansion
- Parallel Phase 2 workflow context is application-owned: non-generic `ParallelIngestionWorkflowContextPort` exposes only `async resolve_plan(workflow_id) -> ParallelIngestionPlan` and `async record_success(workflow_id, success) -> None`. `workflow_id` reuses the published `WorkflowState.workflow_id` type. Plan and success stay outside the seven-field `WorkflowState` snapshot. The port is storage-neutral and has no concrete implementation, no LangGraph wiring, and no failure/degraded semantics
- Parallel Phase 2 workflow composition is application-owned: concrete `ParallelIngestionWorkflowStep` injects exactly `ParallelIngestionWorkflowContextPort` and `ParallelIngestionExecutionPort`. `async run(state: WorkflowState) -> WorkflowState` is resolve → execute → record, then return of the original state object. There is no phase/status mutation, no `FailurePolicyPort`, no LangGraph wiring, and no retry/fallback/degraded policy
...
None of these agents is invoked by LangGraph. `ConcurrentParallelIngestionExecutor` can invoke all five concurrently from a typed `ParallelIngestionPlan` and return `ParallelIngestionSuccess` when every branch succeeds; it is not wired into the graph, API, or composition root. `ParallelIngestionWorkflowContextPort` can resolve that plan and record that success by workflow identity without embedding Phase 2 payloads in `WorkflowState`; there is no concrete context implementation. `ParallelIngestionWorkflowStep` composes resolve → execute → record and returns the original `WorkflowState` unchanged; LangGraph does not call it. Failure/degraded fan-in policy does not exist. None of these agents constitutes complete production ingestion.
