...
- OpenAI client construction, API-key loading, and production model/runtime composition for query embedding
...
- Concrete regulatory LLM/inference provider
...
- Official `openai==3.8.0` (`>=3.8.0`) as an infrastructure-only query-embedding dependency
...
- OpenAI query-embedding architecture tests (SDK confined to the adapter; application/domain/API OpenAI-free; `create_app()` and `graph.py` unwired)
...
- OpenAI query-embedding adapter architecture tests (`OpenAIDocumentQueryEmbeddingAdapter` lives in `infrastructure/embeddings`; OpenAI SDK imports are confined there; application/domain/API remain OpenAI-free; no client/API-key construction; `create_app()` and `graph.py` remain unwired)
...
Everything after Chunk 68 in `ROADMAP.md`. Next is **next Phase 5 Regulatory + Pricing slice or remaining Phase 4 parallel-ingestion work after Chunk 68 review**. Phase 5 has begun. Phase 4's parallel-ingestion parent remains incomplete: remaining failure/retry/degraded gaps did not disappear because Phase 5 work started.
...
Six concrete application agents exist. The five Phase 2 ingestion agents remain provider-neutral. LangGraph does not import them; they are invoked only if a caller composes the execution port behind `ParallelIngestionWorkflowStep`. They remain unwired to API, providers, persistence, and n8n. Regulatory Intelligence is also provider-neutral and unwired from LangGraph and `create_app()`: it composes existing document-vector retrieval with an injected inference port and has no concrete LLM or Qdrant composition. Application can compose query text + limit → `DocumentVectorSearchQueryPreparationService` → `RegulatoryIntelligenceRequest` → `RegulatoryIntelligenceAgent` → `RegulatoryIntelligenceResult` through `RegulatoryIntelligenceQueryExecutionService`. The API-owned `build_regulatory_intelligence_query_execution` constructs that stack from three injected application ports. Infrastructure `OpenAIDocumentQueryEmbeddingAdapter` can satisfy the query-embedding port when an `AsyncOpenAI` client and model are supplied. That is object composition plus an unwired provider adapter, not production RAG readiness: there is no OpenAI client/API-key/model runtime composition, no concrete inference provider, no Qdrant/API provider wiring, and no LangGraph Phase 1 wiring. Do not claim the trading workflow runs end-to-end.
