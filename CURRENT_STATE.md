...
- Fan-in / parallel-ingestion result contract
...
- Application-owned frozen `ParallelIngestionPlan` (exactly the five existing Phase 2 request DTOs; no execution, no fan-in, no LangGraph)
- Parallel-ingestion-plan unit and architecture tests (real request DTOs; no live services)
...
Everything after Chunk 35 in `ROADMAP.md`. Next is **next Phase 4 implementation slice to be selected after Chunk 35 review**. Phase 4 has begun; it is not complete. Five concrete application agents exist and all remain provider-neutral and unwired. An application-owned `ParallelIngestionPlan` can hold their five typed requests; parallel execution and fan-in/join are still not implemented. LangGraph still invokes zero agents. There is no concrete Chief Orchestrator. No degraded/retry/fallback execution exists. Do not claim the trading workflow runs end-to-end. Do not install embedding SDKs or ML stacks until those chunks.
...
- Parallel Phase 2 fan-out planning is application-owned: frozen `ParallelIngestionPlan` composes the five existing typed request DTOs; it is not `WorkflowState`, does not execute agents, does not use LangGraph, does not define fan-in/results, and does not derive source-scope IDs from `portfolio_id`
...
None of these agents is invoked by LangGraph. `ParallelIngestionPlan` can carry the five existing typed requests together; it does not execute them, join their results, or wire the graph. None of these agents constitutes complete production ingestion.
