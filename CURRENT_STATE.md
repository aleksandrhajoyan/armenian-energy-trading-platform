...
- Parallel-ingestion plan/success unit and architecture tests (real request/result DTOs; no live services)
...
Everything after Chunk 36 in `ROADMAP.md`. Next is **next Phase 4 implementation slice to be selected after Chunk 36 review**. Phase 4 has begun; it is not complete. Five concrete application agents exist and all remain provider-neutral and unwired. Application-owned `ParallelIngestionPlan` can hold their five typed requests and `ParallelIngestionSuccess` can hold their five typed results for the all-success case; parallel execution, Phase 2 join, and failure/degraded fan-in semantics are still not implemented. LangGraph still invokes zero agents. There is no concrete Chief Orchestrator. No degraded/retry/fallback execution exists. Do not claim the trading workflow runs end-to-end. Do not install embedding SDKs or ML stacks until those chunks.
...
None of these agents is invoked by LangGraph. `ParallelIngestionPlan` can carry the five existing typed requests together and `ParallelIngestionSuccess` can carry the five existing typed results for the all-success case; neither executes agents, joins at runtime, or wires the graph. Failure/degraded fan-in policy does not exist. None of these agents constitutes complete production ingestion.
