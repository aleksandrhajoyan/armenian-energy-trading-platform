...
- OpenAI client construction, API-key loading, and production model/runtime composition for query embedding and regulatory inference
...
- Concrete `OpenAIDocumentQueryEmbeddingAdapter` structurally implementing `DocumentQueryEmbeddingPort` (injected `AsyncOpenAI`, explicit model, `encoding_format="float"`, no client construction or API-key loading)
- OpenAI query-embedding unit tests (fake async client; no live OpenAI, no API key)
- OpenAI query-embedding architecture tests (SDK confined to approved OpenAI adapter modules; application/domain/API OpenAI-free; `create_app()` and `graph.py` unwired)
- Concrete `OpenAIRegulatoryConstraintInferenceAdapter` structurally implementing `RegulatoryConstraintInferencePort` (injected `AsyncOpenAI`, explicit model, Responses `parse` structured output, infrastructure-local evidence-reference check, canonical `RegulatoryConstraint` mapping, no client construction or API-key loading)
- OpenAI regulatory-inference unit tests (fake async client; no live OpenAI, no API key)
- OpenAI regulatory-inference architecture tests (SDK confined to the two approved OpenAI adapter modules; application/domain/API OpenAI-free; `create_app()`, Chunk 67 builder, and `graph.py` unwired)
...
- OpenAI query-embedding adapter architecture tests (`OpenAIDocumentQueryEmbeddingAdapter` lives in `infrastructure/embeddings`; OpenAI SDK imports are confined to the two approved OpenAI adapter modules; application/domain/API remain OpenAI-free; no client/API-key construction; `create_app()` and `graph.py` remain unwired)
- OpenAI regulatory-constraint inference adapter architecture tests (`OpenAIRegulatoryConstraintInferenceAdapter` lives in `infrastructure/regulatory`; OpenAI SDK imports are confined to the two approved OpenAI adapter modules; public result is `tuple[RegulatoryConstraint, ...]`; no client/API-key construction; evidence checking is infrastructure-local; `create_app()`, Chunk 67 builder, and `graph.py` remain unwired)
...
- Query-text embedding is an application-owned `DocumentQueryEmbeddingPort` over already-normalized query text returning `DocumentQueryEmbedding`; it is not document-chunk embedding, not vector search, not Qdrant, and not a domain contract. Infrastructure `OpenAIDocumentQueryEmbeddingAdapter` structurally implements that port with an injected `AsyncOpenAI` client and explicit model; it does not load API keys or construct the client
- Query-search-query preparation is an application-owned `DocumentVectorSearchQueryPreparationService` over that port plus an explicit caller-supplied `limit`, returning existing `DocumentVectorSearchQuery`; it does not search, rewrite query text, default/clamp limits, or inject into `RegulatoryIntelligenceAgent`; OpenAI runtime client composition remains deferred
- Regulatory query execution is an application-owned `RegulatoryIntelligenceQueryExecutionService` over that preparation service plus the unchanged `RegulatoryIntelligenceAgent`; keyword-only `execute(*, query_text, limit)` constructs existing `RegulatoryIntelligenceRequest` and returns existing `RegulatoryIntelligenceResult` unchanged; it does not embed, search, infer, rewrite query text, or default/clamp limits; graph and `create_app()` remain unwired; OpenAI client/runtime composition remains deferred
